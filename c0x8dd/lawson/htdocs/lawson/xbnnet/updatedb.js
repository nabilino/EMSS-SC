...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xbnnet/updatedb.js,v 1.22.2.69.2.2 2014/08/19 15:21:53 brentd Exp $
//***************************************************************
//*                                                             *
//*                           NOTICE                            *
//*                                                             *
...
{	
	...
	{			
		...
		var msg = '<div class="panewrapper"><form name="optionsFrm" style="display:inline">' 
		+ '<div role="radiogroup"  aria-labelledby="printLbl">'
		+ '<div id="printLbl" style="padding-top:5px">'+getSeaPhrase("UPDATEBEN_14","BEN")+'</div>'
		+ '<div class="dialogSecondaryTextStyler" style="padding-top:5px;padding-left:25px"><input type="radio" id="printYes" name="radioPrint" value="Y" styler="radio"/><label for="printYes">'+getSeaPhrase("YES","BEN")+'</label></div>'
		+ '<div class="dialogSecondaryTextStyler" style="padding-top:5px;padding-left:25px"><input type="radio" id="printNo" name="radioPrint" value="N" styler="radio" checked/><label for="printNo">'+getSeaPhrase("NO","BEN")+'</label></div>'
		+ '</div>';
		// give option to send email if email address and process flow both exist
		if (allowEmail)
		{
			msg += '<div role="radiogroup"  aria-labelledby="emailLbl">'
			msg += '<div id="emailLbl" style="padding-top:5px">'+getSeaPhrase("UPDATEBEN_52","BEN")+'</div>'
			+ '<div class="dialogSecondaryTextStyler" style="padding-top:5px;padding-left:25px"><input type="radio" id="emailYes" name="radioEmail" value="Y" styler="radio"/><label for="emailYes">'+getSeaPhrase("YES","BEN")+'</label></div>'
			+ '<div class="dialogSecondaryTextStyler" style="padding-top:5px;padding-left:25px"><input type="radio" id="emailNo" name="radioEmail" value="N" styler="radio" checked/><label for="emailNo">'+getSeaPhrase("NO","BEN")+'</label></div>'
			+ '</div>';
		}		
		msg += '</form></div>';
		var secondaryMsg = "";
		if (onQuit)
			secondaryMsg = getSeaPhrase("UPDATEBEN_11","BEN")+" "+getSeaPhrase("UPDATEBEN_12","BEN");
		...
		messageDialog.styleDialog = function(wnd)
		{
			...
			if (typeof(wnd["stylerWnd"]) == "undefined" && typeof(window["findStyler"]) == "function")
		...
		var msgObj = messageDialog.messageBox(msg, "ok", "question", window.stylerWnd, false, secondaryMsg, confirmExitPrint);
...
function setMsgDlgBtn(phraseID)
{
	messageDialog.initDialog = function(wnd)
	{
		...
		{
			this.setButtons([
			     {id: "yes", name: "yes", text: getSeaPhrase(phraseID, "BEN"), click: null}				
			]);					
		}
	}	
...
function ChoiceContents(onQuit)
{
	...
	var html = '<div role="radiogroup" class="plaintablecell" style="padding:0px">'
 	html += '<form name="optionsFrm" style="display:inline">'
 	html += '<table class="plaintableborder" border="0" cellpadding="0" cellspacing="0" style="width:100%;margin-left:auto;margin-right:auto" role="presentation">'
	html += '<tr><td class="plaintableheaderborder">'+getSeaPhrase("ELECTION_PRINT","BEN")+'</td></tr>'
	if (text)
		html += '<tr><td class="plaintablecellboldborder" style="border-bottom:0px"><p>'+text+'<p/></td></tr>'
	if (text2)
		html += '<tr><td class="plaintablecellborder" style="border-bottom:0px">'+text2+'<p/></td></tr>'
	html += '</tr><tr><td class="plaintablecellborder">'+getSeaPhrase("UPDATEBEN_14","BEN")
	html += '<div class="plaintablecell" style="padding-top:5px;padding-left:25px"><input type="radio" id="printYes" name="radioPrint" value="Y"/><label for="printYes"><span class="offscreen">'+getSeaPhrase("UPDATEBEN_14","BEN")+'&nbsp;</span>'+getSeaPhrase("YES","BEN")+'</label></div>'
	html += '<div class="plaintablecell" style="padding-left:25px"><input type="radio" id="printNo" name="radioPrint" value="N" styler="radio" checked/><label for="printNo"><span class="offscreen">'+getSeaPhrase("UPDATEBEN_14","BEN")+'&nbsp;</span>'+getSeaPhrase("NO","BEN")+'</label></div>'
	html += '</td></tr>'
	if (allowEmail)
	{
		html += '<tr><td class="plaintablecellborder">'
		html += getSeaPhrase("UPDATEBEN_52","BEN")+'<div class="plaintablecell" style="padding-top:5px;padding-left:25px"><input type="radio" id="emailYes" name="radioEmail" value="Y"/><label for="emailYes"><span class="offscreen">'+getSeaPhrase("UPDATEBEN_52","BEN")+'&nbsp;</span>'+getSeaPhrase("YES","BEN")+'</label></div>'
		html += '<div class="plaintablecell" style="padding-left:25px"><input type="radio" id="emailNo" name="radioEmail" value="N" checked/><label for="emailNo"><span class="offscreen">'+getSeaPhrase("UPDATEBEN_52","BEN")+'&nbsp;</span>'+getSeaPhrase("NO","BEN")+'</label></div>'
		html += '</td></tr>'	
	}
...
function startUpdate()
{
	...
	// if you call getAppVersion() right away and the IOS object isn't set up yet, then the code will be trying to load the sso.js file, and your call for
	...
	parent.startProcessing(getSeaPhrase("UPDATEBEN_17","BEN"), continueUpdate);
}
...
function finishupdate()
{
	if (updatetype=="ERR" && lastFunc=="addbensImmediate" && !senttoPending)
	{
		...
		senttoPending=false
		errorMsg=getSeaPhrase("UPDATEBEN_18","BEN")+" "+getSeaPhrase("COMPANY","BEN")+" "+parent.company
		+", "+getSeaPhrase("EMPLOYEE","BEN")+" "+parent.employee+" "+parent.firstname+" "+parent.lastname+"\n\n"+getSeaPhrase("UPDATEBEN_43","BEN")
		addbensPending()
		return
	}
		...
		if (planstoStop[cnt5]!=null && planstoStop[cnt5][0]!=null)
		{
			...
			for (var k=1; k<21; k++)
			{
				if (cnt5<planstoStop.length)
				{
					...
					else if ((parseFloat(planstoStop[cnt5][0])>1 && parseFloat(planstoStop[cnt5][0])<6) || parseFloat(planstoStop[cnt5][0])==13)
					{
					...
					else if (parseFloat(planstoStop[cnt5][0])>5 && parseFloat(planstoStop[cnt5][0])<11)
					{
					...
					else if (parseFloat(planstoStop[cnt5][0])==12)
					{
			...
			while (k<6 && !done)
			{
				if (cnt2-1<parent.oldelected.length)
				{
					if (shouldBeDeletedFromPending(parent.oldelected[cnt2-1]) == true)
					{
						...
						else if ((parseFloat(parent.oldelected[cnt2-1][0])>1 && parseFloat(parent.oldelected[cnt2-1][0])<6) || parseFloat(parent.oldelected[cnt2-1][0])==13)
						{
						...
						else if (parseFloat(parent.oldelected[cnt2-1][0])>5 && parseFloat(parent.oldelected[cnt2-1][0])<11)
						{
						...
						else if (parseFloat(parent.oldelected[cnt2-1][0])==12)
						{
...
function FormatPlan(index,plans)
{
	...
	if (parseFloat(PlanArray[index][0])==5)
	{
		...
		arr[8]=escape(parent.roundToPennies(PlanArray[index][2][14]))   //cover amount
		arr[24]=escape(parent.roundToPennies(PlanArray[index][2][20]))	//flex cost
		arr[25]=escape(parent.roundToPennies(PlanArray[index][2][18]))	//emp cost
		arr[26]=escape(parent.roundToPennies(PlanArray[index][2][24]))	//comp cost
	}
	if (parseFloat(PlanArray[index][0])==6)
	{
		...
		arr[10]=escape(PlanArray[index][2][24])
		if (typeof(PlanArray[index][2][40])!="undefined" && PlanArray[index][2][40]!=null && PlanArray[index][2][40]!="")
			arr[6]=escape(PlanArray[index][2][40])			//nbr cycles
		if (typeof(PlanArray[index][2][43])!="undefined" && PlanArray[index][2][43]!=null && PlanArray[index][2][43]!="")
	...
	if (parseFloat(PlanArray[index][0])==8 || parseFloat(PlanArray[index][0])==9)
	{
		...
		var plandivisor=PlanArray[index][2][37]
		var paydivisor=1
		if (parent.BenefitRules[6]=="P")
			paydivisor=plandivisor
		...
		// make sure amount contributions are divisible by plan divisor
		if (PlanArray[index][2][24]!="P")
		{	
			if (empPreTaxCont!="" && !isNaN(parseFloat(vl)))
			{
				var periodamt = parent.roundToPennies(parseFloat(vl)/parseFloat(plandivisor));
				vl=periodamt*parseFloat(plandivisor);
			}
			if (empAftTaxCont!="" && !isNaN(parseFloat(v2)))
			{
				var periodamt = parent.roundToPennies(parseFloat(v2)/parseFloat(plandivisor));
				v2=periodamt*parseFloat(plandivisor);
			}
		}
		...
		else
		{
			if (empPreTaxCont!="")
			{
				// PT 166907. Update the pay period amount rather than the annual amount if the employee has elected only a pre-tax amount contribution.
				if (PlanArray[index][2][24]=="P")
				{
					//only pass emp pre cont for percent contributions; let BN31 calculate it for amounts
					arr[14]=escape(parent.roundToPennies(parseFloat(vl))) // pre-tax contribution
					arr[9]=escape(parent.roundToPennies(parseFloat(vl)))  // annual contribution
				}	
				else
					arr[8]=escape(parent.roundToPennies(parseFloat(vl)/parseFloat(plandivisor))) // pay period contribution
				arr[12]="P"
			}
			if (empAftTaxCont!="")
			{
				// PT 166907. Update the pay period amount rather than the annual amount if the employee has elected only an after-tax amount contribution. 
				if (PlanArray[index][2][24]=="P")
				{
					//only pass emp pre cont for percent contributions; let BN31 calculate it for amounts
					arr[15]=escape(parent.roundToPennies(parseFloat(v2))) // after-tax contribution
					...
					arr[8]=escape(parent.roundToPennies(parseFloat(v2)/parseFloat(plandivisor))) // pay period contribution
				arr[12]="A"
			}
		}
...
function updateDepBens()
{
	...
	else
	{
		for (var i=0; i<parent.DependentBens.length; i++)
		{
			...
			if (parent.DependentBens[i]==null || typeof(parent.DependentBens[i])=="undefined" || parent.DependentBens[i][0]==3)
				continue;
			...
			if (parent.DependentBens[i][0]!=5)	//if they did not choose to add or change dependents
			{
				...
				{
					// PT 101599: only re-add a dependent benefit if a new employee benefit is being added
					if (newEmpPlanAdded)
					{
			...
			else
			{
				...
				// if a new employee benefit is not being added, create new dependent benefit records for those that the user has added, leave the ones that have not been changed,
				// and change the ones that have been dropped otherwise, create new add records for all dependents (loop below this one)
				if (!newEmpPlanAdded && parent.oldDepBens[i])
				{
					for (var x=0; x<parent.oldDepBens[i].length; x++)
					{
						...
						if (stopdep==0)
						{
							...
							parent.DependentBens[i][1][parent.DependentBens[i][1].length-1].startdate=parent.formjsDate(parent.formatDME(parent.oldDepBens[i][x].start_date)+"")
							if (parent.event == "annual" || parent.rule_type == "N")
				...
				for (var x=0; x<parent.DependentBens[i][1].length; x++)
				{
					...
					if (parent.DependentBens[i][1][x].fc=="x")
					{
						...
						{
							//PT 165880.  Use the life event change date set up on BS03.2 for any dependents being added to an existing employee benefit.
	...
	else
	{
		...
		if (typeof(tempDepBens[cnt6])!="undefined" && tempDepBens[cnt6]!=null && tempDepBens[cnt6][1][0]!=null)
		{
			...
			if (cnt6<tempDepBens.length)
			{
				...
				{
					if (typeof(tempDepBens[cnt6][1])!="undefined" && typeof(tempDepBens[cnt6][1][x])!="undefined" && tempDepBens[cnt6][1][x].fc!="x" 
					&& tempDepBens[cnt6][1][x].fc!="" && cnt44<11)
					{
						callit=1
	...
	else
	{
		...
		if (typeof(tempDepBens[cnt6])!="undefined")
		{
			...
			for (var z=cnt6; z<tempDepBens.length; z++)
			{
				...
				{
					if (typeof(tempDepBens[z][1])!="undefined" && typeof(tempDepBens[z][1][x])!="undefined" && tempDepBens[z][1][x].fc!="x" 
					&& tempDepBens[z][1][x].fc!="" && cnt33<19)
					{
						callit=1
						var arr=new Array()
