...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xhrnet/qualifications.js,v 1.10.2.7 2010/01/13 20:17:09 brentd Exp $
...
function Do_Edu_Call(qualform, program, fc, type, ee, noProcessingMsg)
{
	...
	{	
		if (isFile)
		{
			ptFields += "&PT-EDU-COM-DATE="+((NonSpace(qualform.renew_date) == 0) ? "00000000" : ParseAGSValue(formjsDate(qualform.renew_date)))		
			// PT 149297. Explicitly zero out the date, if blank.
			pAgsObj.field += "&EDU-COM-DATE"+rowNbr+"1="+((NonSpace(qualform.renew_date) == 0) ? "00000000" : ParseAGSValue(formjsDate(qualform.renew_date)))		
		...
		{	
			ptFields += "&PT-EDU-COM-DATE="+((NonSpace(qualform.renewaldate.value) == 0) ? "00000000" : ParseAGSValue(formjsDate(qualform.renewaldate.value)))
			// PT 149297. Explicitly zero out the date, if blank.
			pAgsObj.field += "&EDU-COM-DATE"+rowNbr+"1="+((NonSpace(qualform.renewaldate.value) == 0) ? "00000000" : ParseAGSValue(formjsDate(qualform.renewaldate.value)))
	...
	pAgsObj.debug     	= false
	if (!noProcessingMsg) {
		showWaitAlert(getSeaPhrase("QUAL_35","ESS"),null,true,"font-size:16pt;font-face:arial;")
	}
...
function Do_Cmp_Call(qualform, program, fc, type, ee, noProcessingMsg)
{
	...
	if (fc != "D")
	{
		if (isFile)
		{
			ptFields += "&PT-DATE-ACQUIRED="+ParseAGSValue(formjsDate(qualform.date_acquired))
			...
			+ "&SKI-DATE-ACQUIRED"+rowNbr+"1="+ParseAGSValue(formjsDate(qualform.date_acquired))
			+ "&SKI-DATE-LST-USED"+rowNbr+"1="+ParseAGSValue(formjsDate(qualform.renew_date))
			+ "&EPC-DATE-RETURNED"+rowNbr+"1="+ParseAGSValue(formjsDate(qualform.date_returned))
		}
		else
		{
			...
			ptFields += "&PT-DATE-ACQUIRED="+ParseAGSValue(formjsDate(qualform.dateacquired.value))
			...
			+ "&SKI-DATE-ACQUIRED"+rowNbr+"1="+ParseAGSValue(formjsDate(qualform.dateacquired.value))
			+ "&SKI-DATE-LST-USED"+rowNbr+"1="+ParseAGSValue(formjsDate(qualform.renewdate.value))
			+ "&EPC-DATE-RETURNED"+rowNbr+"1="+ParseAGSValue(formjsDate(qualform.datereturned.value))
		}
...
// program	... form id that we are updating against (PA21.1 or PA34.1)
// type		... default is "form"; if = "file", reference EMPCODES fields
function Do_Crt_Call(qualform, program, fc, type, ee, noProcessingMsg)
{
	program = program.toUpperCase();
	var isFile = (type && type=="file")?true:false;
	var func = program.split(".").join("_");
	...
	if (fc != "D")
	{
		if (isFile)
		{
			ptFields += "&PT-CER-DATE-ACQUIRED="+ParseAGSValue(formjsDate(qualform.date_acquired))
			...
			+ "&CER-DATE-ACQUIRED"+rowNbr+"1="+ParseAGSValue(formjsDate(qualform.date_acquired))
			+ "&CER-RENEW-DATE"+rowNbr+"1="+ParseAGSValue(formjsDate(qualform.renew_date))
		...
		{	
			...
			ptFields += "&PT-CER-DATE-ACQUIRED="+ParseAGSValue(formjsDate(qualform.dateacquired.value))
			
			pAgsObj.field += "&CER-LIC-NUMBER"+rowNbr+"1="+ParseAGSValue(qualform.licnumber.value,1)
			+ "&CER-DATE-ACQUIRED"+rowNbr+"1="+ParseAGSValue(formjsDate(qualform.dateacquired.value))
			+ "&CER-RENEW-DATE"+rowNbr+"1="+ParseAGSValue(formjsDate(qualform.renewdate.value))
...
function BuildRenewalSelect(value)
{
	if (!RenewalCodesExist)
	{
		...
		renewalCodes = new Array("  ","2Y","3Y", "4Y", "5Y", "6Y", "7Y", "8Y", "9Y",
									"AN", "QT", "SA");
