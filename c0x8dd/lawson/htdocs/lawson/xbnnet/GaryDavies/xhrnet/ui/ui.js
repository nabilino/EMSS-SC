...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xhrnet/ui/ui.js,v 1.34.2.111.4.3 2012/05/02 20:34:18 brentd Exp $
...
function findStyler(searchOpener, ref)
{
	...
	try
	{
		// does the current window have a StylerEMSS object ?
		if (typeof(ref["StylerEMSS"]) == "function" || typeof(ref["StylerEMSS"]) == "object")
		{
			return ref;
		}
		// is there a parent ?
		else if (ref != ref.parent)
		{
			...
			return findStyler(searchOpener, ref.parent);
		}
	}
	catch (e)
	{}
	...
	try
	{
		...
		{
			// does the opener have a StylerEMSS object ?
			if (typeof(ref["opener"]["StylerEMSS"]) == "function" || typeof(ref["opener"]["StylerEMSS"]) == "object")
			{
				// found a copy...
				return ref.opener;
			}
			else
			{
				...
				return findStyler(searchOpener, ref.opener);
			}
		}
	}
	catch (e)
...
function findTheme(searchOpener, ref)
{
	...
	try
	{
		...
		if (typeof(ref["theme"]) != "undefined" && ref.theme && ref.theme != "")
		{
			return ref.theme;
		}
		// is there a parent ?
		...
		{
			// didn't find it... try higher
			return findTheme(searchOpener, ref.parent);
		}
	}
	catch (e)
	{}
	...
	try
	{
		...
		if (searchOpener && ref.opener)
		{
			...
			if (typeof(ref["opener"]["theme"]) != "undefined" && ref.opener.theme && ref.opener.theme != "")
			{
				// found a copy...
				return ref.opener.theme;
			}
			else
			{
				// didn't find it... try higher on the opener
				return findTheme(searchOpener, ref.opener);
			}
		}
	}
	catch (e)
...
function findAuthWnd(searchOpener, ref)
{
	...
	{
		// does the current window have theme object ?
		if (typeof(ref["authUser"]) == "object" && ref.authUser != null)
		{
			return ref;
		}
		// is there a parent ?
		...
		{
			// didn't find it... try higher
			return findAuthWnd(searchOpener, ref.parent);
		}
	}
	catch (e)
	{}
	...
	try
	{
		...
		if (searchOpener && ref.opener)
		{
			// does the opener have an authUser object ?
			if (typeof(ref.opener["authUser"]) == "object" && ref.opener.authUser != null)
			{
				// found a copy...
				return ref.opener;
			}
			...
			{
				// didn't find it... try higher on the opener
				return findAuthWnd(searchOpener, ref.opener);
			}
		}
	}
	catch (e)
...
{		
	...
	window.styler.setByURL(window, window.theme);						

	if (window.styler.showLDS && titleBar)
	{
    	...
    	var titleBarElm = null;
    	var winRef = parent;
		
    	// find the window that contains the title bar
    	while (winRef != winRef.parent && titleBarElm == null)
    	{
       		if (winRef.document.getElementById("titleBarContentList"))
       		{
       		    titleBarElm = winRef.document.getElementById("titleBarContentList");
           	}
           	else
           	{
		...
		// provide a logout button if this is top level window running on LSF 9
		try
		{
			if (window.stylerWnd.parent == window.stylerWnd || typeof(top.stylerWnd) == "object")
			{
				if (titleBarElm != null && window.stylerWnd.iosHandler && window.stylerWnd.iosHandler.getIOS() != null && window.stylerWnd.iosHandler.getIOSVersionNumber() >= "8.1")
				{
					var toolTip = (typeof(window.stylerWnd["getSeaPhrase"]) == "function") ? window.stylerWnd.getSeaPhrase("LOGOUT_LBL","ESS") : "";
					window.styler.addTitleBarIcon(winRef, window.stylerWnd.StylerEMSS.TITLE_BAR_LOGOFF_ICON, emssLogout, toolTip);
				}	
			}
		}
		catch(e) {}
	...
	else if (titleBar)
	{
		...
		try
		{
			if (window.stylerWnd.parent == window.stylerWnd || typeof(top.stylerWnd) == "object")
			{
				if (window.stylerWnd.iosHandler && window.stylerWnd.iosHandler.getIOS() != null && window.stylerWnd.iosHandler.getIOSVersionNumber() >= "8.1")
				{
					var logoutDiv = window.stylerWnd.document.createElement("div");
					var logoutSpan = window.stylerWnd.document.createElement("span");
					var logoutMsg = window.stylerWnd.getSeaPhrase("LOGOUT","ESS") || "[logout]";
					logoutDiv.className = "logoutDiv";
					logoutSpan.className = "logoutSpan";
					logoutSpan.innerHTML = logoutMsg + "&nbsp;";
					logoutSpan.onclick = emssLogout;
...
function styleElement(stylerElement)
{
	...
	if (!stylerElement || !window.styler || !window.styler.showLDS)
	{
		return;
	}
...
function resizeFrame(expandToFit, expandOnly)
{
    ...
    if (window.styler && window.styler != null && window.styler.showLDS)
    {
          return;
    }
...
function setLayerSize(docTag)
{
	...
	{
		docTag.style.width = (window.innerWidth-37)+"px";
	}
	...
	else if (docTag.className == "helpicon")
	{
		...
		docTag.style.left = (window.innerWidth-42)+"px";
	}
	else if (docTag.className == "logosmall")
	{
		docTag.style.left = (window.innerWidth-140)+"px";
	}
	...
	{
		docTag.style.left = (window.innerWidth-49)+"px";
	}	
...
function displaySwitchClick(id, openMsg, closeMsg, openImg, closeImg)
{
...
function setTabContents(tab,tabFrame)
{
	...
	try {
		if (window.navigator.appName == "Microsoft Internet Explorer") {
			//tabFrame.document.getElementById(tabId+"_Body_"+tabNbr).style.overflow = "hidden";
...
function setTabContentSizes(tabId, tabFrame, tabWidth, tabHeight)
{
	...
	while (!done)
	{
		try
		{
			...
			{
				if (tabWidth - 10 > 0)
					contentFrame.style.width = (tabWidth - 20) + "px";
				if (tabHeight - 60 > 0)
					contentFrame.style.height = (tabHeight - 60) + "px";
				if (contentFrame.style.display != "none")
					contentFrame.style.overflow = "auto";
			}
			else
...
function uiTableHeader(innerText,textAlign)
{
	var str = '<td class="tableheaderleft">'
	str+='<div class="tableheaderleft"></div>'
	str+='</td><td class="tableheaderbody"'
	str+=(textAlign)?' align="'+textAlign+'">':'>'
	str+=(innerText)?innerText:''
	str+='</td><td class="tableheaderright">'
	str+='<div class="tableheaderright"></div>'
	str+='</td>'
	return str;
}

function uiTableCell(innerText,textAlign)
{
	var str = '<td class="tablecellleft">'
	str+='&nbsp;</td><td class="tablecellbody"'
	str+=(textAlign)?' align="'+textAlign+'">':'>'
	str+=(innerText)?innerText:''
	str+='</td><td class="tablecellright">&nbsp;</td>'
	return str;
}

function uiTableRowHighlight(innerText)
{
	var str = '<tr class="tablerowhighlight">'
	str+=(innerText)?innerText:''
	return str;
}
...
function uiRequiredIcon(altStr)
{
	var str='<span style="margin-left:5px" styler="asterisk"><img class="requiredicon" border="0" alt="'
...
function uiHelpIcon(action)
{
	var str='<img border="0" class="helpicon" onclick="'+action+'" alt="'+getSeaPhrase("HELP","ESS")+'" src="/lawson/xhrnet/ui/images/ico_help_ltblue.gif"'
	str+=' onmouseover=this.src="/lawson/xhrnet/ui/images/ico_help_ltblue_over.gif"'
	str+=' onmousedown=this.src="/lawson/xhrnet/ui/images/ico_help_ltblue_down.gif"'
	str+=' onmouseout=this.src="/lawson/xhrnet/ui/images/ico_help_ltblue.gif">'
	return str;
}
...
function uiHeaderPane(strHdr,strBody)
{
	var str='<html>'
	str+='<head>'
	str+='<title>header pane</title>'
	str+='<link rel="stylesheet" type="text/css" href="/lawson/xhrnet/ui/ui.css"/>'
	str+='<script src="/lawson/xhrnet/ui/ui.js"></script>'
	str+='</head>'
	str+='<body onload="setLayerSizes()">'

	str+='<div id="paneBorder" class="paneborder">'
	str+='<table id="paneTable" border="0" height="100%" width="100%" cellpadding="0" cellspacing="0">'
	str+='<tr><td style="height:10px">'
		str+='<div id="paneHeader" class="paneheader">'
		str+=(strHdr)?strHdr:'&nbsp;'
		str+='</div>'
	str+='</td></tr>'
	str+='<tr><td style="padding-bottom:5px;padding-left:5px;padding-right:5px">'
		str+='<div id="paneSubLeftCorner" class="panesubleftcorner">'
		str+='</div>'
		str+='<div id="paneSubRightCorner" class="panesubrightcorner">'
		str+='</div>'
		str+='<div id="paneBodyBorder" class="panebodyborder">'
			str+='<div id="paneBody" class="panebody">'
			str+=(strBody)?strBody:''
			str+='</div>'
		str+='</div>'
	str+='</td></tr>'
	...
	str+='<div id="paneBorderLeftCorner" class="paneborderleftcorner">'
	str+='</div>\n'

	str+='<div id="paneBorderRightCorner" class="paneborderrightcorner" style="top:5px">'
	str+='</div>\n'
...
function uiWrite(frameObj,layerId,text)
{
	try {
		frameObj.document.getElementById(layerId).innerHTML = text;
		frameObj.setLayerSizes();
		return true;
	}
...
{
	var str = '<span class="datestring" style="top:3px;padding-bottom:3px;padding-left:5px" id="dateString">('+uiDateString()+')</span>'
	return str;
}

function uiDateFormatSpan2()
{
	var str = '<span class="datestring" style="padding-bottom:3px;padding-left:5px" id="dateString">('+uiDateString()+')</span>'
	return str;
}

function uiDateFormatSpan3()
{
	var str = '<span class="datestring" style="padding-bottom:3px;padding-left:5px;padding-right:5px" id="dateString">('+uiDateString()+')</span>'
	return str;
}
...
function activateMultipleTableRow(tableId,rowNbr,frameObj,isHeader,hasBorder)
{
	try {
		var tblObj;
		var tblRows;
		...
		if (tblObj.getAttribute("currentRow") || tblObj.getAttribute("currentRow") == "0")
		{
			...
			oldRow.className = "";
			oldCells[oldCells.length-1].className = (isHeader)?((hasBorder)?"plaintableheaderborder":"plaintableheader"):((hasBorder)?"plaintablecellborder2":"plaintablecell");
			if(typeof(window[tableId+"_OnRowDeactivate"])=="function")
			{
				if(!eval(tableId+"_OnRowDeactivate("+currRow+")"))
					return;
		...
		tblObj.setAttribute("currentRow",rowNbr);
		tblRow.className = "tablerowhighlight";
		for(i=0;i<tblCells.length;i++){
			tblCells[i].className = (isHeader)?((hasBorder)?"plaintableheaderborderactive":"plaintableheaderactive"):((hasBorder)?"plaintablecellborderactive2":"plaintablecellactive");
		}
		if(typeof(window[tableId+"_OnRowActivate"])=="function")
		{
			if(!eval(tableId+"_OnRowActivate("+rowNbr+")"))
...
function setRequiredField(textBox)
{
	...
	if (typeof(window["styler"]) != "undefined" && window.styler && window.styler.showLDS)
	{
		return;
	}
	else
	{
		if (textBox.className != "" && textBox.className.indexOf("requiredinputbox") != -1)
		{
			return;
		}
		...
		try 
		{
			if (textBox.className != "" && textBox.className.indexOf("plaintablecell") != -1)
			{
				textBox.className += " requiredinputbox";
			}
			else
			{
				textBox.className = "requiredinputbox";
...
function clearRequiredField(textBox)
{
	...
	else
	{
		...
		try 
		{
			// PT 139267. Fix select box jumping issue.
			if (textBox.className.indexOf("plaintablecell") != -1) 
			{
				textBox.className = textBox.className.replace(" requiredinputbox", "");
			} 
			else 
			{
...
{	
	...
	if (typeof(frameObj["styler"]) == "undefined" || !frameObj.styler)
	{
		frameObj.stylePage(true);
	}
	else
	{
		var titleBarElm = frameObj.parent.document.getElementById("titleBarContentList");
		if (!titleBarElm)
		{
	...
	if (typeof(frameObj["styler"]) == "undefined" || !frameObj.styler || !frameObj.styler.showLDS)
	{	
	...
	else if (typeof(window["styler"]) != "undefined")
	{
    		...
    		if (typeof(window["getSeaPhrase"]) == "function" && typeof(window["authUser"]) != "undefined")
		{
...
function uiDetailTable(strHeading,strBody)
{
	...
	+ '<div class="paneheaderlite" style="margin:0px;padding-bottom:4px;padding-left:7px;padding-right:5px;padding-top:4px">'+((strHeading)?strHeading:'&nbsp;')+'</div>'
	+ '</td></tr>'
	+ '<tr><td>'
...
{
	// PT 136896. Strip off outer styled DIV objects, as they affect browser print formatting.
	// Before this fix, strBody was wrapped in these inside the table cell.
	//+ '<div class="panebodyborder"><div class="panebody" style="overflow:hidden">'
      	//+ '</div></div>'

	var strHtml = '<table border="0" cellspacing="0" cellpadding="0" width="100%">'
	+ '<tr><td class="paneheader" style="margin:0px;height:16px;padding-bottom:4px;padding-left:7px;padding-right:5px;padding-top:4px">'
...
function blurTextBox(obj)
{
	try
	{
		obj.className = "inputboxdisabled";
	}
	catch(e)
	{}
}
...
function doFilter(filterForm, recordForm, recordElement, winRef, filterType)
{
	...
	{
        	var retVal = eval("dmeFilter.getLaunchedFrom().validateDmeFieldFilter(dmeFilter, filterForm);");
        	if (retVal == false) {
        		return false;
        	}
	}
	...
	if (typeof(dmeFilter.getLaunchedFrom().performDmeFieldFilter) != "undefined")
	{
		...
		eval("dmeFilter.getLaunchedFrom().performDmeFieldFilter(dmeFilter);");
		return true;
	}
	return false;
}
...
function setFilterOptions(fieldNm, selectedFld, selObj)
{
	...
	if (fldObj && (fldObj.getAttribute("isNumeric") == "true")) {
	...
	if (fldObj && (isNaN(parseInt(fldObj.getAttribute("fieldSize"), 10)) == false)) {
		...
		if (keywords.value.length > fieldSize) {
	...
	{
		eval("dmeFilter.getLaunchedFrom().performDmeFieldFilterOnChange(dmeFilter);");
		return true;
	}	
}
...
function dmeFilterOnLoad(filterForm, recordForm, recordElement, winRef, filterType)
{
	...
	{
		if (eval("dmeFilter.getLaunchedFrom().performDmeFieldFilterOnLoad(dmeFilter);") != false)
		{
			dmeFilter.enable();	
			return true;
		}
...
function positionFilterIFrame(wnd, frameElm, fieldNm)
{
	...
	if (typeof(wnd["getDmeFieldElement"]) == "function")
	{
		...
		if (fldWin && inputFld && typeof(fldWin["PositionObject"]) != "undefined")
		{
			...
			if (rightSpace < 0)
			{
				leftPos = frameLeft + inputFldPos.right - filterWidth + leftPadding + rightPadding;
...
function openDmeFieldFilter(fieldNm, noProcessing)
{
	...
	{		
		...
		var urlStr = "/lawson/xhrnet/ui/filter.htm?recs=" + filterNode.getAttribute("recs_per_page");
		if (typeof(fieldNm) != "undefined") {
			urlStr += "&fieldNm=" + fieldNm.toString();
		}
		try {
...
function openDmeListFieldFilter(frameNm, fieldNm, noProcessing)
{
	...
	var urlStr = "/lawson/xhrnet/ui/list.htm?recs=" + nbrRecs;	
	if (typeof(fieldNm) != "undefined") {
		urlStr += "&fieldNm=" + fieldNm.toString();
	}
...
{
	dmeFilter.setFrame(eval("window." + frameNm));
	dmeFilterFieldHash.put(dmeFilter);
...
function addSingleSelectOption(selObj, selDoc, selValue, selText)
{
	selObj.innerHTML = "";
	var tmpObj = self.left.document.createElement("OPTION");
	tmpObj.value = " ";
	tmpObj.text = " ";
	if (navigator.appName.indexOf("Microsoft") >= 0) {
		selObj.add(tmpObj);
	...
	tmpObj = self.left.document.createElement("OPTION");
	tmpObj.value = selValue;
	tmpObj.text = selText;
	if (navigator.appName.indexOf("Microsoft") >= 0) {
		selObj.add(tmpObj);
	} else {
...
DMEFilterHandler.prototype.setDataMiner = function(frameNm, fileNm, indexNm, fields, keys, condExpr, selectExpr, nbrRecs, otmNbrRecs)
{
	...
	this.setFrame(eval("this.launchedFrom." + frameNm));
...
DMEFilterHandler.prototype.enable = function()
{
	...
	setFilterOptions(this.filterForm.elements["fieldNm"].value, 
		selectedField, 
		this.filterForm.elements["filterOption"]);
