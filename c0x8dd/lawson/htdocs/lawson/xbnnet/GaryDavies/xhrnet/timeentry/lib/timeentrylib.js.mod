...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xhrnet/timeentry/lib/Attic/timeentrylib.js,v 1.1.2.26.4.2 2012/04/18 16:02:00 brentd Exp $
...
function PaintWindowForSelectBox(Information)
{
	...
	html[0] += '<table border="0" cellpadding="0" cellspacing="0" styler="list">';
...
function openActivitySrch()
{
	...
	var SSLLoc = (document.getElementById && !document.all) ? "javascript:''" : "/lawson/xhrnet/dot.htm";
	if (navigator.userAgent.indexOf("Mac") != -1) {	
		ACTSDWIND=window.open("","ACTSDWIND","resizable=yes,width=300,height=400,scrollbars=1")
	}
	else {
		ACTSDWIND=window.open(SSLLoc,"ACTSDWIND","resizable=yes,width=300,height=400,scrollbars=1")
	}
	*/
...
function DspPicks()
{
	...
	if(self.jsreturn.record.length)
	{
		...
		for(var i=0;i<TEActivityCodes.length;i++)
		{
			html[html.length] = '<TR><TD><a href="" onClick="opener.deletePick('+i+');return false;">'
			+ '<img src='+ActivityDeleteIcon+' alt="'+getSeaPhrase("DEL_ACTIVITY","TE")+'" border=0></a>'
...
function drawDmeFieldFilterContent(dmeFilter)
{
	...
	switch(fieldNm)
	{
		...
		case "emppicklst": // employee cached activity
			...
			selectHtml[0] += '<th style="width:6%;text-align:right;padding-right:5px">&nbsp;</th></tr>'
			...
			for (var i=0; i<nbrDmeRecs; i++) 
			{
				...
				selectHtml[i+1] += '</td><td style="padding-left:5px"><a href="javascript:void(0);" onclick="'+fromWndStr+'.deleteCachedActivity('+i+',\''+fieldNm+'\');return false;">'
				selectHtml[i+1] += '<img src="/lawson/xhrnet/ui/images/delete.gif" alt="'+getSeaPhrase("DEL_ACTIVITY","TE")+'" border=0></a>'				
				selectHtml[i+1] += '</td></tr>'
			}
