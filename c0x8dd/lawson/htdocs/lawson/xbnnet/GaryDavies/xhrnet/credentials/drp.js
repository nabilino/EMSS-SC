...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xhrnet/credentials/drp.js,v 1.11.2.6 2009/03/23 22:35:38 brentd Exp $
var DRPV;
...
function PaintDRPVWindow(onsort, sortField)
{
	...
	for (var i=0; i<DRPV.DirectReports.length; i++)
	{
		...
		+ '<td class="plaintablecellborder" style="text-align:left;width:27%" nowrap>'
...
function LinkToProfessionalProfile(Index)
{
   	...
   	if (self.subtask.location.href.indexOf("pp.htm")==-1) {
   		self.subtask.location.replace("/lawson/xhrnet/credentials/pp.htm?employee="+DRPV.DirectReports[Index].Employee+"&type="+DRPV.DirectReports[Index].WebType);
	}
	else {
		self.subtask.employeeNbr = DRPV.DirectReports[Index].Employee;
