...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xhrnet/careermanagement/lib/Attic/cmlib.js,v 1.1.2.10 2011/07/08 17:15:25 brentd Exp $
	...
	relatedBookmarkLinks = new Array();
   	...
   	if ((typeof(authUser) != "undefined") && authUser.OfficeObject) {
		...
		while ((i < authUser.NbrOfOfficeObj) && (!done))
		{
			...
			if ((!foundBkmk[bkName])
			&& (((bkName == "GOALVIEW") && (bkUrl.indexOf("employee") >= 0)) || (bkLawName == "XMLHREMPGOALMGMT"))) {
...
function GetRelatedBookmarkLinksContent() {
	...
	strBuffer[0] = '<table border="0" cellspacing="0" cellpadding="0" width="100%">'

	if (relatedBookmarkLinks.length > 0) {
		for (var i=0; i<relatedBookmarkLinks.length; i++) {

			strBuffer[i+1] = '<tr><td class="contenttextCM">'
			+ '<a href="javascript:void(0);" onclick="parent.launchRelatedBookmarkLink('+i+');">'
			+ relatedBookmarkLinks[i].name
			+ '</a>'
			+ '</td></tr>';
		}
	} else {
		strBuffer[1] = '<tr><td class="contenttextCM">'
		+ getSeaPhrase("CM_NO_RELATED_LINK_ACCESS", "CM")
		+ '</td></tr>';
	}
...
{
	if(nCmd)	showLayer("main", ButtonName+"Button")
	else		hideLayer("main", ButtonName+"Button")
}
...
function MakeButton(Label, Left, Top, Width, Height, BorderColor, ButtonName, Visible, Flyby, isImage, AttrStr)
{
	...
	if (isImage)
	{
		var Desc = '<div onclick="javascript:parent.'+ButtonName+'_OnClick()" onMouseOver="window.status=\''+Flyby+'\';return true;" onMouseOut="window.status=\' \';return true;"'
		Desc += (AttrStr) ? ' '+AttrStr:''
		Desc += '>'+Label+'</div>'
	}
	else
	{
	    var Desc = '<button class="buttonCM" onclick="javascript:parent.'+ButtonName+'_OnClick()" onMouseOver="window.status=\''+Flyby+'\';return true;" onMouseOut="window.status=\' \';return true;"'
	    Desc += (AttrStr) ? ' '+AttrStr:''
	    Desc += '>'+Label+'</button>'
	}
...
function MakeButtonCM(Label, Left, Top, Width, Height, BorderColor, ButtonName, Visible, Flyby, isImage)
{
	...
	if (isImage)
		var Desc = '<div onclick="javascript:parent.'+ButtonName+'_OnClick()" onMouseOver="window.status=\''+Flyby+'\';return true;" onMouseOut="window.status=\' \';return true;">'+Label+'</div>'
	else
	    	var Desc = '<button class="button2CM" onclick="javascript:parent.'+ButtonName+'_OnClick()" onMouseOver="window.status=\''+Flyby+'\';return true;" onMouseOut="window.status=\' \';return true;">'+Label+'</button>'

	buttonLayer("main",ButtonName+"Button",Left,Top,Width,Height,Visible,Desc,"","");
}

function MakeSelectBox(Label, Left, Top, Width, Height, SelectName, SelectValues, Visible, SelectedIndex/*,OnChangeFlag*/)
{
	...
	var Desc = '<form name="'+Label+'" onSubmit="return false">'
		Desc += '<select class="contenttextCM" name="'+SelectName+'" onchange="'+onChangeFunc+'">'
		Desc += '<option value=" " '
...
function MakeTextBox(Label, Left, Top, Width, Height, TextName, Visible, Size, Value, AttrStr)
{
	var Desc = '<form name="'+Label+'" onSubmit="return false">'
	Desc += '<input class="contenttextCM" style="width:'+(Width-15)+'" type=text name="'+TextName+'" size="'+parseInt(Size,10)+'"'
...
// function CareerManagementWindowObject(WindowBackground, FrameLocation, WindowName, Left, Top, Width, Height, Visible, Title)
...
function DrawWindow()
{
	...
	{
		// ui change
		// var Desc = '<img src='+this.WindowBackground+'>'
		var Desc = '<div class="panewrapperCM">'
			+'<table class="panewrapperCM" cellpadding="0" cellspacing="0" width="100%">'
		...
		Desc += '<table class="paneouterborderCM" cellpadding="0" cellspacing="0" width="100%">'
			+'<tr><td class="paneframeCM">'
			+'<div class="paneinnerborderCM" '+((this.GroupBox)?'styler="groupbox"':'')+'></div>'
			+'</td></tr></table>'
			+'</div>'
			+'</td></tr></table>'
			+'</div>'
		...
		if(this.AddCloseIcon)
		{
			Desc = '<a href="javascript:void(0);" onClick="parent.'+this.WindowName+'_XIconClicked();return false;">'
			Desc += '<img name=xIcon src="'+XIconOut+'" border=0 alt="'+getSeaPhrase("CM_CLOSE","CM")+'" '
			Desc += 'onMouseOver="this.src=\''+XIconOver+'\';return true;" '
			Desc += 'onMouseOut="this.src=\''+XIconOut+'\';return true;">'
			Desc += '</a>'
...
function DrawTabbedWindow(TabDesc)
{
	...
	{
		var Desc = '<div class="panewrapperCM">'
			+'<table class="panewrapperCM" cellpadding="0" cellspacing="0"><tr><td class="panepaddingCM">'
			+'<div class="panebackgroundCM">'
			+'<table class="paneouterborderCM" cellpadding="0" cellspacing="0"><tr><td class="paneframeCM">'
			+'<div class="paneinnerborderCM"></div>'
			+'</td></tr></table>'
			+'</div>'
			+'</td></tr></table>'
			+'</div>'
...
{
	var Desc = '<img src="'+this.WindowBackground+'" alt="">'
...
function GetTextBoxFormObject(framename, formName, TextBoxName)
{
	...
	if (navigator.appName.indexOf("Microsoft")>=0 &&
		getDocLoc(framename).getElementsByName(TextBoxName).length==2)
...
function FillSelectBox(Label, SelectName, SelectValues)
{
	var Desc = '<form name="'+Label+'"><select class="contenttextCM" name="'+SelectName+'">'
...
{
	var tmpStr = escape(unescape(unescape(str.toString())));	
	return tmpStr.toString().replace(/%0A/g, "<br/>").replace(/%0D/g, "<br/>");	
}

function PrintAttachment()
{
	var Desc = '<table width=500 cellpadding=0 border=0 cellspacing=0><tr>'
		+ '<td class="contenttextCM">'
		+ '<span class="contentlabelCM">'
		+ getSeaPhrase("CM_108","CM")+' &nbsp;</span>'

	Desc += (StandardCareerProfile.NextReviewDate == null || NonSpace(StandardCareerProfile.NextReviewDate) == 0) ? getSeaPhrase("CM_109","CM") : FormatDte6(StandardCareerProfile.NextReviewDate);
	
	if(Attachment != null && Attachment.CmtRec.length)
	{
		Desc += '<td class="contenttextCM" rowspan=2>'
			+ '<span class="contentlabelCM">'+getSeaPhrase("CM_110","CM")+' </span><br>'
			+ LastUpdated + '<br>'
			+ TimeUpdate + '</td></tr>'
			+ '<tr><td class="contenttextCM">'
			+ '<span class="contentlabelCM">'+getSeaPhrase("CM_111","CM")+' </span>&nbsp;'
	...
	self.PGCONTROL.document.title = getSeaPhrase("CM_329","CM");
...
function ReportsExist()
{
	...
	{
		seaAlert(getSeaPhrase("CM_170","CM"))
		if (opener)
...
// Renewal Codes
var renewalCodes = new Array("  ","2Y","3Y", "4Y", "5Y", "6Y", "7Y", "8Y", "9Y",
							"AN", "QT", "SA")

var renewalNames = new Array(" ", getSeaPhrase("CM_267","CM"), getSeaPhrase("CM_268","CM"),
								  getSeaPhrase("CM_269","CM"), getSeaPhrase("CM_270","CM"),
								  getSeaPhrase("CM_271","CM"), getSeaPhrase("CM_272","CM"),
								  getSeaPhrase("CM_273","CM"), getSeaPhrase("CM_274","CM"),
								  getSeaPhrase("CM_275","CM"), getSeaPhrase("CM_276","CM"),
								  getSeaPhrase("CM_277","CM") )
...
function PaintJobCode_InProgress(frameNm, isSelect, redirect)
{
	...
	if (!isSelect)
	{
		...
		for (var i=0;i<frameRecs.length;i++)
		{
			if (NonSpace(frameRecs[i].job_class) > 0) {
				...
				|| (frameRecs[i].job_class != JobClass[JobClass.length-1].job_class)) {
	...
	else 
	{
		for (var i=0;i<frameRecs.length;i++)
		{
			...
			if (NonSpace(frameRecs[i].job_class) > 0) {			
				if ((JobClass.length == 0) 
				|| (frameRecs[i].job_class != JobClass[JobClass.length-1].code)) {
					JobClass[JobClass.length] = new SelectObject();	
...
function PCODES_Finished(task)
{
	...
	for(var i=0; i<self.jsreturn.NbrRecs; i++)
	{
		...
		else webAvail = eval('pObj.web_available')	
...
function DME_TO_PASESSION(course, func)
{
	...
	pObj.out = "JAVASCRIPT"
	pObj.field = "session;start-date;end-date;end-reg-date;site;facility.description;start-time;end-time;session-status;"
	pObj.max = "600";
...
function getDmeFieldElement(fieldNm)
{
	...
	{	
		...
		case "degree_list":
			...
			fld = [self.main, QualBox];
			break;						
		case "subject":
			pFormObj = (_CURRENTTAB == "ACTIONPLAN") ?
				GetFormFromStansWindowLayer("main", "QualificationsDetail_MyActionPlanWindow", "qualificationform"):
				GetFormFromStansWindowLayer("main", "CompetencyDetail_MyQualificationsWindow", "qualificationform");				
			fld = [self.main, pFormObj.subject];
			break;
		case "instructor":
			pFormObj = (_CURRENTTAB == "ACTIONPLAN") ?
				GetFormFromStansWindowLayer("main", "QualificationsDetail_MyActionPlanWindow", "qualificationform"):
				GetFormFromStansWindowLayer("main", "CompetencyDetail_MyQualificationsWindow", "qualificationform");				
			fld = [self.main, pFormObj.instructor];
			break;			
		case "skillsource":
			pFormObj = (_CURRENTTAB == "ACTIONPLAN") ?
				GetFormFromStansWindowLayer("main", "QualificationsDetail_MyActionPlanWindow", "qualificationform"):
				GetFormFromStansWindowLayer("main", "CompetencyDetail_MyQualificationsWindow", "qualificationform");				
			fld = [self.main, pFormObj.skillsource];
			break;
/*			
		case "state":
...
*/			
		case "currencycode":
			pFormObj = (_CURRENTTAB == "ACTIONPLAN") ?
				GetFormFromStansWindowLayer("main", "QualificationsDetail_MyActionPlanWindow", "qualificationform"):
				GetFormFromStansWindowLayer("main", "CompetencyDetail_MyQualificationsWindow", "qualificationform");				
			fld = [self.main, pFormObj.currencycode];
			break;
		case "course_category":
			pFormObj = GetTextBoxFormObject("main", "CourseCategoryFilterSelect", "CourseCategoryFilterSelect_TrainingOptions");
			fld = [self.main, pFormObj];
			break;	
		case "job_title":
			pFormObj = GetTextBoxFormObject("main", "JobTitleFilterSelect", "JobTitleFilterSelect_ExploreCareers");
			fld = [self.main, pFormObj];
...
function drawDmeFieldFilterContent(dmeFilter)
{
	...
	{	
		...
		case "certification":
			var tmpObj;
			selectHtml[0] = '<table class="filterTable" border="0" cellspacing="0" cellpadding="0" style="width:100%" styler="list">'
		...
		case "certification_list":
			var tmpObj;
			selectHtml[0] = '<table class="filterListTable" border="0" cellspacing="0" cellpadding="0" style="width:100%" styler="list">'
			selectHtml[0] += '<tr><th style="width:50%">'+getSeaPhrase("QUAL_4","ESS")+'</th>'
			selectHtml[0] += '<th style="width:50%">'+getSeaPhrase("JOB_OPENINGS_2","ESS")+'</th></tr>'
			...
			{
				tmpObj = dmeRecs[i];
				selectHtml[i+1] = '<tr onclick="dmeFieldRecordSelected('+i+',\''+fieldNm+'\');return false" class="filterListTableRow" onmouseover="filterListTableRowMouseOver(this);" onmouseout="filterListTableRowMouseOut(this);">'
				selectHtml[i+1] += '<td style="padding-left:5px" nowrap>'
				selectHtml[i+1] += (tmpObj.code) ? tmpObj.code : '&nbsp;'
				selectHtml[i+1] += '</td><td style="padding-left:5px" nowrap>'
				selectHtml[i+1] += (tmpObj.description) ? tmpObj.description : '&nbsp;'
...
*/		
		...
		case "course_category":
			var tmpObj;
			selectHtml[0] = '<table class="filterTable" border="0" cellspacing="0" cellpadding="0" style="width:100%" styler="list">'
			selectHtml[0] += '<tr><th style="width:50%">'+getSeaPhrase("CM_171","CM")+'</th>'
			selectHtml[0] += '<th style="width:50%">'+getSeaPhrase("CM_134","CM")+'</th></tr>'
		...
		case "job_category":
			var tmpObj;
			selectHtml[0] = '<table class="filterTable" border="0" cellspacing="0" cellpadding="0" style="width:100%" styler="list">'
			selectHtml[0] += '<tr><th style="width:50%">'+getSeaPhrase("CM_28","CM")+'</th>'
			selectHtml[0] += '<th style="width:50%">'+getSeaPhrase("CM_134","CM")+'</th></tr>'
		...
		case "job_openings":
			...
			selectHtml[0] += '<table class="filterListTable" border="0" cellspacing="0" cellpadding="0" style="width:100%" styler="list">'
			selectHtml[0] += '<tr><th style="width:50px">'+getSeaPhrase("CM_133","CM")+'</th>'
			selectHtml[0] += '<th style="width:30%">'+getSeaPhrase("CM_134","CM")+'</th>'
			selectHtml[0] += '<th style="width:30%">'+getSeaPhrase("CM_135","CM")+'</th>'
			selectHtml[0] += '<th style="width:30%">'+getSeaPhrase("CM_136","CM")+'</th></tr>'

			for (var i=0; i<nbrDmeRecs; i++) 
			{
				tmpObj = dmeRecs[i];		
				selectHtml[i+1] = '<tr class="filterListTableRow" onmouseover="filterListTableRowMouseOver(this);" onmouseout="filterListTableRowMouseOut(this);">'
				selectHtml[i+1] += '<td nowrap>'
				selectHtml[i+1] += '<input type="checkbox" value="on" name="openings'+i+'">'
				selectHtml[i+1] += '</td><td style="padding-left:5px" onclick="dmeFieldRecordSelected('+i+',\''+fieldNm+'\');return false" nowrap>'
				selectHtml[i+1] += (tmpObj.description) ? tmpObj.description : '&nbsp;'
				selectHtml[i+1] += '</td><td style="padding-left:5px" onclick="dmeFieldRecordSelected('+i+',\''+fieldNm+'\');return false" nowrap>'
		...
		case "reviews":
			var tmpObj;
			selectHtml[0] = '<table class="filterListTable" border="0" cellspacing="0" cellpadding="0" style="width:100%" styler="list">'
			selectHtml[0] += '<tr><th style="width:50%">'+getSeaPhrase("CM_330","CM")+'</th>'
			selectHtml[0] += '<th style="width:50%">'+getSeaPhrase("CM_50","CM")+'</th></tr>'
			...
			for (var i=0; i<nbrDmeRecs; i++) 
			{
				...
				selectHtml[i+1] = '<tr onclick="dmeFieldRecordSelected('+i+',\''+fieldNm+'\');return false" class="filterListTableRow" onmouseover="filterListTableRowMouseOver(this);" onmouseout="filterListTableRowMouseOut(this);">'
				selectHtml[i+1] += '<td style="padding-left:5px" nowrap>'
				selectHtml[i+1] += (tmpObj.sched_date) ? formatDME(tmpObj.sched_date) : '&nbsp;'
				selectHtml[i+1] += '</td><td style="padding-left:5px" nowrap>'
			...
			if (nbrDmeRecs == 0) 
			{
				selectHtml[1] = '<tr class="filterListTableRow2">'
				selectHtml[1] += '<td style="padding-left:5px" colspan="2" nowrap>'
				selectHtml[1] += getSeaPhrase("CM_331","CM");
				selectHtml[1] += '</td></tr>'
			}
		...
		case "training_history":
			var tmpObj;
			selectHtml[0] = '<table class="filterListTable" border="0" cellspacing="0" cellpadding="0" style="width:100%" styler="list">'
			selectHtml[0] += '<tr><th style="width:50%">'+getSeaPhrase("CM_335","CM")+'</th>'
			selectHtml[0] += '<th style="width:50%">'+getSeaPhrase("CM_336","CM")+'</th></tr>'
			...
			for (var i=0; i<nbrDmeRecs; i++) 
			{
				...
				selectHtml[i+1] += '</td><td style="padding-left:5px" nowrap>'
				selectHtml[i+1] += (tmpObj.date_completed) ? formatDME(tmpObj.date_completed) : '&nbsp;'
				selectHtml[i+1] += '</td></tr>'
			}
		...
		case "training_registration":
			...
			selectHtml[0] = '<table class="filterListTable" border="0" cellspacing="0" cellpadding="0" style="width:100%" styler="list">'
			selectHtml[0] += '<tr><th style="width:50%">'+getSeaPhrase("CM_335","CM")+'</th>'
			selectHtml[0] += '<th style="width:50%">'+getSeaPhrase("CM_289","CM")+'</th></tr>'
			...
			for (var i=0; i<nbrDmeRecs; i++) 
			{
				...
				selectHtml[i+1] += (tmpObj.course_short_crs_desc) ? tmpObj.course_short_crs_desc : '&nbsp;'
				selectHtml[i+1] += '</td><td style="padding-left:5px" nowrap>'
				selectHtml[i+1] += (tmpObj.start_date) ? formatDME(tmpObj.start_date) : '&nbsp;'
				selectHtml[i+1] += '</td></tr>'
			}
...
function createLayer(framename, name, left, top, width, height, visible, content, bgtype, background, MSScroll, IFrame, zIndex)
{
	...
	layerList[z] = name;
  	...
  	if (docLoc.all || docLoc.getElementById) {
		...
		divObj.style.clip = "rect(0px " +  width + "px " + height + "px " + "0px)"
		if (background != '') {
			if (bgtype == "color")
...
function buttonLayer(framename, name, left, top, width, height, visible, content, bgtype, background, MSScroll, IFrame, zIndex)
{
	...
	layerList[z] = name;
  	...
  	if (docLoc.all || docLoc.getElementById) {
		var LayHTML = '<div id="' + name + '" align=left style="position:relative;overflow:'
		if (MSScroll)
			LayHTML	+= 'auto'
		else
			LayHTML	+= 'hidden'
		LayHTML	+= '; left:' + left + 'px; top:' + top + 'px; width:' + width + 'px; height:' + height + 'px;' + ' visibility:' + (visible ? 'visible;' : 'hidden;') + ' z-index:' + z
		LayHTML	+= "clip:rect(0px " +  width + "px " + height + "px " + "0px)"
		if (background == '')
			LayHTML	+= '">'
		else {
			if (bgtype == "color")
		...
		LayHTML	+= content;
		LayHTML	+= "<div id=bottom>&nbsp</div>"
	    LayHTML	+= '</div>';
  	}
	...
	if (Index !=null)
  	{
		...
		if (level == 25)
			var levelImg = "/lawson/xhrnet/careermanagement/lib/images/dim25.gif"
		else
		if (level == 50)
			var levelImg = "/lawson/xhrnet/careermanagement/lib/images/dim50.gif"
		else
		if (level == 75)
			var levelImg = "/lawson/xhrnet/careermanagement/lib/images/dim75.gif"
		else
			var levelImg = "/lawson/xhrnet/careermanagement/lib/images/dim100.gif"
...
function flyIn(framename, name, speed, flyleft, flytop, flyclipwidth, flyclipheight)
{
	...
	||(typeof(flytop) == "undefined")
	||(typeof(flylwidth) == "undefined")
	||(typeof(flyheight) == "undefined"))
	&& ((LayerObject[Index].flyleft == null)
	&& (LayerObject[Index].flytop == null)
	&& (LayerObject[Index].flyclipwidth == null)
	&& (LayerObject[Index].flyclipheight == null)))
		return
...
function scrollLayer(framename, name, dx, dy) {
  ...
  else if ((cb + dy > getDocumentBottom(framename, name))
	   && (docLoc.layers))
...
function replaceContent(framename, name, content)
{
	...
	{
		var str = "docLoc.getElementById('" + name + "').innerHTML = '" + repStr(content,"'","&#39;") + "';";
		eval(str);
	}
}
...
function getObjectIndex(framename, name)
{
	...
	{
		if ((LayerObject[x].name == name)
		&& (LayerObject[x].framename == framename))
			return x
	}
	return null
}
...
function GetObjType(framename, name)
{
	...
	&& (WindowIndex(framename, name) != 999))
		return "Window"
	else
	if ((typeof(ObjCollectionTemp) != "undefined")
	&&(typeof(name.NbrOfButtons) != 'undefinded'))
		return "ButtonBank"
	else
		return null
}
...
{
	if (width>100)
		var Desc = "<table width=" + (width+(framewidth*2)) + "><tr><td align=left>" + title + "<td align=right>"
	else
		var Desc = "<table width=" + (width+(framewidth*2)) + "><tr><td align=right>"

	if (ResizeFlag)
	{
		if(Minimized)
			Desc += " <a href=javascript:parent.MaximizeWindow(\"" + framename + "\",\"" + name + "\")><img src='/lawson/xhrnet/careermanagement/lib/images/max.gif' border=0 alt='"+getSeaPhrase('MAXIMIZE','ESS')+"'></a>"
		else
			Desc += " <a href=javascript:parent.MinimizeWindow(\"" + framename + "\",\"" + name + "\")><img src='/lawson/xhrnet/careermanagement/lib/images/min.gif' border=0 alt='"+getSeaPhrase('MINIMIZE','ESS')+"'></a>"
	}
	if (closeFlag)
	{
		Desc += " <a href=javascript:parent.CloseLayerWindow(\"" + framename + "\",\"" + name + "\")><img src='/lawson/xhrnet/careermanagement/lib/images/close.gif' border=0 alt='"+getSeaPhrase('CLOSE','ESS')+"'></a>"
	}
	else
		Desc += "&nbsp"
	Desc += "</td></tr></table> "
...
function DrawTabs(Color) // this one has 1px change
{
	...
	var Tabs = '<div id="CMTabs_' + this.TabName + '" styler="tabcontrol" styler_init="StylerEMSS.initTabControlCM" styler_load="StylerEMSS.onLoadTabControlTabCM">'
		...
		+ '<td height=1 class="tabbordercolorCM" colspan='+(this.TabsArray.length*2)+'>'
		+ '</td>'
		+ '<td height=1 class="tabbackgroundcolorCM">'
		+ '</td>'
		+ '</tr><tr>'

		for(var i=0; i<this.TabsArray.length; i++)
		{
			Tabs += '<td ' + ((i == 0) ? 'class="tableftCM"' : '') + ' nowrap><a href="" '
			...
			Tabs += '</a></td><td class="tabbordercolorCM"><font style="font-size:1pt;">&nbsp;</font></td>'
		}

		Tabs += '<td width=700 class="tabbackgroundcolorCM">&nbsp;</td></tr><tr >'

		if(this.TabSelected == 0)
			Tabs += '<td class="tabupCM">'
		else
			Tabs += '<td class="tabbordercolorCM">'

		for(var i=1; i <this.TabsArray.length; i++)
		{
			Tabs += '</td><td class="tabbordercolorCM"></td>'
		...
		Tabs += '</td><td class="tabbordercolorCM"></td><td class="tabbordercolorCM"></td>'
		Tabs += '</tr></table>'
		Tabs += '</div>'
...
function MenuTab(TabContent, Index, Up)
{
	...
	var MenuTabs = 'id="' + this.TabName + ',' + this.TabsArray[Index].Code + ',TabBody" ';
	if(Up)
	...
	{
		MenuTabs += 'onMouseOver="parent.DisplayMouseOverLayer(\''+this.TabsArray[Index].Code+'\'); return true;" '
		MenuTabs += 'onMouseOut="parent.HideMouseOverLayer(\''+this.TabsArray[Index].Code+'\'); return true;"'
	}

	MenuTabs += '>&nbsp;&nbsp;'+this.TabsArray[Index].Description+'&nbsp;&nbsp;'

	if(TabContent != "" && typeof(TabContent)!="undefined" && TabContent != "undefined" && TabContent != null)
	{
		createLayer("main",this.TabsArray[Index].Code+"Shadow",Left+4,25,200,75,false,"&nbsp;","color","#000000", null, null, 0);

		var docLoc = getDocLoc("main");

		var Desc = '<table border=0 cellpadding=4 cellspacing=0 width=100% height=100%><tr>'

		Desc += '<td class="popuphelptextCM">'
		Desc += TabContent
		Desc += '</td></tr></table>'

		createLayer("main",this.TabsArray[Index].Code,Left,21,200,75,false,Desc,"color",this.bgColorTabDown,null, null, 0);
	}

	return MenuTabs;
}

function DisplayMouseOverLayer(TabCode)
{
	mouseoverTimer = setTimeout('DisplayLayer("'+TabCode+'")',1000)
}

function DisplayLayer(TabCode)
{
	showLayer("main", TabCode+"Shadow", 100000000)
	showLayer("main", TabCode, 100000000);
}

function HideMouseOverLayer(TabCode)
{
	clearTimeout(mouseoverTimer);
	hideLayer("main", TabCode+"Shadow")
	hideLayer("main", TabCode);
}
...
function OnTabClicked_Qualifications(TabName/*, FC*/)
{
	...
	case "Competencies":
		...
		else _CURRENTQUALWINDESC = getSeaPhrase("CM_227","CM")
	...
	{
		CompetencyDetail_MyQualifications_XIconClicked();
		seaAlert(getSeaPhrase("CM_228","CM"));
	}
	else
	{
		seaAlert(getSeaPhrase("CM_229","CM"));
	}
...
function GetWebUpdatableRecords(QualificationList)
{
	...
	{
		if (typeof(QualificationList[i].webavail) != "undefined"
		&& parseInt(QualificationList[i].webavail,10) > 2)
...
function AddQualificationButton_MyQualifications_OnClick()
{
	...
	if(EmployeeQualifications && (_DIRECTREPORTSINDEX < 0 || !DirectReports[_DIRECTREPORTSINDEX] ||
		!NonSpace(DirectReports[_DIRECTREPORTSINDEX].description)))
	{
		seaAlert(getSeaPhrase("CM_230","CM"));
		return;
	}
	...
	var buttonStr = '<tr><td>&nbsp;</td><td width=10>&nbsp;</td><td>'
		+ uiButton(getSeaPhrase("CM_95","CM"),"parent.UpdateQualificationButton_MyQualifications_OnClick();return false;","margin-top:10px")
		+ uiButton(getSeaPhrase("CM_94","CM"),"parent.CompetencyDetail_MyQualifications_XIconClicked();return false;","margin-top:15px")		
		+ '</td></tr>'
	...
	switch(_TABSELECTED)
	{
		case 0:
			...
			Desc += '<table border="0" cellpadding="0" cellspacing="0" width="100%">'
			// PT 124441
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_50","CM")+'<td width=10>&nbsp;<td><div id="competencyType" class="contenttextCM"></div>'
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_46","CM")+'<td width=10>&nbsp;'
			
			if (emssObjInstance.emssObj.filterSelect)
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="code" value="" size="10" maxlength="10" onfocus="this.select()" onkeyup="this.value=\'\';this.form.qualification.value=\'\';this.form.type.value=\'\';document.getElementById(\'competencyType\').value=\'\';document.getElementById(\'competencyDesc\').innerHTML=\'\';parent.FilterProficiencies(this.form,true);">'
				...
				+ '<span id="competencyDesc" style="text-align:left;width:150px" class="fieldlabel"></span>'
				+ '</td></tr>'
			}
			else
			{				
				Desc += '<td><select class="contenttextCM" name=code onchange="parent.FillQualification(this.form,\'Competency\');parent.FilterProficiencies(this.form);">'+BuildSelect("","FilteredQualification")+'</select></td></tr>'
			}	
			
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_232","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=proficiency>'+BuildSelect("","Proficiency")+'</select>'
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_233","CM")+'<td width=10>&nbsp;'
			
			if (emssObjInstance.emssObj.filterSelect)
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="skillsource" value="" size="10" maxlength="10" onfocus="this.select()" onkeyup="this.value=\'\';document.getElementById(\'skillsourceDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'skillsource\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="skillsourceDesc" style="text-align:left;width:150px" class="fieldlabel"></span>'
				+ '</td></tr>'
			}
			else
			{			
				Desc += '<td><select class="contenttextCM" name=skillsource>'+BuildSelect("","Source")+'</select></td></tr>'
			}
			
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_234","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=renewalcode>'+BuildRenewalSelect("")+'</select>'
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_235","CM")+'<td width=10>&nbsp;<td><input class="contenttextCM" type=text size=10 maxlength=10 name=instructor value="">'
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_236","CM")+'<td width=10>&nbsp;<td><input class="contenttextCM" type=text size=6 maxlength=6 name=perrating value="">'
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_237","CM")+'<td width=10>&nbsp;<td>'
				 +'<input class="contenttextCM" type=text name=dateacquired value="" size=10 maxlength=10 onFocus=this.select()>'
				 +'<a class="contenttextCM" href="" onClick="parent.DateSelect(\'dateacquired\');return false;" onMouseOver="window.status=\''+getSeaPhrase("CM_238","CM")+'\';return true;"'
				 +' onMouseOut="window.status=\' \';return true;">'+uiCalendarIcon()+'</a><span class="contenttextCM">' + uiDateFormatSpan()+ '</span>';
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_239","CM")+'<td width=10>&nbsp;<td>'
				 +'<input class="contenttextCM" type=text name=renewdate value="" size=10 maxlength=10 onFocus=this.select()>'
				 +'<a class="contenttextCM" href="" onClick="parent.DateSelect(\'renewdate\');return false;" onMouseOver="window.status=\''+getSeaPhrase("CM_238","CM")+'\';return true;"'
				 +' onMouseOut="window.status=\' \';return true;">'+uiCalendarIcon()+'</a><span class="contenttextCM">' + uiDateFormatSpan()+ '</span>';
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_240","CM")+'<td width=10>&nbsp;<td>'
				 +'<input class="contenttextCM" type=text name=datereturned value="" size=10 maxlength=10 onFocus=this.select()>'
				 +'<a class="contenttextCM" href="" onClick="parent.DateSelect(\'datereturned\');return false;" onMouseOver="window.status=\''+getSeaPhrase("CM_238","CM")+'\';return true;"'
				 +' onMouseOut="window.status=\' \';return true;">'+uiCalendarIcon()+'</a><span class="contenttextCM">' + uiDateFormatSpan()+ '</span>';
			Desc += '<tr><td class="fieldlabelboldliteunderline">'+getSeaPhrase("CM_241","CM")+'<td width=10>&nbsp;<td>'
				 +'<select class="contenttextCM" name=cosponsored>'+BuildYesNo("")+'</select>'
				 
			Desc += buttonStr
			
			Desc += '<tr><td>&nbsp;<td width=10>&nbsp;<td><input type=hidden size=20 maxlength=30 name=qualification value="" onFocus="this.form.code.focus();">'
			Desc += '<tr><td>&nbsp;<td width=10>&nbsp;<td><input type=hidden name=type value="" onFocus="this.form.code.focus();">'				 
			Desc += '</table>'
			Desc += '</form>'
			break;
		case 1:
			...
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="code" value="" size="10" maxlength="10" onfocus="this.select()" onkeyup="this.value=\'\';this.form.qualification.value=\'\';document.getElementById(\'certificationDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'certification\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="certificationDesc" style="text-align:left;width:150px" class="fieldlabel"></span>'
				+ '</td></tr>'
			}
			else
			{			
				Desc += '<td><select class="contenttextCM" name=code onchange="parent.FillQualification(this.form,\'Certifications\');">'+BuildSelect("","FilteredQualification")+'</select>'
			}
			
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_243","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=state>'+BuildStateSelect("")+'</select>'
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_244","CM")+'<td width=10>&nbsp;<td><input class="contenttextCM" type=text name=licnumber size=15 maxlength=20 value="">'
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_237","CM")+'<td width=10>&nbsp;<td>'
				 +'<input class="contenttextCM" type=text name=dateacquired value="" size=10 maxlength=10 onFocus=this.select()>'
				 +'<a class="contenttextCM" href="" onClick="parent.DateSelect(\'dateacquired\');return false;" onMouseOver="window.status=\''+getSeaPhrase("CM_238","CM")+'\';return true;"'
				 +' onMouseOut="window.status=\' \';return true;">'+uiCalendarIcon()+'</a><span class="contenttextCM">' + uiDateFormatSpan()+ '</span>';
			...
			{						
				Desc += '<td><select class="contenttextCM" name=skillsource>'+BuildSelect("", "Source")+'</select>'
			}
			
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_241","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=cosponsored>'+BuildYesNo("")+'</select>'
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_245","CM")+'<td width=10>&nbsp;<td><input class="contenttextCM" type=text size=14 maxlength=14 name=cost value="">'
			Desc += '<tr><td class="fieldlabelboldliteunderline">'+getSeaPhrase("CM_246","CM")+'<td width=10>&nbsp;'
			
			if (emssObjInstance.emssObj.filterSelect)
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="currencycode" value="" size="5" maxlength="5" onfocus="this.select()" onkeyup="this.value=\'\';document.getElementById(\'currencycodeDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'currencycode\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="currencycodeDesc" style="text-align:left;width:150px" class="fieldlabel"></span>'
				+ '</td></tr>'
			}
			else
			{			
				Desc += '<td><select class="contenttextCM" name=currencycode>'+BuildSelect("", "Currency")+'</select>'
			}
			
			Desc += buttonStr
			
			Desc += '<tr><td>&nbsp;<td width=10>&nbsp;<td><input type=hidden size=20 maxlength=30 name=qualification value="">'				 
			Desc += '</table>'
			Desc += '</form>'
			break;
		case 2:
			...
			Desc = '<form name="qualificationform">'
			Desc += '<table border="0" cellpadding="0" cellspacing="0" width="100%">'
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_248","CM")+'<td width=10>&nbsp;'
			
			if (emssObjInstance.emssObj.filterSelect)
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="degree" value="" size="10" maxlength="10" onfocus="this.select()" onkeyup="this.value=\'\';document.getElementById(\'degreeDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'degree\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="degreeDesc" style="text-align:left;width:150px" class="fieldlabel"></span>'
				+ '</td></tr>'
			}
			else
			{			
				Desc += '<td><select class="contenttextCM" name=degree>'+BuildSelect("", "FilteredQualification")+'</select>'
			}
		
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_249","CM")+'<td width=10>&nbsp;'
			
			if (emssObjInstance.emssObj.filterSelect)
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="subject" value="" size="10" maxlength="10" onfocus="this.select()" onkeyup="this.value=\'\';document.getElementById(\'subjectDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'subject\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="subjectDesc" style="text-align:left;width:150px" class="fieldlabel"></span>'
				+ '</td></tr>'
			}
			else
			{			
				Desc += '<td><select class="contenttextCM" name=subject>'+BuildSelect("", "Subject")+'</select>'
			}	
			
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_250","CM")+'<td width=10>&nbsp;<td><input class="contenttextCM" type=text size=6 maxlength=6 name=perrating value="">'
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_251","CM")+'<td width=10>&nbsp;'
			
			if (emssObjInstance.emssObj.filterSelect)
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="instructor" value="" size="10" maxlength="10" onfocus="this.select()" onkeyup="this.value=\'\';document.getElementById(\'instructorDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'instructor\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="instructorDesc" style="text-align:left;width:150px" class="fieldlabel"></span>'
				+ '</td></tr>'
			}
			else
			{			
				Desc += '<td><select class="contenttextCM" name=instructor>'+BuildSelect("", "Institution")+'</select>'
			}
			
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_252","CM")+'<td width=10>&nbsp;<td>'
				 +'<input class="contenttextCM" type=text name=renewaldate value="" size=10 maxlength=10 onFocus=this.select()>'
				 +'<a class="contenttextCM" href="" onClick="parent.DateSelect(\'renewaldate\');return false;" onMouseOver="window.status=\''+getSeaPhrase("CM_238","CM")+'\';return true;"'
				 +' onMouseOut="window.status=\' \';return true;">'+uiCalendarIcon()+'</a><span class="contenttextCM">' + uiDateFormatSpan()+ '</span>';
			Desc += '<tr><td class="plaintablerowheaderlite">'+getSeaPhrase("CM_253","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=inproflag>'+BuildYesNo("")+'</select>'
			Desc += '<tr><td class="fieldlabelboldliteunderline">'+getSeaPhrase("CM_241","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=cosponsored>'+BuildYesNo("")+'</select>'
...
function UpdateQualificationButton_MyQualifications_OnClick()
{
	...
	if (_TABSELECTED == 0 || _TABSELECTED == 1) // Competencies or Certifications
	{
		if (typeof(qualForm.qualification) != "undefined") // Edits for "Add" screen
		{
			if (emssObjInstance.emssObj.filterSelect)
			{
				...
				if (NonSpace(qualForm.code.value) == 0)
				{
					...
					seaAlert(getSeaPhrase("CM_254","ESS"));
					qualForm.code.focus();
					return;
				}
			}
			...
			{		
				...
				{
					seaAlert(getSeaPhrase("CM_254","CM"))
					qualForm.code.focus()
					return
				}
			}	
		...
		if ((_TABSELECTED == 0) && NonSpace(qualForm.perrating.value) && !ValidNumber(qualForm.perrating,6,2))
		{
		...
		if (typeof(qualForm.degree) != "undefined") // Edits for "Add" screen
		{
			if (emssObjInstance.emssObj.filterSelect)
			{
				...
				if (NonSpace(qualForm.degree.value) == 0)
				{
					...
					seaAlert(getSeaPhrase("CM_255","CM"));
				...
				{
					setRequiredField(qualForm.subject);
					seaAlert(getSeaPhrase("CM_256","CM"))
...
function DrawCategoryInstructions_MyQualifications()
{
	...
	createLayer("main","Category_Instructions",100,75,300,20,true,Desc,"","");

	Desc = '<span class="contenttextCM">'+getSeaPhrase("CM_258","CM")+'</span>'
	createLayer("main","Category_InstructionsAdd",80,75,300,20,false,Desc,"","");
...
function DrawInstructions_EmployeeQualifications(SelectValues)
{
	...
	var Desc = '<span class="contenttextCM">'+getSeaPhrase("CM_263","CM")+'</span>'
...
function DataForQualificationsAlert(Index, Win, LinkIndex)
{
	...
	|| (EmployeeQualifications && _DIRECTREPORTSINDEX < 0))
	{
		if (EmployeeQualifications && _DIRECTREPORTSINDEX < 0) // This is a manager tab -- no report selected.
		{	
			return '<center><table border=0 cellpadding=0 cellspacing=0><tr>'
			+ '<td align=center><span class="contenttextCM">'+getSeaPhrase("CM_179","CM")+'</span></td></tr></table></center>'
		}
		else
			return ''
	}
	...
	for(var i=0; i<StandardCareerProfile.CompetencyDetail.length && (((MyQualifications || EmployeeQualifications) && Index==0) || ActionPlan); i++)
	{
		...
		if((!isNaN(dteDiff) && (dteDiff<=emssObjInstance.emssObj.cmQualExpireDays) && ActionPlan) || !ActionPlan)
		{
			...
			DataFlag = true;
			Desc += '<tr><td ' + LinkColor
			Desc += '><a class="contenttextCM" href="" onClick="parent.QualificationClicked('+i+',\'Competency\',\''+Win+'\');return false;" '
			Desc += '>'+ StandardCareerProfile.CompetencyDetail[i].description+'</a></td>';

			if(ActionPlan)
			{
				Desc += '<td ' + LinkColor + ' width=5px>&nbsp;</td>'
				Desc += '<td ' + LinkColor
				Desc += '><a class="contenttextCM" href="" onClick="parent.QualificationClicked('+i+',\'Competency\',\''+Win+'\');return false;">'
				Desc += FormatDte4(StandardCareerProfile.CompetencyDetail[i].renewDate)+'</a></td>';
			}
			Desc += '</tr>'
		}
	}
	...
	for(var i=0; i<StandardCareerProfile.CertificationDetail.length && (((MyQualifications || EmployeeQualifications) && Index==1) || ActionPlan); i++)
	{
		...
		if((!isNaN(dteDiff) && (dteDiff<=emssObjInstance.emssObj.cmQualExpireDays) && ActionPlan) || !ActionPlan)
		{
			...
			DataFlag = true;
			Desc += '<tr><td ' + LinkColor
			Desc += '><a class="contenttextCM" href="" onClick="parent.QualificationClicked('+i+',\'Certifications\',\''+Win+'\');return false;">'
			Desc += StandardCareerProfile.CertificationDetail[i].description+'</a></td>';
			...
			if(ActionPlan)
			{
				...
				Desc += '<td ' + LinkColor
				Desc += '><a class="contenttextCM" href="" onClick="parent.QualificationClicked('+i+',\'Certifications\',\''+Win+'\');return false">'
				Desc += FormatDte4(StandardCareerProfile.CertificationDetail[i].renewDate)+ '</a></td>';
			}
			Desc += '</tr>'
		}
	}
	...
	for(var i=0; i<StandardCareerProfile.EducationDetail.length && (((MyQualifications || EmployeeQualifications) && Index==2) || ActionPlan); i++)
	{
		...
		if((!isNaN(dteDiff) && (dteDiff<=emssObjInstance.emssObj.cmQualExpireDays) && ActionPlan) || !ActionPlan)
		{
			...
			DataFlag = true;
			Desc += '<tr><td ' + LinkColor
			Desc += '><a class="contenttextCM" href="" onClick="parent.QualificationClicked('+i+',\'Education\',\''+Win+'\');return false;">'
			Desc += StandardCareerProfile.EducationDetail[i].description+'</a></td>';
			...
			if(ActionPlan)
			{
				...
				Desc += '<td ' + LinkColor
				Desc += '><a class="contenttextCM" href="" onClick="parent.QualificationClicked('+i+',\'Education\',\''+Win+'\');return false;">'
				Desc += FormatDte4(StandardCareerProfile.EducationDetail[i].renewDate)+ '</a></td>';
			}
			Desc += '</tr>'
		}
	}
	...
	if(MyQualifications || EmployeeQualifications) {
		Desc += "<tr><td>"
		Desc += uiButton(getSeaPhrase("CM_259","CM"),"parent.AddQualificationButton_MyQualifications_OnClick();return false;","margin-top:10px")
		Desc += "</td></tr>"
	}
...
function PaintCompetency_DetailScreen(Index, Win, ReturnFunc)
{
	...
	{
		Desc = '<center><table border=0 cellpadding=0 cellspacing=0><tr><td class="contenttextCM" align="center">'
		+ getSeaPhrase("CM_265","CM")
		+ "</table></center>"
...
function FinishCompetency_DetailScreen(Index, Win, ReturnFunc)
{
	...
	{
		var Desc = '<center><table border=0 cellpadding=0 cellspacing=0>'
		Desc += '<tr><td width=150 class="contentlabelCM">'+getSeaPhrase("CM_232","CM")+'<td class="contenttextCM"> '+MatchForDescription(pDme.profic_level, "FilteredProficiency")+'</td>'
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_236","CM")+'<td class="contenttextCM"> '+PeRating+'</td>'
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_237","CM")+'<td class="contenttextCM"> '+formatDME(pDme.date_acquired)+'</td>'
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_239","CM")+'<td class="contenttextCM"> '+formatDME(pDme.renew_date)+'</td>'
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_240","CM")+'<td class="contenttextCM"> '+formatDME(pDme.date_returned)+'</td>'
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_234","CM")+'<td class="contenttextCM"> '+pDme.renewal_code_xlt+'</td>'
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_233","CM")+'<td class="contenttextCM"> '+pDme.skill_source_description+'</td>'
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_235","CM")+'<td class="contenttextCM"> '+pDme.instructor+'</td>'
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_241","CM")+'<td class="contenttextCM"> '+((pDme.co_sponsored=="Y")?getSeaPhrase("CM_169","CM"):getSeaPhrase("CM_168","CM"))+'</td>'
		Desc += '</table><center>'
	}
	else
	{
		if(Profile.CompetencyDetail[Index].updateFlag != "Y")
		{
			...
			var Desc = '<center><table border=0 cellpadding=0 cellspacing=0>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_46","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+Profile.CompetencyDetail[Index].description+'</td>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_232","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+MatchForDescription(pDme.profic_level, "FilteredProficiency")+'</td>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_233","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.skill_source_description+'</td>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_234","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.renewal_code_xlt+'</td>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_235","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.instructor+'</td>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_236","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+PeRating+'</td>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_237","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+formatDME(pDme.date_acquired)+'</td>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_239","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+formatDME(pDme.renew_date)+'</td>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_240","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+formatDME(pDme.date_returned)+'</td>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_241","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+((pDme.co_sponsored=="Y")?getSeaPhrase("CM_169","CM"):getSeaPhrase("CM_168","CM"))+'</td>'
			Desc += '</table></center>';
		}
		else
		{
			var showButtons = true;
			var Desc = '<center><form name="qualificationform"><table border=0 cellpadding=0 cellspacing=0>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_46","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+Profile.CompetencyDetail[Index].description+'</td>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_232","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=proficiency>'+BuildSelect(pDme.profic_level,"FilteredProficiency")+'</select>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_233","CM")+'<td width=10>&nbsp;'
			
			if (emssObjInstance.emssObj.filterSelect)
			{
				Desc += '<td class="contexttextCM" nowrap>'
				+ '<input class="contexttextCM" type="text" name="skillsource" value="'+pDme.skill_source+'" size="10" maxlength="10" onfocus="this.select()" onkeyup="this.value=\'\';document.getElementById(\'skillsourceDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'skillsource\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="skillsourceDesc" style="text-align:left;width:150px" class="fieldlabel">'+pDme.skill_source_description+'</span>'
				+ '</td></tr>'
			}
			else
			{			
				Desc += '<td><select class="contenttextCM" name=skillsource>'+BuildSelect(pDme.skill_source,"Source")+'</select></td></tr>'
			}
			
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_234","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=renewalcode>'+BuildRenewalSelect(pDme.renewal_code)+'</select>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_235","CM")+'<td width=10>&nbsp;<td><input class="contenttextCM" type=text size=10 maxlength=10 name=instructor value="'+pDme.instructor+'">'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_236","CM")+'<td width=10>&nbsp;<td><input class="contenttextCM" type=text size=6 maxlength=6 name=perrating value="'+PeRating+'">'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_237","CM")+'<td width=10>&nbsp;<td>'
				 +'<input class="contenttextCM" type=text name=dateacquired value="'+formatDME(pDme.date_acquired)+'" size=10 maxlength=10 onFocus=this.select()>'
				 +'<a class="contenttextCM" href="" onClick="parent.DateSelect(\'dateacquired\');return false;" onMouseOver="window.status=\''+getSeaPhrase("CM_238","CM")+'\';return true;"'
				 +' onMouseOut="window.status=\' \';return true;">'+uiCalendarIcon()+'</a><span class="contenttextCM">'+uiDateFormatSpan()+'</span>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_239","CM")+'<td width=10>&nbsp;<td>'
				 +'<input class="contenttextCM" type=text name=renewdate value="'+formatDME(pDme.renew_date)+'" size=10 maxlength=10 onFocus=this.select()>'
				 +'<a class="contenttextCM" href="" onClick="parent.DateSelect(\'renewdate\');return false;" onMouseOver="window.status=\''+getSeaPhrase("CM_238","CM")+'\';return true;"'
				 +' onMouseOut="window.status=\' \';return true;">'+uiCalendarIcon()+'</a><span class="contenttextCM">'+uiDateFormatSpan()+'</span>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_240","CM")+'<td width=10>&nbsp;<td>'
				 +'<input class="contenttextCM" type=text name=datereturned value="'+formatDME(pDme.date_returned)+'" size=10 maxlength=10 onFocus=this.select()>'
				 +'<a class="contenttextCM" href="" onClick="parent.DateSelect(\'datereturned\');return false;" onMouseOver="window.status=\''+getSeaPhrase("CM_238","CM")+'\';return true;"'
				 +' onMouseOut="window.status=\' \';return true;">'+uiCalendarIcon()+'</a><span class="contenttextCM">'+uiDateFormatSpan()+'</span>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_241","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=cosponsored>'+BuildYesNo(pDme.co_sponsored)+'</select>'

			Desc += '<tr><td>&nbsp;</td><td width=10>&nbsp;</td><td>'
			if(MyQualifications || EmployeeQualifications) {
				Desc += uiButton(getSeaPhrase("CM_95","CM"),"parent.UpdateQualificationButton_MyQualifications_OnClick();return false;","margin-top:10px")
				...
				Desc += uiButton(getSeaPhrase("CM_95","CM"),"parent.UpdateQualification_MyActionPlan_OnClick();return false;","margin-top:10px")
			Desc += '</td></tr>'
...
function FinishCertification_DetailScreen(Index, Win, ReturnFunc)
{
	...
	if(JobProfileFlag || EmployeeProfile || ExploreCareers)
	{
		Desc = '<center><table border=0 cellpadding=0 cellspacing=0>'
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_243","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+ReturnStateDescription(pDme.state)
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_244","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.lic_number
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_237","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+formatDME(pDme.date_acquired)
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_240","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+formatDME(pDme.renew_date)
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_234","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.renewal_code_xlt
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_233","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.skill_source_description
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_241","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+((pDme.co_sponsored=="Y")?getSeaPhrase("CM_169","CM"):getSeaPhrase("CM_168","CM"))
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_245","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.cost
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_246","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.currency_description
		Desc += '</table></center>'
	}
	else
	{
		if(Profile.CertificationDetail[Index].updateFlag != "Y")
		{
			var showButtons = false;
			Desc = '<center><table border=0 cellpadding=0 cellspacing=0>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_46","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+Profile.CertificationDetail[Index].description
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_243","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+ReturnStateDescription(pDme.state)
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_244","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.lic_number
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_237","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+formatDME(pDme.date_acquired)
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_240","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+formatDME(pDme.renew_date)
		...
		{
			var showButtons = true;
			Desc = '<center><form name="qualificationform">'
			Desc += '<table border=0 cellpadding=0 cellspacing=0>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_46","CM") +'<td width=10>&nbsp;<td class="contenttextCM"> '+Profile.CertificationDetail[Index].description
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_243","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=state>'+BuildStateSelect(pDme.state)+'</select>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_244","CM")+'<td width=10>&nbsp;<td><input class="contenttextCM" type=text name=licnumber size=15 maxlength=20 value="'+pDme.lic_number+'">'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_237","CM")+'<td width=10>&nbsp;<td>'
				 +'<input class="contenttextCM" type=text name=dateacquired value="'+formatDME(pDme.date_acquired)+'" size=10 maxlength=10 onFocus=this.select()>'
				 +'<a class="contenttextCM" href="" onClick="parent.DateSelect(\'dateacquired\');return false;" onMouseOver="window.status=\''+getSeaPhrase("CM_238","CM")+'\';return true;"'
				 +' onMouseOut="window.status=\' \';return true;">'+uiCalendarIcon()+'</a><span class="contenttextCM">'+uiDateFormatSpan()+'</span>'
			...
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="skillsource" value="'+pDme.skill_source+'" size="10" maxlength="10" onfocus="this.select()" onkeyup="this.value=\'\';document.getElementById(\'skillsourceDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'skillsource\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="skillsourceDesc" style="text-align:left;width:150px" class="fieldlabel">'+pDme.skill_source_description+'</span>'
				+ '</td></tr>'
			...
			{			
				Desc += '<td><select class="contenttextCM" name=skillsource>'+BuildSelect(pDme.skill_source, "Source")+'</select>'
			}
			
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_241","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=cosponsored>'+BuildYesNo(pDme.co_sponsored)+'</select>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_245","CM")+'<td width=10>&nbsp;<td><input class="contenttextCM" type=text size=14 maxlength=14 name=cost value="'+parseFloat(pDme.cost)+'">'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_246","CM")+'<td width=10>&nbsp;'
			
			if (emssObjInstance.emssObj.filterSelect)
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="currencycode" value="'+pDme.currency_code+'" size="5" maxlength="5" onfocus="this.select()" onkeyup="this.value=\'\';document.getElementById(\'currencycodeDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'currencycode\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="currencycodeDesc" style="text-align:left;width:150px" class="fieldlabel">'+pDme.currency_description+'</span>'
				+ '</td></tr>'
			}
			else
			{			
				Desc += '<td><select class="contenttextCM" name=currencycode>'+BuildSelect(pDme.currency_code, "Currency")+'</select>'
			}
...
function FinishEducation_DetailScreen(Index, Win, ReturnFunc)
{
	...
	{
		Desc = '<center><table border=0 cellpadding=0 cellspacing=0>'
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_249","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.subject_description
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_251","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.institution_description
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_250","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+PeRating
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_252","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+formatDME(pDme.renew_date)
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_253","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+inproflag
		Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_241","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+((pDme.co_sponsored=="Y")?getSeaPhrase("CM_169","CM"):getSeaPhrase("CM_168","CM"))
		Desc += '</table></center>'
	}
	else
	{
		if(Profile.EducationDetail[Index].updateFlag != "Y")
		{
			var showButtons = false;
			Desc = '<center><table border=0 cellpadding=0 cellspacing=0>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_248","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.code
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_249","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.subject_description
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_250","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+PeRating
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_251","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+pDme.institution_description
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_252","CM")+'<td width=10>&nbsp;<td class="contenttextCM"> '+formatDME(pDme.renew_date)
		...
		else
		{
			...
			Desc = '<center><form name="qualificationform">'
			Desc += '<table border=0 cellpadding=0 cellspacing=0>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_248","CM")+'<td width=10>&nbsp;<td class="contenttextCM">'+Profile.EducationDetail[Index].description
			//<select class="contenttextCM" name=degree>'+BuildSelect(pDme.code, "Degree")+'</select>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_249","CM")+'<td width=10>&nbsp;'
			
			if (emssObjInstance.emssObj.filterSelect)
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="subject" value="'+pDme.subject+'" size="10" maxlength="10" onfocus="this.select()" onkeyup="this.value=\'\';document.getElementById(\'subjectDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'subject\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="subjectDesc" style="text-align:left;width:150px" class="fieldlabel">'+pDme.subject_description+'</span>'
				+ '</td></tr>'
			}
			else
			{			
				Desc += '<td><select class="contenttextCM" name=subject>'+BuildSelect(pDme.subject, "Subject")+'</select>'
			}
			
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_250","CM")+'<td width=10>&nbsp;<td><input class="contenttextCM" type=text size=6 maxlength=6 name=perrating value="'+PeRating+'">'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_251","CM")+'<td width=10>&nbsp;'
			
			if (emssObjInstance.emssObj.filterSelect)
			{
				Desc += '<td class="contenttextCM" nowrap>'
				+ '<input class="contenttextCM" type="text" name="instructor" value="'+pDme.instructor+'" size="10" maxlength="10" onfocus="this.select()" onkeyup="this.value=\'\';document.getElementById(\'instructorDesc\').innerHTML=\'\';">'
				+ '<a href="javascript:parent.openDmeFieldFilter(\'instructor\')" style="margin-left:5px">'
				+ '<img align="top" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" border="0"></a>'
				+ '<span id="instructorDesc" style="text-align:left;width:150px" class="fieldlabel">'+pDme.institution_description+'</span>'
				+ '</td></tr>'
			}
			else
			{						
				Desc += '<td><select class="contenttextCM" name=instructor>'+BuildSelect(pDme.instructor, "Institution")+'</select>'
			}
			
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_252","CM")+'<td width=10>&nbsp;<td>'
				 +'<input class="contenttextCM" type=text name=renewaldate value="'+formatDME(pDme.renew_date)+'" size=10 maxlength=10 onFocus=this.select()>'
				 +'<a class="contenttextCM" href="" onClick="parent.DateSelect(\'renewaldate\');return false;" onMouseOver="window.status=\''+getSeaPhrase("CM_238","CM")+'\';return true;"'
				 +' onMouseOut="window.status=\' \';return true;">'+uiCalendarIcon()+'</a><span class="contenttextCM">'+uiDateFormatSpan()+'</span>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_253","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=inproflag>'+BuildYesNo(pDme.in_pro_flag)+'</select>'
			Desc += '<tr><td class="contentlabelCM">'+getSeaPhrase("CM_241","CM")+'<td width=10>&nbsp;<td><select class="contenttextCM" name=cosponsored>'+BuildYesNo(pDme.co_sponsored)+'</select>'
...
{	
	...
	showWaitAlert(getSeaPhrase("CM_186","CM"));
	GetCompProficiencyLevels(type, code, -1, null, null, false, true);
}
...
{
	var formObj = (_CURRENTTAB == "ACTIONPLAN") ?
		GetFormFromStansWindowLayer("main", "QualificationsDetail_MyActionPlanWindow", "qualificationform"):
		GetFormFromStansWindowLayer("main", "CompetencyDetail_MyQualificationsWindow", "qualificationform");
...
function UpdateQualification_MyActionPlan_OnClick(/*FC*/)
{
	...
	var pAgsObj;
	var pFormObj = (_CURRENTTAB == "ACTIONPLAN")?
		GetFormFromStansWindowLayer("main", "QualificationsDetail_MyActionPlanWindow", "qualificationform"):
		GetFormFromStansWindowLayer("main", "CompetencyDetail_MyQualificationsWindow", "qualificationform");
	...
	case "Competency":
		...
		ptFields += "&PT-DATE-ACQUIRED="+ParseAGSValue(formjsDate(pFormObj.dateacquired.value));
		...
		pAgsObj.field += "&EPC-RENEWAL-CODE1="+ParseAGSValue(pFormObj.renewalcode.options[pFormObj.renewalcode.selectedIndex].value,1)
			...
			+ "&SKI-DATE-ACQUIRED1="+ParseAGSValue(formjsDate(pFormObj.dateacquired.value))
			+ "&SKI-DATE-LST-USED1="+ParseAGSValue(formjsDate(pFormObj.renewdate.value))
			+ "&EPC-DATE-RETURNED1="+ParseAGSValue(formjsDate(pFormObj.datereturned.value))
	...
	case "Certifications":
		...
		ptFields += "&PT-CER-DATE-ACQUIRED="+ParseAGSValue(formjsDate(pFormObj.dateacquired.value));
		pAgsObj.field += "&EPC-STATE1="+ParseAGSValue(pFormObj.state.options[pFormObj.state.selectedIndex].value,1)
			...
			+ "&CER-DATE-ACQUIRED1="+ParseAGSValue(formjsDate(pFormObj.dateacquired.value))
			+ "&CER-RENEW-DATE1="+ParseAGSValue(formjsDate(pFormObj.renewdate.value))
	...
	case "Education":
		...
		if(FC=="C" || FC=="A")
		{
			...
			ptFields += "&PT-EDU-COM-DATE="+ParseAGSValue(formjsDate(pFormObj.renewaldate.value))
			
			pAgsObj.field += "&EDU-COM-DATE1="+ParseAGSValue(formjsDate(pFormObj.renewaldate.value))
	...
	showWaitAlert(getSeaPhrase("CM_266","CM"));
	pAgsObj.event 		= "CHG";
	pAgsObj.rtn       	= "MESSAGE";
	pAgsObj.longNames 	= "ALL";
	pAgsObj.tds       	= false;
...
function DrawJobListWindow_ExploreCareers(isVisible, SelectValues)
{
	...
	{
		Desc = '<center><table border=0 cellpadding=5 cellspacing=0><tr>'
			+ '<td><span class="contenttextCM">' + getSeaPhrase("CM_175","CM")
			+ '</span><p/></td></tr></table></center>'
	}
	else if (_CURRENTTAB == "M_EXPLORECAREERS")
	{
		Desc = '<center><table border=0 cellpadding=5 cellspacing=0><tr>'
			+ '<td><span class="contenttextCM">' + getSeaPhrase("CM_176","CM")
			+ '</span><p/></td></tr></table></center>'
	}
	else
	{
		Desc = '<center><table border=0 cellpadding=5 cellspacing=0><tr>'
			+ '<td><span class="contenttextCM">' + getSeaPhrase("CM_177","CM")
			+ '</span><p/></table></center>'
	}
	createLayer("main","InstructionsHeader_JobList",375,200,400,50,isVisible,Desc,"","");

	if (_CURRENTTAB == "M_EXPLORECAREERS" && _EXPLORECURRENTWIN == "EXPLORECATEGORY")
	{
		Desc = '<span class="contenttextCM">'+getSeaPhrase("CM_178","CM")+'</span>'
		...
		MakeButton(getSeaPhrase("CM_117","CM"), 475, 372, 150, 30, ButtonBorderColor, "createprofile_ManagerExploreCareers", isVisible);
	}
...
{
	var Msg = "<p><table width=350 border=0 cellpadding=0 cellspacing=0><tr><td>";
	var noJobs = false;
	...
	if (!noJobs)
	{
		Desc += '<table border=0 cellpadding=5 cellspacing=0><tr><td>'
			+ '<span class="contenttextCM">' + getSeaPhrase("CM_183","CM") + '</span></td></tr></table>';
	}

	replaceContent("main","InstructionsHeader_JobList","");

	Desc += "<p><table border=0 cellpadding=0 cellspacing=0>"
	for (var i=0;i<QualificationJobList.Detail.length;i++)
	{
		for (var j=0;j<QualificationJobList.Detail[i].JobDescs.length;j++)
		{
			Desc += "<tr><td"
			if (typeof(Index1) != "undefined" && Index1 != null && Index1 == i &&
				typeof(Index2) != "undefined" && Index2 != null && Index2 == j)
			{
				Desc += " class=tablerowhighlightCM"
			}
			Desc += " nowrap><a class=contenttextCM href=\"\" onClick=\"parent.ReDrawQualCriteriaJobListContent("+i+","+j+");"
			Desc += "parent.JobCodeSelected('Qualification',"+i+","+j+");return false;\">"
			Desc += QualificationJobList.Detail[i].JobDescs[j].description+"</a></td></tr>"
		}
	}
	Desc += "</table></center>"
...
function RepaintCategoryJobListLinks(CatIndex, LinkIndex)
{
	...
	{
		var Desc = '<center><span class="contenttextCM">'+getSeaPhrase("CM_184","CM")+"</span><p>"
		Desc += "<table border=0 cellpadding=0 cellspacing=0>"
		for (var i=0;i<JobClass[CatIndex].Rel_jobcodes.length;i++)
		{
			...
			if (typeof(LinkIndex) != "undefined" && LinkIndex != null && LinkIndex == i)
				Desc += ' class="tablerowhighlightCM"'
			Desc += " nowrap><a class=contenttextCM href=\"\" onClick=\"parent.RepaintCategoryJobListLinks("+CatIndex+","+i+");parent.JobCodeSelected('"
			Desc += "Category',"+CatIndex+","+i
			Desc += ");return false;\">"
			Desc += JobClass[CatIndex].Rel_jobcodes[i].description+"</a></td></tr>"
		}
		Desc += "</table></center>"
		ReplaceWindowContent("main","JobListWindow",Desc);
	}
...
function DrawQualificationSelectBoxes(Name, QualData, Visible, Top)
{
	...
	Desc = '<table border="0" width="300" cellpadding="0" cellspacing="0">'
		+ '<tr>'
		+ '<th align="center" class="contentlabelCM">'+getSeaPhrase("CM_50","CM") + '</th>'
		+ '<th align="center" class="contentlabelCM">'+getSeaPhrase("CM_46","CM") + '</th>'
		+ '</tr></table>'
	...
	{
		Desc = '<span class="contenttextCM">'+QualData.QualLabels[j-1]+'</span>'
		...
		MakeButton('<img style="cursor:hand;cursor:pointer" border="0" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" alt="'+getSeaPhrase("SELECT_VALUE","ESS")+'">', ArrowLeft, Top+(j*30), 12, 16,
...
function QualCriteriaArrowClicked(BoxNbr, Name, Button)
{
	...
	Desc = "<p><p><center><table border=0 cellpadding=0 cellspacing=0>"
	for (var i=0;i<SelectValues.length;i++)
	{
		Desc += "<tr><td nowrap><a class=contenttextCM href=\"\" onClick=\"parent.PickedQualificationType('"
			 + escape(SelectValues[i],1)+"',"+BoxNbr+",'"+escape(Name,1)
			 + "','"+escape(Button,1)+"');return false;\">"
			 + SelectValues[i]+"</a>"
	}
	Desc += "</table></center>"
...
function PickedQualificationType(QualType, BoxNbr, Name, Button)
{
	...
	Desc = "<p><center><table border=0 cellpadding=0 cellspacing=0>"
	for (var i=0;i<QualificationCodes.length;i++)
	{
		Desc += "<tr><td nowrap><a class=contenttextCM href=\"\" onClick=\"parent.PickedQualificationCode('"
			 + escape(QualificationCodes[i].type,1)+"','"+escape(QualificationCodes[i].code,1)
			 + "','"+escape(QualificationCodes[i].description,1)+"',"+BoxNbr+",'"+Name
			 + "','"+Button+"');return false;\">"
			 + QualificationCodes[i].description+"</a>"
	}
	Desc += "</table></center>"
...
function ShowQualCriteriaWin_ExploreCareers(nCmd, Name, Button)
{
    	...
    	if (nCmd)
	{
		if (_CURRENTTAB == "M_EXPLORECAREERS")
		{
			...
			//showLayer("main", Name+"_DirectReportsLabel");
		}
		else
		{
	...
	else
	{
		if (_CURRENTTAB == "M_EXPLORECAREERS")
		{
			...
			//hideLayer("main", Name+"_DirectReportsLabel");			
		}
		else
		{
         		hideLayer("main", Name+"_Instructions");
...
{
	if(nCmd)		showLayer("main", "JobTitle_Instructions");
	else			hideLayer("main", "JobTitle_Instructions");
}

function ShowJobTitleLabel_ExploreCareers(nCmd)
{
	if(nCmd)		showLayer("main", "JobTitle_JobTitleLabel");
	else			hideLayer("main", "JobTitle_JobTitleLabel");
}
...
{
	if(nCmd)		showLayer("main", "JobList_JobLabel");
	else			hideLayer("main", "JobList_JobLabel");
}

function ShowDirectReportsLabel_ExploreCareers(nCmd, Tab)
{
	if(nCmd)		showLayer("main", Tab+"_DirectReportsLabel");
	else			hideLayer("main", Tab+"_DirectReportsLabel");
}
...
function Do_HS50_1_Call_DEFAULTPROFILE_Finished(useDefault)
{
	...
	if (_CURRENTTAB == "M_EXPLORECAREERS") // Manager Explore Careers
	{
		...
		{
			seaAlert(getSeaPhrase("CM_188","CM"));
			ShowJobProfileWin(false,"main");
			removeWaitAlert();
			return;
		}
...
function DrawCourseDetail_Manager(TabName)
{
	...
	{
		var Desc = '<span class="contenttextCM">'+getSeaPhrase("CM_282","CM")+'</span>'
...
function cmbDirectReports_SESS_OnChange()
{
	...
	{
		// PT 107336
		// _EMPLOYEEINDEX = DirectReports[FormObj.selectedIndex-1].code;
		_DIRECTREPORTSINDEX = FormObj.selectedIndex - 1;
	}
}
...
function SubmitApplication_OnClick(TabName)
{
	...
	if (!sessionChecked)
	{
		seaAlert(getSeaPhrase("CM_284","CM"));
		return;
	}
	...
	if (sessionStatus == "FA" || sessionStatus == "FL") 
	{
		...
		{
			SubmitTrainingSession(sessionIndex, "W");
		}
	}
	else
	{
		SubmitTrainingSession(sessionIndex, "C");
	}
}
...
function SubmitTrainingSession(sessionIndex, status)
{
	showWaitAlert(getSeaPhrase("CM_186","CM"));
...
{
	if(self.lawheader.gmsgnbr == "000")
		seaAlert(getSeaPhrase("CM_285","CM"))
	else
		seaAlert(self.lawheader.gmsg)
...
{
	var Desc = '<span class="layertitleCM">'
			+ getSeaPhrase("CM_280","CM")+" - "+ TrainingCourses[CourseIndex].Course.courseDescription
			+ '</span>'
	...
	{
		case 0:
			if (TrainingCourses[CourseIndex].Course.sessionFlag == null ||
				TrainingCourses[CourseIndex].Course.sessionFlag != "Y")
			{
				DrawCourseDetailTabs(true, 0, false, TabName);
				__TrainingCoursesIndexSelected = CourseIndex;
				var Desc = '<center><table border=0 cellspacing=0 cellpadding=0><tr><td align=center>'
				+ '<span class="contenttextCM">'+getSeaPhrase("CM_286","CM")+' '+ TrainingCourses[CourseIndex].Course.courseDescription 
				+ '.</span></table></center>'
			...
			break;
		case 1:
			if (TrainingCourses[CourseIndex].Course.preRequisitesFlag == null ||
				TrainingCourses[CourseIndex].Course.preRequisitesFlag != "Y")
			{
				DrawCourseDetailTabs(true, 1, false, TabName);
				__TrainingCoursesIndexSelected = CourseIndex;
				if(_CURRENTTAB == "M_TRAININGOPTIONS")
				...
				var Desc = '<center><table border=0 cellspacing=0 cellpadding=0><tr><td align=center>'
				+ '<span class="contenttextCM">'
				+ getSeaPhrase("CM_287","CM")+' '+ TrainingCourses[CourseIndex].Course.courseDescription 
				+ '.</span></table></center>'
				ReplaceWindowContent("main", "CourseDetail_"+TabName+"Window", Desc);
				ShowCourseDetailWin(true, TabName);
				ShowSubmitApplication(false, TabName);
...
function PaintSessions(Index, TabName)
{
	...
	var arg = '<center><form name=sessionForm><table border=0 cellpadding=0 cellspacing=0>';
	...
	for(var i=0, j=0;i<self.jsreturn.record.length;i++)
	{
		...
		for(var k=0; k<TrainingCourses[Index].Sessions.length; k++)
		{
			...
			if(parseInt(sessObj.session,10) == parseInt(pObj.session,10) && isEqual(sessObj.sessionStartDate,formjsDate(formatDME(pObj.start_date))))
			{
				...
				PrintFlag = true;
				arg += '<tr><td>'
				+ '<input type=radio value="on" name=session>'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_288","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+pObj.session
				+ '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_289","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+formatDME(pObj.start_date)
				+ '<td>&nbsp;&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_290","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+formatDME(pObj.end_date)
				+ '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_291","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM" colspan=4>'+pObj.site
				+ '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_292","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM" colspan=4>'+pObj.facility_description
				+ '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_293","CM")+':&nbsp;&nbsp;'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+pObj.start_time
				+ '<td>&nbsp;&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_294","CM")+':&nbsp;&nbsp;'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+pObj.end_time
				+ '<tr><td colspan=6>&nbsp;'
				+ '<input type=hidden value="'+pObj.session+'" name=sessionCode'+j+'>'
	...
	arg += '</table></form></center>'

	if(!SessionObj.length || !PrintFlag)
	{
		Desc = '<center><table border=0 cellspacing=0 cellpadding=0><tr><td align=center>'
		+ '<span class="contenttextCM">'+getSeaPhrase("CM_286","CM")+' '+ TrainingCourses[Index].Course.courseDescription 
		+ '.</span></table></center>'
		ReplaceWindowContent("main", "CourseDetail_"+TabName+"Window", Desc);
		if(_CURRENTTAB == "M_TRAININGOPTIONS")
		{
...
function ContinuePaintingPreReqs(Index, TabName)
{
	...
	if(!PreReqs.length)
	{
		var arg = '<center><table border=0 cellspacing=0 cellpadding=0><tr><td align=center>'
		+ '<span class="contenttextCM">'
		+ getSeaPhrase("CM_287","CM")+' '+ TrainingCourses[Index].Course.courseDescription 
		+ '.</span></table></center>'
	}
	else
	{
		...
		{
			var pObj = PreReqs[i];
			if ((self.jsreturn.record || typeof(self.jsreturn.record) != "undefined")
			&& (self.jsreturn.record[0] || typeof(self.jsreturn.record[0]) != "undefined"))
			{
			...
			else pObj.Eligible = null

			if (typeof(pObj.Type) != "undefined" && pObj.Type != null && NonSpace(pObj.Type))
			{
				arg += '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_50","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+pObj.Type
			}
			if (typeof(pObj.Description) != "undefined" && pObj.Description != null && NonSpace(pObj.Description))
			{
				arg += '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_134","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+pObj.Description
			}
			if (typeof(pObj.CompanyName) != "undefined" && pObj.CompanyName != null && NonSpace(pObj.CompanyName))
			{
				arg += '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_307","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+pObj.CompanyName
			}
			if (typeof(pObj.FromDate) != "undefined" && pObj.FromDate != null && NonSpace(pObj.FromDate))
			{
				arg += '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_308","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM" colspan=4>'+FormatDte3(formjsDate(pObj.FromDate))
			}
			if (typeof(pObj.Eligible) != "undefined" && pObj.Eligible != null && NonSpace(pObj.Eligible))
			{
				arg += '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_309","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+FormatDte3(pObj.Eligible)
			}
			if (typeof(pObj.MinimumProficiency) != "undefined" && pObj.MinimumProficiency != null && NonSpace(pObj.MinimumProficiency))
			{
				arg += '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_310","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+pObj.MinimumProficiency
			}
			if (typeof(pObj.Length) != "undefined" && pObj.Length != null && NonSpace(pObj.Length)
			&& typeof(pObj.DateMeasure) != "undefined" && pObj.DateMeasure != null && NonSpace(pObj.DateMeasure))
			{
				arg += '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_311","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM">'+parseFloat(pObj.DateMeasure)+'&nbsp;'
			...
			{
				arg += '<tr><td>&nbsp;'
				+ '<td class="contentlabelCM">'+getSeaPhrase("CM_305","CM")+':'
				+ '<td width=10>&nbsp;'
				+ '<td class="contenttextCM" colspan=4>'+pObj.Required
			}
			arg += '<tr><td colspan=6>&nbsp;'
		}
		arg += '</table></center>'
...
function GetProfileInformation(Tab)
{
	...
	var Profile = (useDefault)?StandardCareerProfile:CareerProfile;
	var ManagerTab = (_CURRENTTAB.substring(0,2)=="M_"||_CURRENTTAB=="INDIVIDUALPROFILE"|| 
		_CURRENTTAB=="GROUPPROFILE"||_CURRENTTAB=="EMPLOYEEQUALIFICATIONS"||_CURRENTTAB=="EMPLOYEEACTIONPLAN")?true:false;
	var pObj;
	...
	if ((ManagerTab && _DIRECTREPORTSINDEX < 0) || pObj.length == 0) 
	{
		...
		{
			Desc = '<center><table border=0 cellpadding=0 cellspacing=0><tr>'
			+ '<td align=center><span class="contenttextCM">'+getSeaPhrase("CM_199","CM")+'</span></td></tr></table></center>'
		}	
	}
	else
	{
...
// Provide this function to see if it fires off of the chart links.
...
function Do_HS50_3_Call_JOBPROFILE_Finished(Index)
{
	...
	{
		arg = '<center><table border=0 cellpadding=0 cellspacing=0>'
		for(var i=0;i<TrainingCourses.length; i++) {
			arg += '<tr><td><a class="contenttextCM" href="javascript:void(0);" onClick="parent.RepaintCourses_MyJobProfileLinks('+i+');parent.ShowJobProfileSessions('+i+');return false;">'
	...
	{
		arg = '<center><table border=0 cellpadding=0 cellspacing=0><tr><td align=center>'
		+ '<span class="contenttextCM">' + getSeaPhrase("CM_200","CM") + '</span></table></center>'
		ReplaceWindowContent("main", "Courses_MyJobProfileWindow", arg);
	}
...
function RepaintCourses_MyJobProfileLinks(Index)
{
		...
		for(var i=0;i<TrainingCourses.length; i++)
		{
			...
			arg += '><a class="contenttextCM" href="javascript:void(0);" onClick="parent.RepaintCourses_MyJobProfileLinks('+i+');parent.ShowJobProfileSessions('+i+');return false;">'
			arg += TrainingCourses[i].Course.courseDescription + '</a><br>'
		}
...
function OnTabClicked_JobProfile_MyJobProfile(TabName)
{
	if (_CURRENTTAB.indexOf("EXPLORECAREERS")!=-1)
	{
		...
		switch(TabName)
		{
			case "Competencies": 	//if (_TABSELECTED != 0) 
						//{
							_TABSELECTED = 0; 
							OpenProgram(Index, _TABSELECTED, 0);
						//}
						break;
			case "Certifications": 	//if (_TABSELECTED != 1) 
						//{
							_TABSELECTED = 1; 
							OpenProgram(Index, _TABSELECTED, 1);
						//}
						break;
			case "Education": 	//if (_TABSELECTED != 2)
						//{
							_TABSELECTED = 2; 
...
function PrintProfile_MyJobProfile_OnClick()
{
	...
	self.PGCONTROL.document.title = getSeaPhrase("CM_128","CM");
	self.PGCONTROL.document.getElementById("paneBody").innerHTML = Desc;
	self.PGCONTROL.stylePage();
	self.PGCONTROL.document.getElementById("paneBorder").style.height = "auto";
	self.PGCONTROL.document.getElementById("paneBodyBorder").style.height = "auto";	
	showWaitAlert(getSeaPhrase("CM_202","CM"));
	setTimeout(function() { self.PGCONTROL.focus(); self.PGCONTROL.print(); removeWaitAlert(); }, 2000);
}

function PrintJobDetail_MyJobProfile_OnClick()
{
	var Desc = '<table border="0" cellspacing="0" cellpadding="0"><tr><td class="contenttextCM">';
	var JobDesc = '';
	var PosDesc = '';
	...
	if(_DIRECTREPORTSINDEX >=0 && DirectReports[_DIRECTREPORTSINDEX])
		Desc += '<span class="contentlabelCM">'+getSeaPhrase("CM_203","CM")+'</span> &nbsp;'+DirectReports[_DIRECTREPORTSINDEX].description+'<br>'
	else
		Desc += '<span class="contentlabelCM">'+getSeaPhrase("CM_203","CM")+'</span> &nbsp;'+authUser.name+'<br>'
	
	Desc += '<span class="contentlabelCM">'+getSeaPhrase("CM_204","CM")+'</span> &nbsp;'+JobDesc+'<br>';
	Desc += '<span class="contentlabelCM">'+getSeaPhrase("CM_395","CM")+'</span> &nbsp;'+ PosDesc+'<p>';
	Desc += JobDetailInformation(false);
	
	self.PGCONTROL.document.title = getSeaPhrase("CM_201","CM");
	self.PGCONTROL.document.getElementById("paneBody").innerHTML = Desc;
	self.PGCONTROL.stylePage();
	self.PGCONTROL.document.getElementById("paneBorder").style.height = "auto";
	self.PGCONTROL.document.getElementById("paneBodyBorder").style.height = "auto";	
...
{	
	...
	// PT 139619
	// if this is a relative URL, try loading under the Portal directory and careermanagement directories
	if (urlStr.indexOf(location.protocol) != 0) {
		urlStr = origUrlStr;
		...
		if (!fileExists(urlStr)) {
			...
			urlStr = location.protocol + "//" + location.host + "/lawson/xhrnet/careermanagement/" + urlStr;
	...
	{
		var Desc = '<iframe id="jobdesc" name="jobdesc" height="100%" width="100%" frameborder="0" src="'+urlStr+'" '
			+'></iframe>'
...
function JobDetailInformation(notForPrint)
{
	...
	var Desc = '<center><table border=0 cellpadding=3 cellspacing=0><tr>'
		+ '<th class="contentlabelCM" width=130>' + getSeaPhrase("CM_50","CM") + '</th>'
		+ '<th class="contentlabelCM" width=70>' + getSeaPhrase("CM_212","CM") + '</th>'
		+ '<th class="contentlabelCM" width=70>' + getSeaPhrase("CM_213","CM") + '</th>'
		+ '</tr>'

	for(var i=0; i<OrderOfExecutionOfType.length;i++)
	{
		Desc += '<tr><td class="contentlabelCM" colspan=3>'+OrderOfExecutionOfDescription[i]
		var FOUND = false;
		for(var j=0, k=0; j<JobDetail.Detail.length; j++)
		{
			...
			{
				FOUND = true;
				Desc += '<tr><td class="contenttextCM">'
					+ JobDetail.Detail[j].description
					+ '</td><td align=center class="contenttextCM">'
					+ ((JobDetail.Detail[j].requiredFlag=="Y")?getSeaPhrase("CM_169","CM"):getSeaPhrase("CM_168","CM"))
					+ '</td><td align=center class="contenttextCM">'
					+ parseFloat(JobDetail.Detail[j].pctOfTime)
					+ '</td></tr>'
			}
		}
		if(!FOUND)
...
function DrawInstructionsManager_MyJobProfile(SelectValues)
{
	...
	var Desc = '<span class="contenttextCM">' + getSeaPhrase("CM_215","CM") + '</span>'
	createLayer("main","InstructionsHeader_IndividualProfile",415,55,400,50,true,Desc,"","");
	
	MakeSelectBox("DirectReports", 525, 75, 275, 30, "cmbDirectReports", SelectValues, true, _DIRECTREPORTSINDEX, true)

	Desc = '<span class="contenttextCM">'+getSeaPhrase("CM_216","CM")+'</span>'
	createLayer("main","InstructionsHeader",415,105,400,100,true,Desc,"","");
	
	createLayer("main","InstructionsHeaderAlert",415,165,400,75,true,"","","");
}
...
function DrawInstructions_MyJobProfile()
{
	...
	Desc = '<span class="contenttextCM">'+getSeaPhrase("CM_216","CM")+'</span>'
	createLayer("main","InstructionsHeader",415,55,400,100,true,Desc,"","");
	...
	Desc += '</span>'
	createLayer("main","InstructionsHeaderAlert",415,150,400,75,true,Desc,"","");
}
...
function DrawJobProfile_MyJobProfile()
{
	...
	var Desc = ''
	if(typeof(Profile) == "undefined" || Profile == null || 
		!Profile.jobDescFlag || Profile.jobDescFlag == null)
	{	
		Desc += '<span class="contenttextCM">'+getSeaPhrase("CM_205","CM")+'</span>'
	}
	else
	{
		Desc += '<a class="contenttextCM" href="javascript:void(0);" onClick="parent.JobDescription_MyJobProfile_OnClick('+useDefault+');" '
	...
	createLayer("main","JobDescriptionLink_MyJobProfile",90,475,100,20,isVisible,Desc,"","");

	Desc = ''
	if(typeof(Profile) == "undefined" || Profile == null || 
		((!Profile.jobDetailFlag || Profile.jobDetailFlag == null) && (!Profile.posDetailFlag || Profile.posDetailFlag == null)))
	{	
		Desc += '<span class="contenttextCM">'+getSeaPhrase("CM_201","CM")+'</span>'
	}
	else
	{
		Desc += '<a class="contenttextCM" href="javascript:void(0);" onClick="parent.AdditionalJobDetail_MyJobProfile_OnClick('+useDefault+');" '
...
function RefreshJobProfileLink_MyJobProfile()
{
	...
	{
		Desc += '<a class="contenttextCM" href="javascript:void(0);" onClick="parent.JobDescription_MyJobProfile_OnClick('+false+');" '
		Desc += 'onMouseOver="window.status=\''+getSeaPhrase("CM_223","CM")+'\';return true;" onMouseOut="window.status=\' \';return true;">'
		Desc += getSeaPhrase("CM_205","CM")+'</a>'
	}	
	...
	{
		Desc += '<a class="contenttextCM" href="javascript:void(0);" onClick="parent.AdditionalJobDetail_MyJobProfile_OnClick('+false+');" '
		Desc += 'onMouseOver="window.status=\''+getSeaPhrase("CM_224","CM")+'\';return true;" onMouseOut="window.status=\' \';return true;">'
		Desc += getSeaPhrase("CM_201","CM")+'</a>'
	}
...
{
	if(nCmd)		showLayer("main", "JobDescriptionLink_MyJobProfile");
	else			hideLayer("main", "JobDescriptionLink_MyJobProfile");
}

function ShowAdditionalJobDetailLink_MyJobProfile(nCmd)
{
	if(nCmd)		showLayer("main", "AdditionalJobDetailLink_MyJobProfile");
	else			hideLayer("main", "AdditionalJobDetailLink_MyJobProfile");
}

function ShowInstructionsHeader_MyJobProfile(nCmd)
{
	if(nCmd)		showLayer("main", "InstructionsHeader_MyJobProfile");
	else			hideLayer("main", "InstructionsHeader_MyJobProfile");
}

function ShowInstructionsHeaderAlert_MyJobProfile(nCmd)
{
	if(nCmd)		showLayer("main", "InstructionsHeaderAlert_MyJobProfile");
	else			hideLayer("main", "InstructionsHeaderAlert_MyJobProfile");
}
...
function SetupCourseListForCategoryTab_TrainingOptions()
{
	...
	{
		seaAlert(getSeaPhrase("CM_312","CM"));
		removeWaitAlert();
		return;
	}
...
function Do_HS50_3_Call_TRAININGOPTIONS_Finished()
{
	...
	if(TrainingCourses.length)
	{
		var arg = '<center><table border=0 cellspacing=0 cellpadding=0>'
		for(var i=0;i<TrainingCourses.length; i++)
		{
			arg += '<tr><td>'
			arg += '<a class="contenttextCM" href="javascript:void(0);" onClick="parent.RepaintCourses_TrainingOptionsLinks('+i+');parent.ShowSessionsPreReq('+i+',\'TrainingOptions\');return false;">'
			arg += TrainingCourses[i].Course.courseDescription + '</a>'
		}
		arg += '</table></center>'
	...
	else
	{
		...
		{
			var msg = "<center><table border=0 cellspacing=0 cellpadding=0><tr><td align=center>"
			+ '<span class="contenttextCM">'+getSeaPhrase("CM_314","CM")+"</span></table></center>";
		}
		else if (_TRAINCURRENTWIN == "TRAININGJOBGAPS")
		{
			var msg =  "<center><table border=0 cellspacing=0 cellpadding=0><tr><td align=center>"
			+ '<span class="contenttextCM">'+getSeaPhrase("CM_315","CM")+"</span></table></center>";
		}
		else // TRAINING CATEGORY
		{
			var msg =  "<center><table border=0 cellspacing=0 cellpadding=0><tr><td align=center>"
			+ '<span class="contenttextCM">'+getSeaPhrase("CM_316","CM")+"</span></table></center>";
		}
...
function SetupCourseListForQualificationTab_TrainingOptions()
{
	...
	{
		seaAlert(getSeaPhrase("CM_317","CM"))
		removeWaitAlert();
		return;
	}
	else
	{
		Do_HS50_3_Call(HS50_3_Fields,"HS50.3",false,"TRAININGQUALCRITERIA")
	}
}
...
function GetJobGapQualifications_TrainingOptions(Tab)
{
	...
	{
		case 0:
			pObj = Profile.CompetencyDetail;
			break;
		case 1:
			pObj = Profile.CertificationDetail;
			break;
		case 2:
			pObj = Profile.EducationDetail;
			break;
	}
...
{
	var Desc = '<p><center><table border=0 cellpadding=0 cellspacing=0>'
	...
	if (GapObj.length>0)
	{
		for(var i=0; i<GapObj.length; i++)
		{
			...
			Desc += '><a class="contenttextCM" href="javascript:parent.RepaintGapListLinks(\''+Tab+'\','+useDefault+','+i+');parent.Profile_TrainingOptions_OnClick('+i+','+Tab+','+useDefault+');">'
  			Desc += GapObj[i].description+'</a></td></tr>'
		}
	}
	else
	{
		Desc += '<tr><td align=center><span class="contenttextCM">'+getSeaPhrase("CM_318","CM")+'</span></td></tr>'
	}

	return Desc + '</table></center>';
}
...
function DrawInstructions_TrainingOptions(Manager)
{
	...
	if (emssObjInstance.emssObj.filterSelect)
	{
		MakeTextBox("CourseCategoryFilterSelect", 510, 325, 105, 25, "CourseCategoryFilterSelect_TrainingOptions", false, 30, "", 'styler="select" styler_click="parent.CourseCategoryFilterSelectArrow_OnClick"');
		MakeButton('<img style="cursor:hand;cursor:pointer" border="0" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" alt="'+getSeaPhrase("SELECT_VALUE","ESS")+'">', 605, 325, 12, 16,
	...
	{
		MakeSelectBox("CourseCategorySelect", 510, 325, 300, 23, "CourseCategorySelect_TrainingOptions", CourseCategories, false, _COURSECATEGORYINDEX, true);	
	}
	
	
	MakeButton(getSeaPhrase("CM_322","CM"), 510, 354, 150, 30, ButtonBorderColor, "CreateCourseList_TrainingOptions", false);

	MakeButton(getSeaPhrase("CM_122","CM"), 610, 450, 150, 30, ButtonBorderColor, "ClearForm_TrainingOptions", false);
	MakeButton(getSeaPhrase("CM_322","CM"), 450, 450, 160, 30, ButtonBorderColor, "CreateCourseList_TrainingOptions2", false);
	...
	if(Manager) {
		MakeSelectBox("DirectReports", 525, 275, 275, 30, "cmbDirectReports", DirectReports, false, _DIRECTREPORTSINDEX, true)
...
{
	if (typeof(Max) == "undefined" || typeof(setValue(Max)) == "undefined" || isNaN(parseFloat(setValue(Max)))
	|| parseFloat(setValue(Max))<=0)
		Max = 5
	if (typeof(Min) == "undefined" || typeof(setValue(Min)) == "undefined" || isNaN(parseFloat(setValue(Min))))
		Min = 0

	if ((JobProficiency.length == 0 && EmployeeProficiency.length == 0) ||
		(NonSpace(JobProficiency.join("")) == 0 && NonSpace(EmployeeProficiency.join("")) == 0))
		return ''
	...
	Desc = '<applet id="cmChartApplet" alt="Proficiency Applet" codebase="/lawson/java"'
		+ ' archive="jchart.jar" code="lawson.jchart.ChartApplet.class" width="'+Width+'" height="'+Height+'">'
...
//	}
	...
	else Desc += '<param name="valueURL" value="/lawson/xhrnet/careermanagement/lib/chartdetail.htm">'
	...
	var accessibleApplet = '<table><CAPTION>Graph Showing Skills</CAPTION><tr><th>Description</th><th>Employee Proficiency</th><th>Job Proficiency</th></tr>';
	for(var i=JobLabels.length-1; i>=0; i--) {
		Desc += (i == JobLabels.length-1) ? JobLabels[i] : ','+JobLabels[i];
		accessibleApplet += '<tr><td>'+JobLabels[i]+'</td><td>'+EmployeeProficiency[i]+'</td><td>'+JobProficiency[i]+'</td></tr>';
	}
	accessibleApplet += '</table>'
	...
	Desc += accessibleApplet	
	Desc += '</applet>'	
	Desc += '<br /><a style="color:#ffffff;cursor:default;font-size:1px;" href="\#" onclick="openAccessibleAppletWindow(\''+accessibleApplet.replace(/\'/g,"\\\\'")+'\');">'+getSeaPhrase("ACC_PRO","ESS")+'</a>'	
