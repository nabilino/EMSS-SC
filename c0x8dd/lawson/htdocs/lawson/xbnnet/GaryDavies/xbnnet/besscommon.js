...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xbnnet/besscommon.js,v 1.32.2.46.2.1 2012/03/12 06:12:48 juanms Exp $
...
function determineCoverage(recType)
{
	...
	if(SelectedPlan[6]!='' && SelectedPlan[8]!='' && SelectedPlan[6]!=null && SelectedPlan[8]!=null && typeof(SelectedPlan[6])!='undefined' && typeof(SelectedPlan[8])!='undefined')
	{
		...
		for(var i=0;i<ElectedPlans.length;i++)
		{
			...
			{
				if ((rule_type != "A" && ElectedPlans[i][5] >= SelectedPlan[3]) ||
    				ElectedPlans[i][5] == BenefitRules[2])
				{
					found=true
					flg=i
					break
		...
		if(!found)
		{
			...
			if(found)
				seaAlert(getSeaPhrase("CONBEN_7","BEN")+
				      " '"+EligPlans[i][4]+"'\n"+
				      getSeaPhrase("CONBEN_8","BEN")+
				      " '"+SelectedPlan[2]+"'")
			else
			{
				removeWaitAlert();
				...
				var alertmsg = getSeaPhrase("CONBEN_9","BEN") +
				               " '" + SelectedPlan[2] + "'\n" +
		...
		else
		{
				 ...
				 ((recType==3 || recType==13 || recType==4) && ( (parseFloat(SelectedPlan[53])/100) > parseFloat(newCov)) ) )
			{
				...
				self.document.getElementById("main").src = "/lawson/xbnnet/availplans.htm"
				seaAlert(getSeaPhrase("CONBEN_12","BEN") +
				      " " +
				      ElectedPlans[flg][2][2])
			}
		}
	}
	else
...
function coverageLimit(recType)
{
	...
	if(SelectedPlan[6]!='' && SelectedPlan[8]!='' && SelectedPlan[6]!=null && SelectedPlan[8]!=null && typeof(SelectedPlan[6])!='undefined' && typeof(SelectedPlan[8])!='undefined')
	{
		...
		if(!found)
		{
			...
			if(found)
				covObj.msg=getSeaPhrase("CONBEN_15","BEN") +
				           " " + 
				           EligPlans[i][4] +
				           " " +
				           getSeaPhrase("CONBEN_16","BEN")
			else
			{
				covObj.msg=getSeaPhrase("CONBEN_9","BEN") +
				           " '" + 
				           SelectedPlan[2] +
					   "'\n" +
					   getSeaPhrase("CONBEN_10","BEN") 
...
//	var wnd=window.open(temp,"DescWindow"," toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=400")
	var wnd=window.open(temp,"DescWindow"," toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=600,height=400")
// END OF MOD
	wnd.focus()
}
...
function CloseLifeEnrollment()
{
	...
	try 
	{
		parent.left.setImageVisibility("benefits_checkmark", "visible");
	}
	catch(e) {}
}
function EndEnroll(flag, noNavigation)
{
	...
	if (flag.toUpperCase() == "YES") 
	{
		...
		else 
		{
			...
			{
				showWaitAlert(getSeaPhrase("EXITING", "ESS"));
	...
	if(status!="C")	{
		if(choices.length>currentChoice+1) {
			...
			if(CurrentPlanGroup<EligPlanGroups.length-1) {
				for(i=1;i<CurrentBens.length;i++) {
...
//PT 161506
// Have all other plans other than the current one in this group already been elected?
// Note: it is assumed that CurrentBens[i][45] = true (meaning it has been marked for election) 
// for the current plan prior to calling this function.
...
function writeDepPortion(dte)
{
	var deps=''
	var NbrCoveredDeps=0
// MOD BY BILAL 
//	deps+='<table class="plaintableborder" border="0" cellpadding="0" cellspacing="0" width="80%" styler="list">'
	deps+='<table class="plaintableborder" border="0" cellpadding="0" cellspacing="0" width="60%" styler="list">'
	deps+='<tr>'
//	deps+='<th class="plaintableheaderborder" style="text-align:center" colspan="2">'
	deps+='<th class="plaintableheaderborder" style="text-align:center" colspan="2" nowrap>'
// END OF MOD
	deps+=getSeaPhrase("COVERED_DEPS_AS_OF","BEN")+' '
	if (typeof(dte)!="undefined" && dte!=null)
		deps+=dte
	else 
		deps+=newdate
	...
	for(var i=0; i<dependents.length; i++) {
		for(var x=0; x<coveredDeps.length; x++) {
			if(dependents[i].seq_nbr==coveredDeps[x].dependent) {
				...
				deps+='<td class="plaintablecellborder" style="width:50%">'
				deps+=dependents[i].label_name_1+'&nbsp;'
				deps+='</td>'
				NbrCoveredDeps++
				if (NbrCoveredDeps % 2 == 0) {
					deps+='</tr>'
	...
	if (NbrCoveredDeps % 2 != 0) {
		deps+='<td class="plaintablecellborder" style="width:50%">&nbsp;</td></tr>'
	}
	deps+='</table>'
	if(NbrCoveredDeps>0)
		return deps
...
function getSplitCosts(arr,REC_TYPE,PLAN_TYPE,isflex,showDate,preTaxVal,aftTaxVal)
{
	var str="";
	if(preTaxVal==0 && aftTaxVal!=0)
		str+=getCosts(arr,REC_TYPE,PLAN_TYPE,isflex,true,"A",false)
	else if(preTaxVal!=0 && aftTaxVal==0)
		str+=getCosts(arr,REC_TYPE,PLAN_TYPE,isflex,true,"P",false)
	else
	{
		str+=getCosts(arr,REC_TYPE,PLAN_TYPE,isflex,true,"P",false)+'</tr>'
		str+='<tr><td class="plaintablecellborder" nowrap>&nbsp;</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;</td>'
		if(showDate)
			str+='<td class="plaintablecellborder">&nbsp;</td>'				
		str+=getCosts(arr,REC_TYPE,PLAN_TYPE,isflex,true,"A",true)
	}
	return str;
}
var contType
var noCosts=false
function getCosts(arr,REC_TYPE,PLAN_TYPE,isflex,issplit,preaft,splitline)
{			
		var bod=""	
		noCosts=false
		empprecost=0
		...
		if(REC_TYPE=="01" || parseFloat(REC_TYPE)==1)
		{
			flexcost=arr[6]
			empcost=arr[5]
		...
		if(REC_TYPE=="02" || parseFloat(REC_TYPE)==2 || REC_TYPE=="03" || parseFloat(REC_TYPE)==3 || REC_TYPE=="13" || parseFloat(REC_TYPE)==13 || REC_TYPE=="04" || parseFloat(REC_TYPE)==4 || REC_TYPE=="05" || parseFloat(REC_TYPE)==5)
		{
			contType=arr[51]
			flexcost=arr[20]
			empcost=arr[18]
		...
		if(REC_TYPE=="06" || parseFloat(REC_TYPE)==6)
		{
			taxable=arr[26]
			flexcost=arr[18]
			empcost=arr[16]
			...
			if(arr[24]!="P" && !(arr[19] && arr[19]==-1))
			{
				...
				{
					if(arr[17] && arr[17]==-1)
						empcost=parseFloat(arr[37])
					else
						empcost=parseFloat(arr[23])/parseFloat(arr[37])
				}
				//else empcost=parseFloat(arr[23])/parseFloat(arr[37])
			}
		...
		if(REC_TYPE=="07" || parseFloat(REC_TYPE)==7)
		{
			taxable=arr[26]
			flexcost=arr[18]
			empcost=arr[16]
		...
		if(REC_TYPE=="08" || parseFloat(REC_TYPE)==8 || REC_TYPE=="09" || parseFloat(REC_TYPE)==9 || REC_TYPE=="10" || parseFloat(REC_TYPE)==10)
		{
			taxable=arr[26]
			flexcost=arr[18]
			empcost=arr[16]
		...
		if(REC_TYPE=="12" || parseFloat(REC_TYPE)==12)
		{
			taxable=""
			flexcost=""
			empcost=""
...
/*
		...
		{
			bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(flexcost)
			if(contType=="P")
...
// END OF MOD
		...
		bod+='<td class="plaintablecellborder" width="'+TableSpacer+'" nowrap>&nbsp;</td>'
 		if(REC_TYPE=="08" || parseFloat(REC_TYPE)==8 || REC_TYPE=="09" || parseFloat(REC_TYPE)==9)
 		{
			...
			if(arr[24]!="P")
			{
				if(arr[23]!="" && arr[36]!="")
				{
					if(parseFloat(type89flg)==0)
					{
						...
						{
							bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont((parseFloat(arr[23])/parseFloat(divisor)))+'&nbsp;</td><td class="plaintablecellborder" nowrap>&nbsp;'+getSeaPhrase("PRE_TAX","BEN")+'&nbsp;</td>'
					...
					else
					{
						...
						{
							bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont((parseFloat(arr[36])/parseFloat(divisor)))+'&nbsp;</td><td class="plaintablecellborder" nowrap>&nbsp;'+getSeaPhrase("AFTER_TAX","BEN")+'&nbsp;</td>'
				...
				else
				{
					if(arr[23] && arr[23]!="" && parseFloat(arr[23])!=0)
					{
						...
						bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(cov)+'&nbsp;</td><td class="plaintablecellborder" nowrap>&nbsp;'+getSeaPhrase("PRE_TAX","BEN")+'&nbsp;</td>'
						ThisCost = unFormat(formatCont(parseFloat(arr[23])/parseFloat(divisor)))
						Ycost+=ThisCost
						if(ThisCost<0)
							EmpNegPreTaxTotal+=Math.abs(ThisCost)
			...
			else if(arr[24]!="A")
			{
				if(arr[23] && arr[23]!="" && parseFloat(arr[23])!=0)
				{
					bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont((parseFloat(arr[23])/parseFloat(divisor)))+getSeaPhrase("PER","BEN")+'&nbsp;</td><td class="plaintablecellborder" nowrap>&nbsp;'+getSeaPhrase("PRE_TAX","BEN")+'&nbsp;</td>'
					ThisCost = unFormat(formatCont(parseFloat(arr[23])/parseFloat(divisor)))
					Pcost+=ThisCost
				...
				if(arr[36] && arr[36]!="" && parseFloat(arr[36])!=0)
				{
					bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont((parseFloat(arr[36])/parseFloat(divisor)))+getSeaPhrase("PER","BEN")+'&nbsp;</td><td class="plaintablecellborder" nowrap>&nbsp;'+getSeaPhrase("AFTER_TAX","BEN")+'&nbsp;</td>'
					ThisCost = unFormat(formatCont(parseFloat(arr[36])/parseFloat(divisor)))
					Pcost+=ThisCost
		...
		else
		{
			...
			if(issplit)
			{
				...
				if (preaft == "P")
				{
					if (empprecost!=0 && empprecost!="" && empprecost != null && typeof(empprecost)!='undefined' && !isNaN(empprecost))
					{
						...
						bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(empprecost)+'&nbsp;</td><td class="plaintablecellborder" nowrap>&nbsp;'+getSeaPhrase("PRE_TAX","BEN")+'&nbsp;</td>'
						if(ThisCost<0)
							EmpNegPreTaxTotal += Math.abs(ThisCost)
						else 
						{
						...
						bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;</td><td class="plaintablecellborder" nowrap>&nbsp;</td>'
				}
				else
				{
					...
					{		
						...
						bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(empaftcost)+'&nbsp;</td><td class="plaintablecellborder" nowrap>&nbsp;'+getSeaPhrase("AFTER_TAX","BEN")+'&nbsp;</td>'
						EmpAfterTaxTotal += ThisCost	
					}
					else
						bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;</td><td class="plaintablecellborder" nowrap>&nbsp;</td>'
			...
			{
				bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(empcost)
		...
		if (!issplit)
		{
				...
				typeof(empcost)!='undefined' && !isNaN(empcost)))
			{
				...
				if(taxable!="" && (REC_TYPE!="08" && parseFloat(REC_TYPE)!=8 && REC_TYPE!="09" && parseFloat(REC_TYPE)!=9))
...
function removeStoppedElections()
{
	...
	removeUserStoppedPlans()
}
function removeUserStoppedPlans()
{
	var cleanElections = new Array()
	var cleanDependents = new Array()
	var cleanOldDependents = new Array()
	...
	for (var j=0;j<ElectedPlans.length;j++)
	{
		...
		else if(parseInt(ElectedPlans[j][0],10)==12)
		{
			...
			continue
		for (var c=1;c<CurrentBens.length;c++)
		{
			...
			{
				if (ElectedPlans[j][5] == BenefitRules[2])
				{
					delete CurrentBens[c][0]
				}
				else
				{
...
function getCoverage(arr,REC_TYPE,PLAN_TYPE,planDate,CHANGED,isflex)
{		
	var bod=""
	noCosts=false
	if(arr[2] && arr[2].toString().split('*').length)
		arr[2] = arr[2].toString().split('*').join('+');
	var issplit = isEmpCostSplit(arr,REC_TYPE)
	...
	if(REC_TYPE=="01" || parseFloat(REC_TYPE)==1)
	{
		...
		if(CHANGED)
			bod+="* "
		bod+=PLAN_TYPE+'&nbsp;</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'+arr[2]+'&nbsp;</td>'
		if(showDate)
			bod+='<td class="plaintablecellborder">&nbsp;'+planDate+'&nbsp;</td>'
		if (!issplit)
			bod+=getCosts(arr,REC_TYPE,PLAN_TYPE,isflex,issplit,"P",false)+'</tr>'
		else
			bod+=getSplitCosts(arr,REC_TYPE,PLAN_TYPE,isflex,showDate,preTaxVal,aftTaxVal)+'</tr>'
	}
	if(REC_TYPE=="02" || parseFloat(REC_TYPE)==2)
	{
		bod+='<tr><td class="plaintablecellborder" nowrap>&nbsp;'
		if(CHANGED)
			bod+="* "
		...
		if(arr[4]=="E")
		{
			...
			bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td>'			
			bod+='</tr><tr><td class="plaintablecellborder" align="right" nowrap>'+getSeaPhrase("EMPLOYEE","BEN")+'</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(arr[14])+'&nbsp;</td>'
			if(showDate)
				bod+='<td class="plaintablecellborder">&nbsp;'+planDate+'&nbsp;</td>'
		}
		else if(arr[4]=="S")
		{
			bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td>'
			bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td>'			
			bod+='<tr><td class="plaintablecellborder" align="right" nowrap>'+getSeaPhrase("SPOUSE","BEN")+'</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(arr[14])+'&nbsp;</td>'
			if(showDate)
				bod+='<td class="plaintablecellborder">&nbsp;'+planDate+'&nbsp;</td>'
		}
		else if(arr[4]=="D")
		{
			bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td>'
			bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td>'			
			bod+='</tr><tr><td class="plaintablecellborder" align="right" nowrap>'+getSeaPhrase("DEPENDENT","BEN")+'</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(arr[14])+'&nbsp;</td>'
			if(showDate)
				bod+='<td class="plaintablecellborder">&nbsp;'+planDate+'&nbsp;</td>'
		}
		else if(arr[4]=="B")
		{
			bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td>'
			bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td>'			
			bod+='</tr><tr><td class="plaintablecellborder" align="right" nowrap>'+getSeaPhrase("DEPENDENT","BEN")+'</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(arr[7])+'&nbsp;</td>'
			bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td>'
			bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td>'				
			bod+='</tr><tr><td class="plaintablecellborder" align="right" nowrap>'+getSeaPhrase("SPOUSE","BEN")+'</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(arr[14])+'&nbsp;</td>'
			if(showDate)
				bod+='<td class="plaintablecellborder">&nbsp;'+planDate+'&nbsp;</td>'				
		}
		else
		{
			bod+='<td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(arr[14])+'&nbsp;</td>'
			if(showDate)
				bod+='<td class="plaintablecellborder">&nbsp;'+planDate+'&nbsp;</td>'			
		}	
		if (!issplit)
	...
	if(REC_TYPE=="03" || parseFloat(REC_TYPE)==3 || REC_TYPE=="13" || parseFloat(REC_TYPE)==13)
	{
		bod+='<tr><td class="plaintablecellborder" nowrap>&nbsp;'
		if(CHANGED)
			bod+="* "
		bod+=PLAN_TYPE+'&nbsp;</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'
		if(arr[11] && arr[11]!="")
	...
	if(REC_TYPE=="04" || parseFloat(REC_TYPE)==4)
	{
		bod+='<tr><td class="plaintablecellborder" nowrap>&nbsp;'
		if(CHANGED)
			bod+="* "
		bod+=PLAN_TYPE+'&nbsp;</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(arr[17])+'&nbsp;</td>'
		if(showDate)
			bod+='<td class="plaintablecellborder">&nbsp;'+planDate+'&nbsp;</td>'
		if (!issplit)
			bod+=getCosts(arr,REC_TYPE,PLAN_TYPE,isflex,issplit,"P",false)+'</tr>'
	...
	if(REC_TYPE=="05" || parseFloat(REC_TYPE)==5)
	{
		bod+='<tr><td class="plaintablecellborder" nowrap>&nbsp;'
		if(CHANGED)
			bod+="* "
		bod+=PLAN_TYPE+'&nbsp;</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'
		if(arr[12])
			bod+=arr[12]+getSeaPhrase("PER","BEN")+'&nbsp;'
		bod+='</td>'	
		if(showDate)
			bod+='<td class="plaintablecellborder">&nbsp;'+planDate+'&nbsp;</td>'
		if (!issplit)
	...
	if(REC_TYPE=="07" || parseFloat(REC_TYPE)==7)
	{
		...
		if(CHANGED)
			bod+="* "
		bod+=PLAN_TYPE+'&nbsp;</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'+arr[23]+' '+getSeaPhrase("HOURS","BEN")+'&nbsp;</td>'
		if(showDate)
			bod+='<td class="plaintablecellborder">&nbsp;'+planDate+'&nbsp;</td>'
		if (!issplit)
			bod+=getCosts(arr,REC_TYPE,PLAN_TYPE,isflex,issplit,"P",false)+'</tr>'
	...
	if(REC_TYPE=="08" || parseFloat(REC_TYPE)==8 || REC_TYPE=="09" || parseFloat(REC_TYPE)==9)
	{
		...
		if(arr[23] && arr[23]!="" && parseFloat(arr[23])!=0)
		{
			...
			if(!isNaN(parseFloat(arr[23])))
			{
				...
				else
				{
					...
					bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(arr[23])
		...
		if(arr[36] && arr[36]!="" && parseFloat(arr[36])!=0)
		{
			...
			if(!isNaN(arr[36]))
			{
				if(type89flg==0)
				{
					bod+='<tr><td class="plaintablecellborder" nowrap>&nbsp;</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;</td>'
				}
				else
				{
				    if(arr[41])
				...
				else
				{
					...
					// Flex, Spacer, and Emp columns
					bod+='<td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder">&nbsp;</td><td class="plaintablecellborder" align="right" nowrap>&nbsp;'+formatCont(arr[36])
	...
	if(REC_TYPE=="10" || parseFloat(REC_TYPE)==10)
	{
		bod+='<tr><td class="plaintablecellborder" nowrap>&nbsp;'
		if(CHANGED)
			bod+="* "
		bod+=PLAN_TYPE+'&nbsp;</td><td class="plaintablecellborder">&nbsp;</td>'
		if(showDate)
			bod+='<td class="plaintablecellborder">&nbsp;'+planDate+'&nbsp;</td>'
		if (!issplit)
			bod+=getCosts(arr,REC_TYPE,PLAN_TYPE,isflex,issplit,"P",false)+'</tr>'
...
function GetSummaryTable()
{
	...
	var table = '<table class="plaintableborder" border="0" cellspacing="0" cellpadding="0" width="50%" styler="list">'
	table += '<tr>'
	table += '<th class="plaintableheaderborder" style="text-align:right;width:60%">'+cost_div_desc+'</th>'
	table += '<th class="plaintableheaderborder" style="text-align:right;width:40%">'+getSeaPhrase("COST","BEN")+'</th>'
	table += '</tr>'
...
// MOD BY BILAL
	...
	if (EmpFlexTotal != 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_4","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+(formatCont(EmpFlexTotal)?formatCont(EmpFlexTotal):'&nbsp;')+'</td>'
		table += '</tr>'
	}
	if (BenCreditTotal != 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_5","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+(formatCont(BenCreditTotal)?formatCont(BenCreditTotal):'&nbsp;')+'</td>'
		table += '</tr>'
	}
	if (FlexCreditTotal != 0 && EmpFlexTotal != 0 && BenCreditTotal != 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_6","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+(formatCont(FlexCreditTotal)?formatCont(FlexCreditTotal):'&nbsp;')+'</td>'
		table += '</tr>'
	}
	if (FlexCreditTotal != 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">*** '+getSeaPhrase("CURBEN_7","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">&nbsp;</td>'
		table += '</tr>'
	}
	if (EmpPreTaxTotal != 0 && FlexCreditTotal != 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_8","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+(formatCont(EmpPreTaxTotal)?formatCont(EmpPreTaxTotal):'&nbsp;')+'</td>'
		table += '</tr>'
	}
    ISH 2007 End */
// END OF MOD
	...
	{
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_9","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'
...
// MOD BY BILAL
	...
	if (CreditsLessPreTax == 0 && EmpPreTaxFlexTotal != 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_10","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">&nbsp;</td>'
		table += '</tr>'
	}
	if (CreditsLessPreTax > 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_11","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+(formatCont(CreditsLessPreTax)?formatCont(CreditsLessPreTax):'&nbsp;')+'</td>'
		table += '</tr>'
	}
	if (CreditsLessPreTax > 0 && AddToGrossPercent == 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_12","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">&nbsp;</td>'
		table += '</tr>'
	}
	if (ForfeitedDifference	!= 0 && AddToGrossPercent != 0)	{
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'
		table += formatCont(AddToGrossPercent*100)+getSeaPhrase("CURBEN_13","BEN")
		table += '<br>'
		table += getSeaPhrase("CURBEN_15","BEN")+' '+formatCont(ForfeitedDifference)
		table += '</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">&nbsp;</td>'
		table += '</tr>'
	}
...
// END Of MOD
	if (AddedToPay != 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_16","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+(formatCont(AddedToPay)?formatCont(AddedToPay):'&nbsp;')+'</td>'
		table += '</tr>'
	}
	table += '<tr>'
	table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_17","BEN")+'</td>'
	table += '<td class="plaintablecellborder" style="text-align:right">'
	...
	if (EmpPreTaxPercentTotal > 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_18","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+(formatCont(EmpPreTaxPercentTotal)?formatCont(EmpPreTaxPercentTotal):'&nbsp;')+'</td>'
		table += '</tr>'
	}
	if (EmpAfterTaxPercentTotal > 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_19","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+(formatCont(EmpAfterTaxPercentTotal)?formatCont(EmpAfterTaxPercentTotal):'&nbsp;')+'</td>'
		table += '</tr>'
	}
	if (EmpPreTaxPercentTotal > 0 && EmpAfterTaxPercentTotal > 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_20","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+(formatCont(TaxPercentTotal)?formatCont(TaxPercentTotal):'&nbsp;')+'</td>'
		table += '</tr>'
	}
	if (CompanyTotal != 0) {
		table += '<tr>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+getSeaPhrase("CURBEN_21","BEN")+'</td>'
		table += '<td class="plaintablecellborder" style="text-align:right">'+(formatCont(CompanyTotal)?formatCont(CompanyTotal):'&nbsp;')+'</td>'
		table += '</tr>'
	}
	table += '</table>'
	table += '<table border="0" cellspacing="0" cellpadding="0" width="50%">'
	table += '<tr>'
	table += '<td colspan="2" class="plaintablecellsmall" style="padding-top:3px;font-size:8pt;text-align:right">'+getSeaPhrase("ROUNDING_NOTE","BEN")+'</td>'
	table += '</tr>'
	table += '</table>'
	return table
}
...
function GetShowElectionsHeader(justText)
{
	...
	hdr +='<p><table class="plaintableborder" border="0" style="width:500px;margin-left:auto;margin-right:auto" cellpadding="0" cellspacing="0">'
	    + '<tr><th class="plaintablerowheaderborder" nowrap>'+getSeaPhrase("EMPNAME","BEN")
	    + '</th><td class="plaintablecellborderdisplay" style="text-align:right" nowrap>'+fullname
 	    + '</td><th class="plaintablerowheaderborder" nowrap>'+getSeaPhrase("SSN","BEN")
	    + '</th><td class="plaintablecellborderdisplay" style="text-align:right" nowrap>'+("XXX-XX-" + ficanbr.substring(ficanbr.length-4,ficanbr.length))
	    + '</td></tr><tr><th class="plaintablerowheaderborder" nowrap>'+getSeaPhrase("EMPNUM","BEN")
	    + '</th><td class="plaintablecellborderdisplay" style="text-align:right" nowrap>'+employee
	    + '</td><th class="plaintablerowheaderborder" nowrap>'+getSeaPhrase("DOB","BEN")
	    + '</th><td class="plaintablecellborderdisplay" style="text-align:right" nowrap>'+dateofbirth
	    + '</td></tr>'
// MOD BY BILAL - Prior customization
//ISH 2007 Remove Rate and Birth dateofbirth
//	if (emssObjInstance.emssObj.benPayInfo)
//	{
//		hdr += '<tr><th class="plaintablerowheaderborder" nowrap>'
//		if (salaryclass=="H")
//			hdr += getSeaPhrase("HR_RATE2","BEN")
//		else 
//			hdr += getSeaPhrase("ANNUAL_SAL","BEN")
//		hdr +='</th><td class="plaintablecellborderdisplay" style="text-align:right" nowrap>'+formatCont(empsalary)		
//			+ '</td><th class="plaintablerowheaderborder" nowrap>&nbsp;</th>'
//			+ '<td class="plaintablecellborderdisplay" style="text-align:right" nowrap>&nbsp;</td></tr>'
//	}
// END OF MOD
	hdr += '</table></p></center>'
	if (!justText) 
	{
		hdr +='<p class="plaintablecell">'
		    + getSeaPhrase("COMMENT_TEXT","BEN")
		    + '</p>'
	} 
	else
		hdr += '<br>'	
...
function printScr(frm,justText,onFinal)
{
	...
	head+='<div class="plaintablecell" style="padding:10px">';
	if (printIt && !justText)
	{		
	...
	{
		head += '<div class="plaintablecellbold" style="text-align:left">' + getSeaPhrase("CONBEN_17","BEN") + "<br>"
			+ getSeaPhrase("CONBEN_18","BEN") + "<p>"
			+ "</div>";
	}
	removeStoppedElections();
	InitSummaryTotals();	
	...
	for (var _k=0; _k<ElectedPlans.length; _k++) 
	{
		...
		if (parseInt(ElectedPlans[_k][0],10) > 1 && parseInt(ElectedPlans[_k][0],10) < 6)
		{
			plan_type = ElectedPlans[_k][2][37];
			plan_code = ElectedPlans[_k][2][38];
		}
		if (parseInt(ElectedPlans[_k][0],10) > 5 && parseInt(ElectedPlans[_k][0],10) < 11)
		{
			plan_type = ElectedPlans[_k][2][38];
			plan_code = ElectedPlans[_k][2][39];
		}
		if (parseInt(ElectedPlans[_k][0],10) == 12)
		{
	...
	head += '<center>'
	+ '<table class="plaintableborder" border="0" width="'+TableWidth+'" cellspacing="0" cellpadding="0" styler="list">'
	+ '<tr>'
	...
	if (rule_type != "A")
		head += '<th class="plaintableheaderborder" style="text-align:center" nowrap scope=col>' + getSeaPhrase("START_DATE","BEN") + "</th>";
	if (flexflag == 1)
		head += '<th class="plaintablecellbold" style="text-align:center" nowrap scope=col>' + getSeaPhrase("FLEX_CR","BEN") + "</th>";
	else
		head += '<th class="plaintableheaderborder">&nbsp;</th>';
	// Add spacer column for display
	head += '<th class="plaintableheaderborder" width="' + TableSpacer + '" nowrap>&nbsp;';
	if (empflag == 1)
		head += '<th class="plaintableheaderborder" colspan=2 nowrap scope=col>' + getSeaPhrase("YOUR_COST","BEN") + "</th>";
	else
		head += '<th class="plaintableheaderborder" colspan=2 nowrap>&nbsp;</th>';
	if (compflag == 1)
		head += '<th class="plaintableheaderborder" align=right nowrap scope=col>' + getSeaPhrase("CO_COST","BEN") + "</th>";
	else
		head += '<th class="plaintableheaderborder" width="0" nowrap>&nbsp;</th>';
	head += '</tr>'
	if (!justText) 
	{
		html += head;
...
function emailScr(wnd, error)
{
	...
	if (typeof(pfServiceObj) != "undefined" && pfServiceObj != null && typeof(wnd["ProcessFlowObject"]) == "function")
	{
		if (rule_type == "F")
			setTimeout(function(){wnd.parent.parent.showWaitAlert(getSeaPhrase("SENDING_EMAIL","ESS"))}, 50);
		else	
			setTimeout(function(){wnd.parent.showWaitAlert(getSeaPhrase("SENDING_EMAIL","ESS"))}, 50);

		var techVersion = (iosHandler && iosHandler.getIOSVersionNumber() >= "9.0") ? wnd.ProcessFlowObject.TECHNOLOGY_900 : wnd.ProcessFlowObject.TECHNOLOGY_803;	
		...
		if (rule_type == "F")
			setTimeout(function(){wnd.parent.parent.removeWaitAlert()}, 2000);
		else	
			setTimeout(function(){wnd.parent.removeWaitAlert()}, 2000);
	}
}
...
function writeElectionsContent()
{
	...
	if (event=="annual" || rule_type=="A") 
		...
		headerStr = getSeaPhrase("BEN_ELECT","BEN");
	electionsFrame.document.title = headerStr;
...
function SendMail(to,msg)
{
	...
	if(rule_type=="A")
		...
		typedesc="Life Event"
	var object = new EMAILObject(to,to)
	object.subject = typedesc+" "+getSeaPhrase("ERROR_3","BEN")
	object.message = msg
	if (rule_type == "F")
		parent.parent.showWaitAlert(getSeaPhrase("ERROR_4","BEN")+" "+to)
	else
		showWaitAlert(getSeaPhrase("ERROR_4","BEN")+" "+to)
	EMAIL(object, "MAIL")
}
