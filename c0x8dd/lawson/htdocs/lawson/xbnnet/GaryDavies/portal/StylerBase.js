/* $Header: /cvs/cvs_archive/applications/webtier/webappjs/javascript/objects/StylerBase.js,v 1.60 2011/08/24 19:38:29 brentd Exp $ */
/* $NoKeywords: $ */
/* LaVersion=8-)@(#)@(201111) 09.00.01.06.00 Thu Sep  8 13:45:04 CDT 2011 */
/*-----------------------------------------------------------------------------
 ...
 *
 *  Copyright (c) 2006-2007 by Lawson Software. All Rights Reserved.
...
for (var i = scriptElements.length-1; i>=0; i--)
{
    ...
    if (foundIndex != -1)
    {
        ...
        var foundParameter =
            scriptElements[i].src.substring(foundIndex).toLowerCase();
...
if (StylerBase.TYPE == StylerBase.RSS)
{
    ...
    {
        var newBaseNamespace = window.location.pathname;
        newBaseNamespace = newBaseNamespace.
            substring(1, newBaseNamespace.indexOf("/html")) + ".";
        newBaseNamespace += StylerBase.baseNamespace;
        StylerBase.baseNamespace = newBaseNamespace;
    }
	else
	{
...
{
    if (window.location.pathname.indexOf("webappjs/Test") == -1)
    {
...
StylerBase.highlightListRow = function(obj)
{
    ...
    // find the position of this table row
    var rowFldPos = window.PositionObject.getInstance(obj);
    ...
    hoverLeft.style.height = ((rowFldPos.height) ? rowFldPos.height : 22) + "px";
    ...
    hoverRight.style.height = ((rowFldPos.height) ? rowFldPos.height : 22) + "px";
...
StylerBase.calendarControlOnClick = function(wnd, obj, evt)
{
    ...
    {
        wnd.calObj = new wnd.CalendarObject(wnd,
                                            wnd.CalendarObject.MMDDYY,
                                            "/",
                                            null);
...
StylerBase.selectComboBoxItem = function(evt, obj, open, highLight)
{
    ...
    var selectTxt = (obj.innerText != null) ?
                     obj.innerText :
                     ((obj.textContent != null) ? obj.textContent : "");
...
StylerBase.loadComboBox = function(obj)
{
    ...
    {
        var optObj = opt[j];
        var optTxt = (optObj.innerText != null) ?
                     optObj.innerText :
                     ((optObj.textContent != null) ?
                        optObj.textContent :
                        "\u00a0");
...
StylerBase.openComboBox = function(obj, wnd)
{
    ...
    else
    {
        if (comboBoxList.childNodes.length > 0)
        {
            try
            {
                ...
                var lastCbElm = comboBoxList.childNodes[0];
    ...
    else if (doc && doc.body && doc.body.clientHeight)
    {
        // IE 6 in "quirks mode"
...
StylerBase.isMouseOutsideCalendar = function(evt, calObj)
{
    ...
    if (calObj.openDirection.toLowerCase() == "up")
    {
...
StylerBase.doComboBoxOnKeyDown = function(evtArg, wnd)
{
    ...
    switch (evt.keyCode)
    {
        ...
        default:        // character input
            ...
            {
                cbItem = cbItems[i];
                itemTxt = (cbItem.innerText != null) ?
                          cbItem.innerText :
                          ((cbItem.textContent != null) ?
                            cbItem.textContent : "");
...
StylerBase.prototype.setByURL = function(wnd)
{
    ...
    var searchStringLowerCase = "";

    // Added second parameter for SMR and
    // its usage of URL forwarding (parameters lost)
    if (arguments.length >= 2)
    {
        searchStringLowerCase = arguments[1].toLowerCase();
    }
    else
    {
        searchStringLowerCase = wnd.location.search.toLowerCase();
    }
    ...
    {
        this.setReplaceCss(true,
                           getVarFromString("replacefrom",
                                            searchStringLowerCase),
                           getVarFromString("replaceto",
                                            searchStringLowerCase));
    }

    // override any configuration setting with URL parameter
    if (searchStringLowerCase.indexOf("theme=9") != -1 ||
    ...
    if (this.moreInfo)
    {
        var confirmPrompt = "Show " + ((this.showLDS) ? "LDS-9" : "Classic-8");
...
StylerBase.prototype.selectStyleSheet = function(wnd, id)
{
    ...
    var links = this.getLikeElements(wnd,
                                     "link",
                                     "rel",
    ...
    for (var i=0; i<links.length; i++)
    {
        ...
        if (this.showLDS && links[i].id == id + "LDS")
        {
//          alert("Using "+id+"LDS");
            links[i].disabled = false;
            selectedLinkElement = links[i];
        }
        if (!this.showLDS && links[i].id == id)
        {
//          alert("Using "+id);
            links[i].disabled = false;
            selectedLinkElement = links[i];
...
{
    wnd.document.getElementsByTagName("head")[0].
        appendChild(this.createCssElement(wnd, fileName));
};
...
StylerBase.prototype.processListElementColumns = function(wnd, theElement)
{
    ...
    if (rows.length - rowsNotStyled.length > 0)
    {
        ...
        if (hdrs.length > 0)
        {
            ...
            for (var k=0; k<hdrs.length; k++)
            {
                ...
                if (clickAttribute != null)
                {
                    ...
                    hdr.onclick = function()
                    {
                        ...
                        var mHdr = this.childNodes[0];
                        var contentDiv = mHdr.childNodes[0];
                        ...
                        var clickAttr = mHdr.getAttribute("styler_click");
                        if (clickAttr != null)
                        {
            ...
            {
                var isSAF = (navigator.userAgent.indexOf("Safari") >= 0);

                theElement.style.backgroundPosition = "0px " + hdrRowHeight + "px";
                if(!isSAF)
                {
...
// END OF MOD
    ...
    else if (rows.length > 0)
    {
        ...
        bCell.setAttribute("colspan", String(cells.length));
        bCell.colSpan = cells.length;
    }
    else
    {
...
//    if (theElement.className.toString().indexOf("inputBoxStyler") >= 0)
    {
        var selectState = "";
        ...
        if (selectState != theElement.getAttribute("state"))
        {
            ...
            var righSelectSpan = StylerBase.getNextSibling(theElement);
            ...
            if (righSelectSpan.className.indexOf("selectRightSideStyler") != -1)
            {
                righSelectSpan.className =
                    (theElement.getAttribute("state") == "enabled") ?
                ...
                if (!theElement.disabled)
                {
                    this.defineSelectControlOnClickStaticCall(righSelectSpan, wnd);
                }
                else
                {
                    this.clearSelectControlOnClickStaticCall(righSelectSpan, wnd);
                }
            }
...
//    if ((parNode.className.toString().indexOf("radioButtonStyler") >= 0) ||
//        (parNode.className.toString().indexOf("radioButtonActiveStyler") >= 0))
...
StylerBase.prototype.attachRadioElementEvents = function(wnd, spanElement, isDisabled)
{
    ...
    else if (spanElement.onclick == null)
    {
        spanElement.onmousedown = function()
        {
            ...
            var radioBtn = this.childNodes[0];
            StylerBase.resetRadioButtons(radioBtn.form,
...
StylerBase.prototype.attachCheckboxElementEvents = function(wnd, spanElement, isDisabled)
{
    ...
    else if (spanElement.onclick == null)
    {
        spanElement.onclick = function(evt)
        {
            ...
            // make sure this event does not bubble up to the span
...
StylerBase.prototype.processButtonElement = function(wnd, theElement)
{
    ...
    var elementType = theElement.getAttribute("styledAs");
    if (elementType != null && parseInt(elementType, 10) == StylerBase.ElementTypeButton)
//    if (theElement.className.toString().indexOf("pushButtonStyler") >= 0)
    {
        var btnState = "";
...
//            }
    ...
    var buttonTxt = (theElement.innerText != null) ?
...
StylerBase.prototype.processComboBoxElement = function(wnd, theElement)
{
    ...
    if (theElement.getAttribute("styledAs") == StylerBase.ElementTypeComboBox)
    {
        ...
        if (theElement.selectedIndex != -1)
        {
            ...
            var selectTxt = (selectElm.innerText != null) ?
                        selectElm.innerText :
                        ((selectElm.textContent != null) ?
                            selectElm.textContent : "");
            newCbDropDown.setAttribute("hilightIndex",
                                String(theElement.selectedIndex));
            StylerBase.setSelectElementValue(selectElm,
                                         cbId,
                                         theElement.selectedIndex,
                                         selectTxt);
        }
    ...
    if (theElement.selectedIndex != -1)
    {
        ...
        var selectTxt = (selectElm.innerText != null) ?
                        ...
                        ((selectElm.textContent != null) ?
                            selectElm.textContent : "");
        cbDropDown.setAttribute("hilightIndex",
                                String(theElement.selectedIndex));
        StylerBase.setSelectElementValue(selectElm,
                                         cbId,
                                         theElement.selectedIndex,
...
//-----------------------------------------------------------------------------
StylerBase.prototype.getLikeDescendants = function(theElement,
                                                   tagName,
                                                   attrName,
                                                   attrValue)
{
...
//-----------------------------------------------------------------------------
StylerBase.prototype.getLikeElements = function(wnd,
                                            tagName,
                                            attrName,
                                            attrValue)
{
...
StylerBase.prototype.addTitleBar = function(wnd, appName, msg, text)
{
    ...
    if (appName == "Smart Reconciliation")
    {
        // remove current title bar
        var t = null;
        t = wnd.document.getElementById("topHeader");
...
//-----------------------------------------------------------------------------
StylerBase.prototype.addSortArrow = function(wnd,
                                             theElement,
                                             sortDirection,
                                             clickFunctionStr)
{
    sortDirection = (sortDirection) ? sortDirection.toLowerCase() : "ascending";
    var contentDiv = theElement.childNodes[0];
    ...
    if (clickFunctionStr)
    {
        ...
        {
            var sortAttribute = this.getAttribute("styler_sort");
            var contentDiv = this.childNodes[0];
