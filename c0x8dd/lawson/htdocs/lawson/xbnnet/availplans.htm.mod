...
//*         decline FSA medical
//----------------------------------------------------------------------------------------
// MOD BY Gary Davies  01/13/15
// Additions logic for available plans for each suppl life group
// may be limited based on prior elections.  The logic is as follows:
//
//***************************************************
//*      EMPLOYEE SUPPLEMENTAL LIFE screen:
//*      ===========================
//*      if do not have ESL plan selected
//*            decline ESL
//*     
//*      DEPENDENT SUPPLEMENTAL LIFE screen:
//*      ==========================
//*      if do not SSL plan selected
//*         decline SSL
//***************************************************
...
//GDD  01/13/15  Mods for Suppl Life
if (vPlanGroup == parent.gEmpSupLifePlanGroup)
{// EMPLOYEE SUPPL LIFE
	if (parent.gHasEmpSupWaivePlan)
	{
		vPlanAry[vPlanAry.length] = parent.gEmpSupLifeWaive;
	}
	if (parent.gHasEmpSupPlan)
	{
		vPlanAry[vPlanAry.length] = parent.gEmpSupLife;
		vPlanAry[vPlanAry.length] = parent.gEmpSupLifeWaive;
	}

}

if (vPlanGroup == parent.gSpouseSupLifePlanGroup)
{// SPOUSE SUPPL LIFE
	if (parent.gHasSpouseSupWaivePlan)
	{
		vPlanAry[vPlanAry.length] = parent.gSpouseSupLifeWaive;
	}
	if (parent.gHasSpouseSupPlan)
	{
		vPlanAry[vPlanAry.length] = parent.gSpouseSupLife;
		vPlanAry[vPlanAry.length] = parent.gSpouseSupLifeWaive;
	}
}
...
// MOD BY LARRY TAYLOR
	...
	//GDD 01/13/15  Added logic for supp life
	if (vPlanGroup == parent.gEmpSupLifePlanGroup)
	{// only do this for EMPLOYEE SUPP LIFE
		parent.gHasEmpSupWaivePlan = false;
		if (parent.EligPlans[plan][2] == parent.gEmpSupLifeWaive)
		{// set global flag if employee chose waive plan
			parent.gHasEmpSupWaivePlan = true;
		}
		parent.gHasEmpSupPlan = false;
		if (parent.EligPlans[plan][2] == parent.gEmpSupLife)
		{// set global flag if employee chose plan
			parent.gHasEmpSupPlan = true;
		}
	}

	if (vPlanGroup == parent.gSpouseSupLifePlanGroup)
	{// only do this for SPOUSE SUPP LIFE
		parent.gHasSpouseSupWaivePlan = false;
		if (parent.EligPlans[plan][2] == parent.gSpouseSupLifeWaive)
		{// set global flag if employee chose waive plan
			parent.gHasSpouseSupWaivePlan = true;
		}
		parent.gHasSpouseSupPlan = false;
		if (parent.EligPlans[plan][2] == parent.gSpouseSupLife)
		{// set global flag if employee chose plan
			parent.gHasSpouseSupPlan = true;
		}
	}
	//GDD end of change
...
// END OF MOD 	
	...
	{	
		table1 += '<th scope="col" class="plaintableheaderborder" style="text-align:left">'+getSeaPhrase("PLAN","BEN")+'</th>'
		table1 += '<th scope="col" class="plaintableheaderborder" style="text-align:left">'+getSeaPhrase("AVAILBEN_1","BEN")+'</th>'
	}
	else
		table1 += '<th scope="col" class="plaintableheaderborder" style="text-align:left;width:100%">'+getSeaPhrase("PLAN","BEN")+'</th>'
	table1 += '</tr>'
...
//				var vShowRow  = false;
				...
				var vPlanType = parent.EligPlans[i][1];
				...
				//GDD  end of change	
				//GDD  01/13/15  only show waived if not selected Supp Life previously for Open Enrollement only.
				if (parent.rule_type != "N" && !parent.gHasEmpSupPlan && vPlanCode == parent.gEmpSupLife && vPlanType == "EL") vShowRow = false;
				if (parent.rule_type != "N" && !parent.gHasSpouseSupPlan && vPlanCode == parent.gSpouseSupLife && vPlanType == "DL") vShowRow = false;
				//GDD  end of change
				//GDD  01/21/15  only show waived if PILB selected for Supp Life for Open Enrollement only.
				if (parent.rule_type != "N" && vPlanCode == parent.gEmpSupLife && vPlanType == "EL" && parent.gHasPilbPlan) vShowRow = false;
				if (parent.rule_type != "N" && vPlanCode == parent.gSpouseSupLife && vPlanType == "DL" && parent.gHasPilbPlan) vShowRow = false;
				//GDD  end of change
...
//				
				...
				vRow += '</td>'
				if (parent.rule_type == "N")
					vRow += '<td class="plaintablecellborder" align="center">'+parent.FormatDte4(parent.EligPlans[i][5])+'</td>'
				if (dspbox)
					imgNbr++;
				vRow += '</tr>'	  
// MOD BY LARRY TAYLOR
...
// END OF MOD	
	table1 += '<tr>'
	table1 += '<div align="left"><td class="plaintablecellborder" style="text-align:left"><BR>' + wording + '<BR></td></div></tr>'
// END OF MOD
	table1 += '</table>'
// MOD BY BILAL
	table1 += "</center>"
