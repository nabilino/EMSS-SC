...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xbnnet/bessleload.js,v 1.14.2.37 2012/07/19 14:42:25 brentd Exp $
//***************************************************************
//*                                                             *
//*                           NOTICE                            *
//*                                                             *
...
{	
	...
	obj.field = "employee;last-name;first-name;full-name;nick-name;"
		+ "fica-nbr;pay-rate;pay-step;pay-grade;schedule;"			
...
{
	self.document.title = getSeaPhrase("LIFE_EVENT_ENROLLMENT","BEN");		
	stopProcessing();
	...
	{
		seaAlert(getSeaPhrase("NHBULLETIN_18","BEN"))
		if (window.opener) 
...
function getFamilyStatusHistory()
{
	if (LoadError) 
		return
	startProcessing(getSeaPhrase("LOOK_UP_EMP_INFO","BEN"));
	updatetype = "FSH"
...
function getdependents()
{
	...
	else
	{
		...
		obj.max = "600"
		obj.debug = false
		obj.cond = "Active" // PT 108938 - do not display inactive dependents
		DME(obj,"jsreturn")
	}
}
...
function DspEmdepend()
{
	...
	if(typeof(window.lawheader.FSHobject.Benefit_update)!='undefined' 
	&& window.lawheader.FSHobject.Benefit_update=="Y")
	{
		var html = '<div class="plaintablecell" style="padding:0px">'
		html += '<p>' + getSeaPhrase("ERROR_112","BEN") + '</p>'
		html += '</div>'
		self.main.document.getElementById("paneBody").innerHTML = html;
		//self.main.document.getElementById("paneHeader").innerHTML = getSeaPhrase("BEN_ENROLL","BEN");
		stopProcessing();		
	}
	else
		getRules("I")
...
function ProcessSmokerStatus(status)
{
	...
	agsObj.field 		= "FC=C"
		  				...
		  				+ "&XMIT-REQDED=1"
	agsObj.func = "parent.checkCurrentEnroll()";
	agsObj.debug = false;
	updatetype = "SMK";
	AGS(agsObj,"jsreturn");
}
...
function DspBnben()
{
	...
	{
		stopProcessing();	
		seaAlert(getSeaPhrase("ERROR_113","BEN"))
		if (window.opener) 
			setTimeout("window.close()",3000)
		LoadError=true
		return
...
function EligPlan(fc)
{
	...
	if(fc == "+")
		fc = "%2B"
	startProcessing(getSeaPhrase("PREPARE_ENROLL","BEN"));
	...
	{
		stopProcessing();
		seaAlert(getSeaPhrase("NHBULLETIN_19","BEN"))
		if (window.opener) 
			setTimeout("window.close()",3000)
 		LoadError=true
		return
...
function CurrentBenefits(fc)
{
	...
	{
		startProcessing(getSeaPhrase("RETRIEVE_CUR_BENS","BEN"));
		updatetype 	= "BEN"
...
function CheckEnrollDates()
{
	...
	for(var i=1;i<EligPlans.length;i++)
	{
		...
		for(var j=1;j<CurrentBens.length;j++)
		{
			if(CurrentBens[j][1] == EligPlans[i][1] && CurrentBens[j][2] == EligPlans[i][2])
			{
				currentBenIndex = j;
				// PT 147390:
				// According to BS12/BS03.2, is the change or stop flag set to yes to
				// allow changing this benefit?
	...
	{
		stopProcessing();		
		seaAlert(getSeaPhrase("NHBULLETIN_20","BEN"))
		if (window.opener) 
			setTimeout("window.close()",3000)
 		LoadError=true
		return
...
function BuildPlanGroups()
{
	...
	EligPlanGroups[0][3]	= new Array() 	// store off whether or not each "add-allowed"
	...
	// error out if any of our plan groups require eoi for every "add-allowed" plan
	// in the group
