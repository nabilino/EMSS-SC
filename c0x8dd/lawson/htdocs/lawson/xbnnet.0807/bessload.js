...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xbnnet/bessload.js,v 1.27.2.37 2012/07/26 19:20:33 brentd Exp $
//***************************************************************
//*                                                             *
//*                           NOTICE                            *
//*                                                             *
	...
	if (fc == "%2B" && TC_val == "") 
	{
		...
		{		
			...
			if (BenefitRules[2] == "" && nhRulesFromBS09)
			{
				noGo = true;
				stopProcessing();
				seaAlert(getSeaPhrase("ERROR_136","BEN"));
				if (opener) 
					setTimeout("window.close()",3000);
				LoadError = true;
				return;				
...
function ProcessSmokerStatus(status)
{
	...
	agsObj.field 		= "FC=C"
		  				...
		  				+ "&XMIT-REQDED=1"
	agsObj.func = "parent.getdependents()";
	agsObj.debug = false;
	updatetype = "SMK";
	AGS(agsObj,"jsreturn");
}
