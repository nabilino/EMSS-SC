...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xhrnet/directdeposit/jslib/Attic/directdepositlib.js,v 1.1.2.45 2012/07/18 15:27:24 brentd Exp $
//***************************************************************
//*                                                             *
//*                           NOTICE                            *
//*                                                             *
...
function DspDeposits(action,notify)
{
	...
	if (Rules.company_accts < thisCtry.openCount || thisCtry.acctAvailable < 0)
	{
		...
		try {
			...
			self.list.document.getElementById("paneBody").innerHTML = '<div class="fieldlabelboldleft" style="padding:10px">'
	...
	else
	{
		...
		{
			btnContent[btnContent.length] = uiButton(getSeaPhrase("DD_234","DD"),"parent.DspDeposits('AddDefault');return false","margin-right:5px");
		}
		else if (thisCtry.openCount > 1 && thisCtry.defaultAcctPtr != -1 && action.indexOf("Default") == -1 && action != "ReOrder")
		{
			btnContent[btnContent.length] = uiButton(getSeaPhrase("DD_234","DD"),"parent.DspDeposits('ChangeDefault');return false","margin-right:5px");
		}

		if (thisCtry.openCount > 1 && action.indexOf("Default") == -1 && action != "ReOrder"
		&& !(thisCtry.openCount == 2 && thisCtry.defaultAcctPtr != -1))
		{
			btnContent[btnContent.length] = uiButton(getSeaPhrase("DD_235","DD"),"parent.DspDeposits('ReOrder');return false","margin-right:5px");
		}
		...
		if (action.indexOf("Default") != -1 || action == "ReOrder")
		{
			...
			{
				btnContent[btnContent.length] = uiButton(getSeaPhrase("DD_109","DD"),"parent.DspDeposits('');return false","margin-right:5px");
			}
		}
...
function Notify()
{
	...
	if (Rules.notify == "Y" || Rules.notifyee == "Y")
	{
		...
		if ((parent.emssObjInstance.emssObj.emailAddressType.toString().toLowerCase() == "personal")
...
function PaintAddWindow(action,isdefault,waiveReceipt)
{
	...
	strHtml += '<tr>'
	strHtml += '<td class="plaintableheaderwhiteright" style="width:20%" nowrap><label for="desc">'
	...
	strHtml += '</td><td class="plaintableheaderwhite" style="width:70%" colspan="4" nowrap><input class="inputbox" id=desc type=text name=description maxlength=30 size=30 onfocus="this.select();" styler="search" styler_click="StylerEMSS.searchControlOnClick" onkeyup="parent.CheckBankSelection(this);return false;"'
	if(!manualBankEntry)
	...
	strHtml += '<td class="plaintablecell" style="vertical-align:top;padding-top:7px;padding-bottom:5px;white-space:nowrap" nowrap="nowrap">' + getSeaPhrase("HOME_ADDR_1","ESS") + '</td>'
	strHtml += '<td class="plaintablecell" style="vertical-align:top;padding-top:5px;padding-bottom:5px;white-space:nowrap" nowrap="nowrap">'
	strHtml += '<input class="inputbox" type="text" name="effectivedate" size="10" maxlength="10" onfocus="this.select()" onchange="parent.ValidateDate(this);" value="'+fmttoday+'">'
	strHtml += '<a href=javascript:parent.DateSelect("effectivedate")'
	strHtml += ' onmouseover="window.status=\''+getSeaPhrase("DISPLAY_CAL","ESS").replace(/\'/g,"\\'")+'\';return true"'
	strHtml += ' onmouseout="window.status=\'\';return true">' + uiCalendarIcon() + '</a>' + uiRequiredIcon() + '</td>'
	strHtml += '</tr><tr><td class="plaintablecell" colspan="6">&nbsp;</td><td class="plaintablecell" nowrap="nowrap">' + uiDateFormatSpan() + '</td>'
	strHtml += '</tr><tr>'
		
	if (Employee.work_country == "CA")
	{		
		strHtml += '<td class="plaintableheaderwhiteright" style="width:20%" nowrap><label for="check_desc">'+getSeaPhrase("DD_48","DD")+'</label></td>'
	...
	{
		strHtml += '<td class="plaintableheaderwhiteright" style="width:20%" nowrap><label for="check_desc">'+getSeaPhrase("DD_36","DD")+'</label></td>'
		strHtml += '<td class="plaintablecell" nowrap>'
		strHtml += '<input class="inputbox" id=check_desc type=text name=check_desc maxlength=8 size=8>'	
		strHtml += uiRequiredIcon()	+ '</td>'
	}	
	...
	if (Employee.work_country != "CA")
	{
		...
		strHtml += '<td class="plaintablecell" style="padding-top:5px;width:30%" nowrap>'
	...
	strHtml += '<td class="plaintablecell" colspan="2">&nbsp;</td>';
	strHtml += '</tr>'
	...
	strHtml += '<td class="plaintablecellsmall" nowrap>'
	strHtml += Employee.label_name_1+'</td>'
	strHtml += '<tr><td class="plaintablecellsmall" nowrap>'
	...
	else
	{
		...
		strHtml += '<tr><td class="plaintablecellsmall" nowrap>'+Employee.country_code
	}
	strHtml += '</table>'
	...
	if (isdefault)
	{
		strHtml += '<td class="plaintableheaderwhiteright" style="width:100%" nowrap>'
		strHtml += getSeaPhrase("DD_51","DD")+'</td><td class="plaintablecellright" style="width:10px" nowrap>100%'
		strHtml += '</td>'		
	}
	else
	{
		strHtml += '<td class="plaintableheaderwhiteright" style="width:100%">'
		strHtml += '<label for="deposit_amt">'+getSeaPhrase("DD_50","DD")+'</label></td><td class="plaintableheaderwhite" nowrap>'
		...
		strHtml += '<tr>'
		strHtml += '<td class="plaintableheaderwhiteright" style="width:100%"><label for="net_percent">'+getSeaPhrase("DD_51","DD")+'</label></td><td class="plaintablecell" style="text-align:left" nowrap>'
	...
	strHtml += '<td class="plaintablecell" colspan="2">'+getSeaPhrase("DD_172","DD") 
	...
	strHtml += '<tr><td class="plaintablecell" colspan="3">'
...
function ProcessAddAccount(action,isdefault,waiveReceipt)
{
	...
	if (CheckElement("!NonSpace(obj.effectivedate.value)",
		getSeaPhrase("EFFECTIVE_DATE","ESS"),obj,obj.effectivedate,true,parent))
			return
	
	if (CheckElement("NonSpace(obj.effectivedate.value) && formjsDate(obj.effectivedate.value) < ymdtoday",
		getSeaPhrase("DD_533","DD"),obj,obj.effectivedate,true,parent))
			return	
...
function ChangeContents(index,action)
{
	...
	// Effective Date
	if (formjsDate(Accts[index].beg_date) <= ymdtoday)
	{
		arg += '<tr><th class="plaintablerowheaderlite">'+getSeaPhrase("HOME_ADDR_1","ESS")+'<td class="plaintablecelldisplay" nowrap>'+Accts[index].beg_date
	}
	else
	{
		arg += '<tr><th class="plaintablerowheaderlite">'
		arg += getSeaPhrase("HOME_ADDR_1","ESS") + '</td>'
		arg += '<td class="plaintablecelldisplay" nowrap><input class="inputbox" type="text" name="effectivedate" size="10" maxlength="10" onfocus="this.select()" onchange="parent.ValidateDate(this);"'
		arg += ' value="'+Accts[index].beg_date+'">'
		arg += '<a href=javascript:parent.DateSelect("effectivedate")'
		arg += ' onmouseover="window.status=\''+getSeaPhrase("DISPLAY_CAL","ESS").replace(/\'/g,"\\'")+'\';return true"'
		arg += ' onmouseout="window.status=\'\';return true">'
		arg += uiCalendarIcon() + '</a><br/></td></tr>'
	}
...
function ChangeDefaultContents(index,action)
{
	...
	arg += '<tr><th class="plaintablerowheaderlite">'+getSeaPhrase("HOME_ADDR_1","ESS")+'</td><td class="plaintablecelldisplay" nowrap>'+Accts[index].beg_date	
...
function CloseDefault(new_index,old_index)
{
	...
	if (formjsDate(OldDefault.beg_date) > ymdtoday)
...
function PaintAuthWindow(index,action,msg)
{
	...
	getSeaPhrase("DD_470","DD")+'</p>'

	Contents += '<form name="auth" onsubmit="return false"><p style="text-align:center"><table border="0" cellspacing="0" cellpadding="0" style="width:auto;margin-left:auto;margin-right:auto">'
	...
	+ getSeaPhrase("DD_106","DD")+'</td></tr><tr><td class="plaintablecelldisplay">'
	+ '<input class="input" TYPE="radio" name="agree" value="N" onclick="parent.VerifyAuth('+index+',\''+action+'\',\''+msg+'\')"/>'
	+ getSeaPhrase("DD_107","DD")+'</td></tr></table></p></form>'
	+ '</div>'
...
function PaintQueryWindow(action,type)
{
	...
	strHtml += '<br/><form name="query" onsubmit="return false">'
	strHtml += table
	strHtml += '</form></body>'
...
function PrepAccount(acct)
{
	...
	{		
		// Grab the next available distribution number of least value for this new account.
