...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xhrnet/JobRequisitions/NewJobRequisitions/SHR031_DME.js,v 1.11.2.33 2012/06/29 17:24:25 brentd Exp $
//***************************************************************
//*                                                             *
//*                           NOTICE                            *
//*                                                             *
...
function SelectOption(option)
{
	switch(option)
	{
		...
		case "payschedule":
			if (emssObjInstance.emssObj.filterSelect) {
				openFieldValueList("payschedule");
			}
			else {
...
function performDmeFieldFilterOnLoad(dmeFilter)
{
	...
	{	
		...
		case "stepandgrade":
			...
			{
				selectStr = "effect-date=" + formjsDate(selectedScheduleRec.effect_date);
...
function getDmeFieldElement(fieldNm)
{
	...
	switch(fieldNm.toLowerCase())
	{
		...
		case "replacementfor":
			...
			break;
		case "payschedule":
			fld = [self.right, self.right.document.newjobreqdtlform.payschedule];			
			break;										
		default:
			break;
	}
...
function drawDmeFieldFilterContent(dmeFilter)
{
	...
	switch(fieldNm)
	{
		...
		case "grade":
			selectHtml[0] = '<table class="filterTable" border="0" cellspacing="0" cellpadding="0" width="100%;padding-left:5px;padding-right:5px" styler="list">'
			selectHtml[0] += '<tr><th style="width:50%">'+getSeaPhrase("SCHEDULE","ESS")+'</th>'
			selectHtml[0] += '<th style="width:50%">'+getSeaPhrase("JOB_OPENINGS_2","ESS")+'</th></tr>'
			for (var i=0; i<nbrDmeRecs; i++) 
			{			
				tmpObj = dmeRecs[i];
		...
		case "stepandgrade":
			...
			{
				selectHtml[0] = '<table class="filterTable" border="0" cellspacing="0" cellpadding="0" width="100%;padding-left:5px;padding-right:5px" styler="list">'
				selectHtml[0] += '<tr><th style="width:33%">'+getSeaPhrase("GRADE","ESS")+'</th>'
				selectHtml[0] += '<th style="width:33%">'+getSeaPhrase("STEP","ESS")+'</th>'
				selectHtml[0] += '<th style="width:33%">'+getSeaPhrase("PAY_RATE_2","ESS")+'</th></tr>'
				for (var i=0; i<nbrDmeRecs; i++) 
				{			
					tmpObj = dmeRecs[i];
					selectHtml[i+1] = '<tr onclick="dmeFieldRecordSelected('+i+',\''+fieldNm+'\');return false" class="filterTableRow" onmouseover="filterTableRowMouseOver(this);" onmouseout="filterTableRowMouseOut(this);">'
			...
			else
			{
				selectHtml[0] = '<table class="filterTable" border="0" cellspacing="0" cellpadding="0" width="100%;padding-left:5px;padding-right:5px" styler="list">'
				selectHtml[0] += '<tr><th style="width:33%">'+getSeaPhrase("GRADE","ESS")+'</th>'
				selectHtml[0] += '<tr><th style="width:33%">&nbsp;</th>'
				selectHtml[0] += '<th style="width:33%">&nbsp;</th></tr>'
				for (var i=0; i<nbrDmeRecs; i++) 
				{			
					tmpObj = dmeRecs[i];
					selectHtml[i+1] = '<tr onclick="dmeFieldRecordSelected('+i+',\''+fieldNm+'\');return false" class="filterTableRow" onmouseover="filterTableRowMouseOver(this);" onmouseout="filterTableRowMouseOut(this);">'
...
function drawValueListContent(dmeFilter)
{
	...
	switch(fieldNm)
	{
		case "payschedule": // payschedule		
			var tmpObj;
			selectHtml[0] = '<table class="filterTable" border="0" cellspacing="0" cellpadding="0" width="100%;padding-left:5px;padding-right:5px"  styler="list">'
			selectHtml[0] += '<tr><th style="width:100%">'+getSeaPhrase("SCHEDULE_TYPE","ESS")+'</th></tr>'
			selectHtml[1] = '<tr onclick="valueListFieldRecordSelected(0,\''+fieldNm+'\');return false" class="filterTableRow" onmouseover="filterTableRowMouseOver(this);" onmouseout="filterTableRowMouseOut(this);">'
			selectHtml[1] += '<td style="padding-left:5px" nowrap>' + getSeaPhrase("STEP_GRADE_SCHEDULE","ESS") + '</td></tr>'
			selectHtml[2] = '<tr onclick="valueListFieldRecordSelected(1,\''+fieldNm+'\');return false" class="filterTableRow" onmouseover="filterTableRowMouseOver(this);" onmouseout="filterTableRowMouseOut(this);">'
			selectHtml[2] += '<td style="padding-left:5px" nowrap>' + getSeaPhrase("GRADE_RANGE_SCHEDULE","ESS") + '</td></tr>'
			selectHtml[3] = '</table>'			
			break;
		default: break;
	}
...
function getSchedules(type)
{
	...
	showWaitAlert(getSeaPhrase("PROCESSING_WAIT","ESS"));
	DMECall("PRSAGHEAD","SGHSET2","schedule;description;indicator;effect-date",String(authUser.company)+'='+type,"","schedule","","PaySchedule",true);
}
function GotoStepAndGrade(Index)
{
	...
	DMECall("PRSAGDTL","SGDSET3","pay-step;pay-grade;pay-rate;effect-date",String(authUser.company)+"="+escape(Selects.Schedule[Index].schedule),"","",effectDate,"PayGradeStep",true);
}
