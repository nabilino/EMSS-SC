...
function Update(n)
{
//GDD  12/10/14  Force a comment if Eligible for Rehire is N or M
	if (self.main.document.getElementById("actionsSelect").value == "TERMPEND") {
	    for (var i=0; i<Data.length; i++)
	    {
		if (Data[i].Field == "Eligible for Rehire" && (Data[i].ChangeText == "N" || Data[i].ChangeText == "M") && self.main.document.getElementById("comments").value == "") {
