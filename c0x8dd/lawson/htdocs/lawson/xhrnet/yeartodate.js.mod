...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xhrnet/yeartodate.js,v 1.1.2.17.2.1 2014/07/23 20:29:57 brentd Exp $
//***************************************************************
//*                                                             *
//*                           NOTICE                            *
//*                                                             *
...
function GetYTDRecordsForYear(year, ytd_index)
{
	...
	if ((ytdCheckIdList.length > 0) && (parseInt(ytd_index,10) < ytdCheckIdList.length)) 
	{
...
function FillYTDObjectsForYear(year)
{
	...
	for (i=0; i<len; i++) 
	{
		...
		if (parseInt(pObj.payroll_year,10) != parseInt(year,10))
            continue;
		...
		pPaySumGrpObj = pCurrencyObj.StorePaySumGrp(pObj.pay_sum_grp, pObj.pay_sum_group_description);
	...
	for (i=0; i<len; i++) 
	{
		...
		pDeductionsObj = pCurrencyObj.StoreDeductions(pObj.ded_code, pObj.deduct_code_description);				      
...
function FillWageObject()
{
	...
	for (var i=0; i<self.jsreturn.NbrRecs; i++)
	{
		...
		if (emssObjInstance.emssObj.futurePayments || Number(pObj.payroll_year) < Number(thisYear))
		{
			pCountryObj = pYearObj.StoreCountry(pObj.country_code);
			pCurrencyObj = pCountryObj.StoreCurrency(pObj.currency_code, false);
			pPaySumGrpObj = pCurrencyObj.StorePaySumGrp(pObj.pay_sum_grp, pObj.pay_sum_grp_description);
			pQuarterObj = pPaySumGrpObj.StoreQuarter(pObj.quarter,
				 pObj.hours,
				 pObj.wage_amount,
                 pObj.process_level,
...
function FillDeductionObject(year)
{
	...
	for (var i=0; i<self.jsreturn.NbrRecs; i++)
	{
		...
		pCountryObj = pYearObj.FindCountry(pObj.country_code);
		pCurrencyObj = pCountryObj.StoreCurrency(pObj.currency_code, true);
		pDeductionsObj = pCurrencyObj.StoreDeductions(pObj.ded_code, pObj.deduction_code_description);
		if (pObj.country_code == "CA")
			FillCADeductions(pDeductionsObj, pObj)
		else
			FillUSDeductions(pDeductionsObj, pObj)
...
{
	if (pObj.deduction_code_calc_type == "T" && pObj.deduction_code_adjust_pay == "S" && pObj.deduction_code_tax_status == "")
	{
	...
	else if (pObj.deduction_code_adjust_pay == "S" && pObj.deduction_code_tax_status != "" && pObj.deduction_code_tax_status != 16 && pObj.deduction_code_tax_status != 17)
	{
		pDeductionsObj.StoreDeductionsQuarter(pObj.quarter, pObj.ded_amt, pObj.wage_amount, pObj.tax_wages, pObj.process_level, pObj.work_state)
		pDeductionsObj.Section = 2; //Pre Tax Deductions
	}
	else if (pObj.deduction_code_calc_type != "T" && pObj.deduction_code_adjust_pay == "S" && (pObj.deduction_code_tax_status == "" || pObj.deduction_code_tax_status == 16 || pObj.deduction_code_tax_status == 17))
	{
		pDeductionsObj.StoreDeductionsQuarter(pObj.quarter, pObj.ded_amt, pObj.wage_amount, pObj.tax_wages, pObj.process_level, pObj.work_state)
	...
	else if (pObj.deduction_code_print_flag == "Y" && pObj.deduction_code_calc_type == "T" && pObj.deduction_code_adjust_pay == "C" && pObj.deduction_code_tax_status == "")
	{
		pDeductionsObj.StoreDeductionsQuarter(pObj.quarter, pObj.ded_amt, pObj.wage_amount, pObj.tax_wages, pObj.process_level, pObj.work_state)
		pDeductionsObj.Section = 5; //Taxable Company Deductions
	}
	else if (pObj.deduction_code_print_flag == "Y" && pObj.deduction_code_calc_type != "T" && pObj.deduction_code_adjust_pay == "C")
	{
		pDeductionsObj.StoreDeductionsQuarter(pObj.quarter, pObj.ded_amt, pObj.wage_amount, pObj.tax_wages, pObj.process_level, pObj.work_state)
...
function FillCADeductions(pDeductionsObj, pObj)
{
	...
	if (AdjustPayIsCompany && (TaxStatusIsC1 || TaxStatusIsC5 || TaxStatusIsC3 || TaxStatusIsC4 || TaxStatusIsC9))
	{
		pDeductionsObj.StoreDeductionsQuarter(pObj.quarter, pObj.ded_amt, pObj.wage_amount, pObj.tax_wages, pObj.process_level, pObj.work_state)
...
function StoreQuarter(Quarter, Hours, WageAmount, ProcessLevel, WorkState)
{
	...
	{
		if (this.Quarter[i].Quarter == Number(Quarter) && this.Quarter[i].ProcessLevel == ProcessLevel && this.Quarter[i].WorkState == WorkState)
		{
