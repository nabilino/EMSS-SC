...
//	restored by ianm 2014/10/06
        	    ...
        	    //alert(relatedLinkIsAccessible("PERSONNEL ACTIONS", "XMLHREMPPACTIONS"))
...
//GDD  09/08/14  Remove Emergency Contact
                   /* 
...
function getDrill(id)
{
	...
	// ianm
	//window.clipboardData.setData('Text', drillStr);
	//drillStr = drillStr.replace('webappjs/drillTOhtml','portal/drillTOhtml_ism')
	drillStr = drillStr.replace('drillTOhtml','drillTOhtml_ism')
...
function relatedLinkIsAccessible(bkName, bkLawName)
{
	...
	if (relatedLinks[bkLawName])
   	...
   	if ((typeof(authUser) != "undefined") && authUser.OfficeObject)
   	{
		...
		if ( false && ! imdone )
		{
			var s = "", x = bkmks; for (var k in x) if (x[k].name.toLowerCase().indexOf('erson') >= 0) s += x[k].name + ":"; alert(s)
			imdone = true;
		}
		var foundBkmk = false;
