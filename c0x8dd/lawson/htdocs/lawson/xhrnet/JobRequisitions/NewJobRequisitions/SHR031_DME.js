...
function performDmeFieldFilterOnLoad(dmeFilter)
{
	...
	{	
		case "position":
			...
			//dmeFilter.addFilterField("supervisor.full-name", 30, 'Supervisor Name', false);	
			filterDmeCall(dmeFilter,
				"jsreturn",
				"paposition", 
				"posset3", 
				"process-level;department;position;description;supervisor;supervisor.employee;supervisor.full-name;", 
		...
		case "replacementfor":
			...
			filterDmeCall(dmeFilter,
				...
				"", // "pos-level-1", // 20150827 -- ianm - allow mgr to choose emp from non primary posn
				null, 
				dmeFilter.getNbrRecords(), 
				"1");		
		break;	
...
function performDmeFieldFilter(dmeFilter)
{
	switch (dmeFilter.getFieldNm().toLowerCase())
	{
		...
		filterDmeCall(dmeFilter,
			...
			"paposition", 
			"posset3", 
			"position;description;process-level;department;supervisor;supervisor.employee;supervisor.full-name", 
			String(authUser.company), 
			"active-current", 
...
function drawDmeFieldFilterContent(dmeFilter)
{
	...
	switch (fieldNm)
	{
		case "position":
			...
			selectHtml[0] += '<tr><th scope="col" style="width:15%">'+'Proc Lev' /*getSeaPhrase("PROCESS_LEVEL_CODE","ESS")*/+'</th>'
			selectHtml[0] += '<th scope="col" style="width:15%">'+'Dept' /*getSeaPhrase("DEPARTMENT_CODE","ESS")*/+'</th>'
			selectHtml[0] += '<th scope="col" style="width:15%">'+'Position' /*getSeaPhrase("JOB_PROFILE_8","ESS")*/+'</th>'
			selectHtml[0] += '<th scope="col" style="width:15%">'+'Super'+'</th>'
			...
			for (var i=0; i<nbrDmeRecs; i++) 
			{
				...
				selectHtml[i+1] += '</a></td><td style="padding-left:5px" nowrap><a href="javascript:;" //onclick="dmeFieldRecordSelected(event,'+i+',\''+fieldNm+'\');return false">'
				selectHtml[i+1] += (tmpObj.supervisor_full_name) ? tmpObj.supervisor_full_name : '&nbsp;'
				selectHtml[i+1] += '</a></td><td style="padding-left:5px" nowrap><a href="javascript:;" onclick="dmeFieldRecordSelected(event,'+i+',\''+fieldNm+'\');return false">'
