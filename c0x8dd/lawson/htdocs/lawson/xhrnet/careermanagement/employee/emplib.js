...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xhrnet/careermanagement/employee/Attic/emplib.js,v 1.1.2.44 2014/02/25 22:49:19 brentd Exp $
//***************************************************************
//*                                                             *
//*                           NOTICE                            *
//*                                                             *
...
function Do_HS50_1_Call_ACTIONPLAN_Finished()
{
	...
	if (StandardCareerProfile.CompetencyDetail.length || StandardCareerProfile.CertificationDetail.length || StandardCareerProfile.EducationDetail.length)
	{
...
function PaintJobAlert_JobAlertScreen()
{
	...
	var Desc = '<table border="0" cellpadding="0" cellspacing="0" role="presentation">';
	if (ReqsExist) 
	{
		...
		for (var i=0; i<JobAlert.length; i++) 
		{
			...
			if (JobAlert[i].link)
			{
				toolTip = JobAlert[i].description+' - '+getSeaPhrase("VIEW_DTL_FOR_JOB_4","SEA");
				Desc += '<a class="contenttextCM" href="javascript:;" onclick="parent._JOBALERTDESC=\''+JobAlert[i].description+'\';parent.OnTabClicked_MainTabs(\'Openings\');return false;" title="'+toolTip+'">'+JobAlert[i].description+'<span class="offscreen"> - '+getSeaPhrase("VIEW_DTL_FOR_JOB_4","SEA")+'</span></a>';
			}	
			else
	...
	{
		toolTip = getSeaPhrase("CM_85","CM")+' - '+getSeaPhrase("VIEW_X","SEA");
		if (JobsExist) 
		{
		...
		Desc += '<tr><td>&nbsp;</td></tr>'
		Desc += '<tr><td><a class="contenttextCM" href="javascript:;" onclick="parent.OnTabClicked_MainTabs(\'Openings\');return false;" title="'+toolTip+'">'+getSeaPhrase("CM_85","CM")+'<span class="offscreen"> - '+getSeaPhrase("VIEW_X","SEA")+'</span></a></td></tr>'
	}
	Desc += "</table>";
...
function URL(Link, LinkName, WindowName)
{
	this.Link = Link;
...
var URLArray = new Array(
	//new URL("http://www.ijob.com", "IJob", "IJOB"),
	//new URL("http://www.personalogic.aol.com/?product=careers,aolcom,aolcom", "Career Match", "CAREERMATCH")
);

function AttachmentCollected()
{
	RefreshAttachmentWindow_MyActionPlan()
	var Desc = '<table cellpadding="0" cellspacing="0" border="0" role="presentation">'
	for (var i=0; i<URLArray.length; i++)
	{
		var toolTip = URLArray[i].LinkName+' - '+getSeaPhrase("OPEN_LINK_TO","SEA")+' '+getSeaPhrase("OPENS_WIN","SEA");
		Desc += '<tr><td><a class="contenttextCM" href="javascript:;" onclick="window.open(\''+URLArray[i].Link+'\', \''+URLArray[i].WindowName+'\');return false;"'
		+ ' title="'+toolTip+'" aria-haspopup="true">'+URLArray[i].LinkName+'<span class="offscreen"> - '+getSeaPhrase("OPEN_LINK_TO","SEA")+' '+getSeaPhrase("OPENS_WIN","SEA")+'</span></a></td></tr>'
	}
	if (!URLArray.length)
...
function HS50_RePaintQualificationsAlert()
{
	...
	seaPageMessage(getSeaPhrase("CM_87","CM"), null, null, "info", null, true, getSeaPhrase("APPLICATION_ALERT","SEA"), true);
}

function AttachmentWindowDone()
{
	var nextFunc = function(){GetActionPlanAttachment("RefreshAttachmentWindow_MyActionPlan", "C", authUser.company, authUser.employee, true);};
...
function DeleteActionPlan()
{
	...
	pAttachObj.key 	= "K1="+escapeEx(authUser.company)+"&K2=" + escapeEx(authUser.employee);
	pAttachObj.opm = "D";
...
{
	var nextFunc = function() { GetActionPlanAttachment("PrintAttachment", "C", authUser.company, authUser.employee, true); };
...
function DrawQualifications_MyActionPlan()
{
	...
	createLayer("main","QualificationsHeader_MyActionPlan",515,60,265,45,true,"","","");
}
...
function DrawJobAlert_MyActionPlan()
{
	...
	createLayer("main","JobAlertHeader_MyActionPlan",515,235,265,65,true,"","","");
}
...
function RefreshNextReviewDate_MyActionPlan(date)
{
	...
	Desc += '<caption class="offscreen">'+getSeaPhrase("TCAP_1","CM")+'</caption>'
	Desc += '<tr><th scope="col" colspan="2"></th></tr>'
	Desc += '<tr><th scope="row" class="contenttextCM"><span class="contentlabelCM">'+getSeaPhrase("CM_108","CM")+'</span></th>'
...
// Flag to indicate whether there are comments. Attachment will be refreshed, thus losing the correct reference.
var commentsExist = false;

function RefreshAttachmentWindow_MyActionPlan()
{
	Attachment = self.jsreturn;
	var arg = '<div class="contenttextCM" style="width:100%;height:80%;overflow:auto" tabindex="0">';
	...
	{	
		...
		+ '<tr><td><div class="contenttextCM">'+ getSeaPhrase("CM_112","CM")+'<p/>'+getSeaPhrase("CM_113","CM")+'</div>'
...
function DrawJobOpeningsWindows()
{
	...
	if (emssObjInstance.emssObj.filterSelect)
		MakeButton(getSeaPhrase("CM_137","CM"), 10, 475, "auto", 30, ButtonBorderColor, "SubmitApplication_JobOpenings", true);
...
function PAJOBINTRS_Finished(func)
{
	for (var i=0; i<self.jsreturn.NbrRecs; i++)
	{
		...
		JobAlert[JobAlert.length] = new JobAlertObject(pDme.job_code, pDme.position, pDme.job_code_description,
			pDme.job_requis_requisition, pDme.job_requis_description, pDme.job_requis_closed_date);
	}
...
{
	MakeButton(getSeaPhrase("CM_95","CM"), 519, 225, "auto", 30, ButtonBorderColor, "updateJobAlert", true);
	if (emssObjInstance.emssObj.filterSelect)
...
function PAJOBREQ_Finished(func)
{
	...
	{
		pDme = self.jsreturn.record[i];
		JobOpeningsWindowInformation[JobOpeningsWindowInformation.length] = new JobOpeningsWindowInformationObject(pDme.description, pDme.location_description, pDme.department_name,
...
function DME_TO_JOB_CODE_FINISHED()
{
	...
	if (emssObjInstance.emssObj.filterSelect)
	{
		toolTip = dmeFieldToolTip("job_alert1");
		MakeTextBox("JobAlertFilterSelect1", 519, 150, 115, 25, "JobAlertFilterSelect1_JobOpenings", true, 30, GetJobInterestCode(0), 'styler="select" styler_click="parent.JobAlertFilterSelect1Arrow_OnClick"', getSeaPhrase("JOB_1","SEA"));	
		MakeButton('<img style="cursor:hand;cursor:pointer" border="0" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" alt="'+toolTip+'" title="'+toolTip+'">', 624, 150, 12, 16,
		...
		toolTip = dmeFieldToolTip("job_alert2");
		MakeTextBox("JobAlertFilterSelect2", 519, 173, 115, 25, "JobAlertFilterSelect2_JobOpenings", true, 30, GetJobInterestCode(1), 'styler="select" styler_click="parent.JobAlertFilterSelect2Arrow_OnClick"', getSeaPhrase("JOB_2","SEA"));
		MakeButton('<img style="cursor:hand;cursor:pointer" border="0" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" alt="'+toolTip+'" title="'+toolTip+'">', 624, 173, 12, 16,
		...
		toolTip = dmeFieldToolTip("job_alert3");
		MakeTextBox("JobAlertFilterSelect3", 519, 196, 115, 25, "JobAlertFilterSelect3_JobOpenings", true, 30, GetJobInterestCode(2), 'styler="select" styler_click="parent.JobAlertFilterSelect3Arrow_OnClick"', getSeaPhrase("JOB_3","SEA"));
		MakeButton('<img style="cursor:hand;cursor:pointer" border="0" src="/lawson/xhrnet/ui/images/ico_form_dropmenu.gif" alt="'+toolTip+'" title="'+toolTip+'">', 624, 196, 12, 16,
	...
	{
		MakeSelectBox("JobAlertSelect1", 519, 150, 210, 23, "jobalertselect1", JobSelects, true, GetJobInterestIndexes(0), "", getSeaPhrase("JOB_1","SEA"))
		MakeSelectBox("JobAlertSelect2", 519, 173, 210, 23, "jobalertselect2", JobSelects, true, GetJobInterestIndexes(1), "", getSeaPhrase("JOB_2","SEA"))
		MakeSelectBox("JobAlertSelect3", 519, 196, 210, 23, "jobalertselect3", JobSelects, true, GetJobInterestIndexes(2), "", getSeaPhrase("JOB_3","SEA"))
	}
	ResetJobInterests();
...
function RepaintJobOpeningsLinks()
{
	...
	else
	{
		...
		Desc += '<table width="100%" cellpadding="0" cellspacing="0" border="0" nowrap styler="list" summary="'+getSeaPhrase("TSUM_13","SEA")+'">'
		Desc += '<caption class="offscreen">'+getSeaPhrase("CM_85","CM")+'</caption>'
		Desc += '<tr class="contentlabelCM">'
		Desc += '<th scope="col" style="width:50px">'+getSeaPhrase("CM_133","CM")+'</th>'
		Desc += '<th scope="col" style="width:33%">'+getSeaPhrase("CM_134","CM")+'</th>'
		Desc += '<th scope="col" style="width:33%">'+getSeaPhrase("CM_135","CM")+'</th>'
		Desc += '<th scope="col" style="width:33%">'+getSeaPhrase("CM_136","CM")+'</th>'
		Desc += '</tr>'
		for (var i=0; i<JobOpeningsWindowInformation.length; i++)
		{
			var toolTip = JobOpeningsWindowInformation[i].description+' - '+getSeaPhrase("VIEW_DTL_FOR_JOB_4","SEA");
			Desc += '<tr><td nowrap><input type="checkbox" value="on" id="openings'+i+'" name="openings'+i+'">'
			Desc += '<label for="openings'+i+'">'+JobOpeningsWindowInformation[i].description+'<span class="offscreen"> '+JobOpeningsWindowInformation[i].location+' '+JobOpeningsWindowInformation[i].department+'</span></label></td>'
			Desc += '<td nowrap><a class="contenttextCM" href="javascript:;" onclick="parent.ShowJobOpeningDetail('+i+');return false;" title="'+toolTip+'">'
			Desc += JobOpeningsWindowInformation[i].description+'<span class="offscreen"> - '+getSeaPhrase("VIEW_DTL_FOR_JOB_4","SEA")+'</span></a>&nbsp;</td>'
...
function PAREQAPP_Finished(func)
{
	...
	{
		pDme = self.jsreturn.record[i];
		JobAppliedFor[JobAppliedFor.length] = new JobAppliedForObject(pDme.requisition, pDme.requisition_description, pDme.date_applied);
	}
	if (self.jsreturn.Next)
		self.jsreturn.location.replace(self.jsreturn.Next);
	else
...
{
	var jobDesc = (emssObjInstance.emssObj.filterSelect) ? self.HIDDEN.record[Index].description : JobOpeningsWindowInformation[Index].description;
	Desc = '<span class="layertitleCM">'+getSeaPhrase("CM_138","CM")+' - '+jobDesc+'</span>';
	ChangeTitle("main", "JobOpeningDetail", Desc);
	Desc += '<table border="0" cellpadding="0" cellspacing="0" summary="'+getSeaPhrase("TSUM_14","SEA",[jobDesc])+'">'
	Desc += '<caption class="offscreen">'+getSeaPhrase("TCAP_10","SEA",[jobDesc])+'</caption>'
	if (emssObjInstance.emssObj.filterSelect)
	{
		var jobArray = self.HIDDEN.record;
		Desc += '<tr><th scope="col" colspan="2"></th></tr>'	
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_140","CM")+'</th>'
		+ '<td class="contenttextCM">'+jobArray[Index].requisition+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_141","CM")+'</th>'
		+ '<td class="contenttextCM">'+jobArray[Index].process_level_name+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_142","CM")+'</th>'
		+ '<td class="contenttextCM">'+jobArray[Index].department_name+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_143","CM")+'</th>'
		+ '<td class="contenttextCM">'+jobArray[Index].openings+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_144","CM")+'</th>'
		+ '<td class="contenttextCM">'+jobArray[Index].supervisor_description+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_145","CM")+'</th>'
		+ '<td class="contenttextCM">'+jobArray[Index].work_schedule_description+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_146","CM")+'</th>'
		+ '<td class="contenttextCM">'+jobArray[Index].contact_first+' '+jobArray[Index].contact_mi+' '+jobArray[Index].contact_last+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_147","CM")+'</th>'
		+ '<td class="contenttextCM">'+jobArray[Index].wk_phone_nbr+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_148","CM")+'</th>'
		+ '<td class="contenttextCM">'+jobArray[Index].wk_phone_ext+'</td></tr>'
	}
	else
	{
		Desc += '<tr><th scope="col" colspan="2"></th></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_140","CM")+'</th>'
		+ '<td class="contenttextCM">'+JobOpeningsWindowInformation[Index].requisition+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_141","CM")+'</th>'
		+ '<td class="contenttextCM">'+JobOpeningsWindowInformation[Index].processlevel+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_142","CM")+'</th>'
		+ '<td class="contenttextCM">'+JobOpeningsWindowInformation[Index].department+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_143","CM")+'</th>'
		+ '<td class="contenttextCM">'+JobOpeningsWindowInformation[Index].openings+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_144","CM")+'</th>'
		+ '<td class="contenttextCM">'+JobOpeningsWindowInformation[Index].supervisor+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_145","CM")+'</th>'
		+ '<td class="contenttextCM">'+JobOpeningsWindowInformation[Index].workschedule+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_146","CM")+'</th>'
		+ '<td class="contenttextCM">'+JobOpeningsWindowInformation[Index].contact+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_147","CM")+'</th>'
		+ '<td class="contenttextCM">'+JobOpeningsWindowInformation[Index].phonenbr+'</td></tr>'
		+ '<tr><th scope="row" class="contentlabelCM">'+getSeaPhrase("CM_148","CM")+'</th>'
		+ '<td class="contenttextCM">'+JobOpeningsWindowInformation[Index].phoneext+'</td></tr>'
	}
	Desc += '</table>'
...
function updateJobAlert_OnClick()
{
	if (emssObjInstance.emssObj.filterSelect)
	{
		...
		&& ((NonSpace(FilterSelect2Value) == 0) && NonSpace(FilterSelect2Value_New) == 0) && (NonSpace(FilterSelect3Value) == 0) && (NonSpace(FilterSelect3Value_New) == 0))
		{
			seaAlert(getSeaPhrase("CM_149","CM"), null, null, "error");
			return;
		}
	}
	else
	...
	else
	{
		if (JobAlert1Index != 0)
		{
			HS50_Lgth = HS50_5_Codes.length;
			HS50_5_Codes[HS50_Lgth] = new Object();
			HS50_5_Codes[HS50_Lgth].jobcode = JobSelects[JobAlert1Index-1].code;
			HS50_5_Codes[HS50_Lgth].position = null;
			HS50_5_Codes[HS50_Lgth].fc = "D";
		}
		if (JobAlert2Index != 0)
		{
			HS50_Lgth = HS50_5_Codes.length;
			HS50_5_Codes[HS50_Lgth] = new Object();
			HS50_5_Codes[HS50_Lgth].jobcode = JobSelects[JobAlert2Index-1].code;
			HS50_5_Codes[HS50_Lgth].position = null;
			HS50_5_Codes[HS50_Lgth].fc = "D";
		}
		if (JobAlert3Index != 0)
		{
			HS50_Lgth = HS50_5_Codes.length;
			HS50_5_Codes[HS50_Lgth] = new Object();
			HS50_5_Codes[HS50_Lgth].jobcode = JobSelects[JobAlert3Index-1].code;
			HS50_5_Codes[HS50_Lgth].position = null;
			HS50_5_Codes[HS50_Lgth].fc = "D";
		}
	}
...
function Do_HS50_5_Call_JOBALERT_DELETE_Finished(skipErrorCheck)
{
	...
	{
		ResetJobInterests()
		var msg = (HS50_5_Codes.length == 1) ? getSeaPhrase("CM_150","CM") : getSeaPhrase("CM_151","CM");
		removeWaitAlert();
...
function SubmitApplication_JobOpenings_OnClick(checkForError, n)
{
	var nextFunc = function()
	{
		...
		if (emssObjInstance.emssObj.filterSelect)
		{
			...
			// if an AGS call to PA43 has already occurred, check for an error, otherwise assemble the list of jobs to submit.
			if (checkForError)
			{
		...
		else
		{
			...
			{	
				selectedJobs = new Array();
				var len = JobOpeningsWindowInformation.length;
				for (var i=0; i<len; i++)
				{
...
function displayPA43Error(n)
{
	...
	switch (msgNbr)
	{
		case 108:
		case 123:
			errorMsg += "\n\n" + getSeaPhrase("WEB_APPLICANT_STATUS_ERROR", "ESS"); break;
		case 107:
		case 124:
			errorMsg += "\n\n" + getSeaPhrase("WEB_HIRE_SOURCE_ERROR", "ESS"); break;
		default: break;
	}
...
function ApplicationSubmitted()
{
	...
	{
		case 0: 
			seaAlert(getSeaPhrase("CM_152","CM"), null, null, "error");
			break;
		case 1:
		default:
			var refreshData = function()
			{
				showWaitAlert(getSeaPhrase("PROCESSING_WAIT","ESS"), function(){PAREQAPP_Call("RefreshJobsAppliedFor()");});
			}
			var alertResponse = seaPageMessage(getSeaPhrase("CM_153","CM"), "", "info", null, refreshData, true, getSeaPhrase("APPLICATION_ALERT","SEA"), true);
...
function DrawJobTitleTab_EmployeeExploreCareers()
{
	...
	if (emssObjInstance.emssObj.filterSelect)
	{
		var toolTip = dmeFieldToolTip("job_title");
		MakeTextBox("JobTitleFilterSelect", 500, 275, 105, 25, "JobTitleFilterSelect_ExploreCareers", true, 30, "", 'styler="select" styler_click="parent.JobTitleFilterSelectArrow_OnClick"', getSeaPhrase("CM_114","CM"));
		...
		MakeSelectBox("JobTitleSelect", 500, 275, 300, 23, "jobtitleselect", JobCode, true, _JOBCODEINDEX, true, getSeaPhrase("CM_23","CM"));	
	MakeButton(getSeaPhrase("CM_117","CM"), 500, 304, "auto", 30, ButtonBorderColor, "createprofile_EmployeeExploreCareers", true);
}
...
function DrawCategoryTab_EmployeeExploreCareers()
{
	...
	if (emssObjInstance.emssObj.filterSelect)
	{
		var toolTip = dmeFieldToolTip("job_category");
		...
		MakeSelectBox("CategorySelect", 500, 275, 300, 23, "categoryselect", JobClass, true, _JOBCATEGORYINDEX, true, getSeaPhrase("CM_28","CM"));
	MakeButton(getSeaPhrase("CM_119","CM"), 500, 304, "auto", 30, ButtonBorderColor, "createjoblist_EmployeeExploreCareers", true);
...
function DrawQualCriteriaTab_EmployeeExploreCareers()
{
	...
	pObj.FontStyle = 'color:'+TitleBarFgColor+';font-weight:bold;font-family:arial;font-size:'+TitleBarFontSize+';'	
	pObj.Draw();
	var toolTip = getSeaPhrase("CM_120","CM"); 
	Desc = '<center><a class="contenttextCM" href="javascript:parent.Do_HS50_1_Call_DEFAULTPROFILE_Finished(true)" title="'+toolTip+'">'
	+ toolTip+'</a></center><br/><span class="contenttextCM">'+getSeaPhrase("CM_121","CM")+'</span><p>'
	...
	MakeButton(getSeaPhrase("CM_122","CM"), 610, 450, "auto", 30, ButtonBorderColor, "ClearForm_ExploreCareers", true);	
	MakeButton(getSeaPhrase("CM_123","CM"), 415, 450, "auto", 30, ButtonBorderColor, "qualcriteria_createjoblist_EmployeeExploreCareers", true);	
...
function createprofile_EmployeeExploreCareers_OnClick()
{
	...
	{
		seaAlert(getSeaPhrase("CM_124","CM"), null, null, "error");
		return;
	}
	_EXPLOREPROFILETABSELECTED = 0;
	showWaitAlert(getSeaPhrase("CM_155","CM"), function(){Do_HS50_Call(2,1," ",SelectedJobCode,"HS50.1","EXPLORECAREERS", authUser.employee);});
}
...
function Do_HS50_1_Call_EXPLORECAREERS_Finished()
{
	...
	if (CareerProfile.jobCompFlag != "Y" && CareerProfile.empCompFlag != "Y")
		seaPageMessage(getSeaPhrase("CM_125","CM"), null, null, "info", null, true, getSeaPhrase("APPLICATION_ALERT","SEA"), true);
	else if (CareerProfile.jobCompFlag != "Y")
		seaPageMessage(getSeaPhrase("CM_126","CM"), null, null, "info", null, true, getSeaPhrase("APPLICATION_ALERT","SEA"), true);
	else if (CareerProfile.empCompFlag != "Y")
		seaPageMessage(getSeaPhrase("CM_127","CM"), null, null, "info", null, true, getSeaPhrase("APPLICATION_ALERT","SEA"), true);	
}
...
function qualcriteria_createjoblist_EmployeeExploreCareers_OnClick()
{
	...
	showWaitAlert(getSeaPhrase("CM_155","CM"), function(){Do_HS51_Call(HS51Fields,authUser.employee,"HS51.1");});
}
