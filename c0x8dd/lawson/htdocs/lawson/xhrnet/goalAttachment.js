...
// $Header: /cvs/cvs_archive/applications/webtier/shr/src/xhrnet/Attic/goalAttachment.js,v 1.1.2.32 2014/02/17 16:30:21 brentd Exp $
//***************************************************************
//*                                                             *
//*                           NOTICE                            *
//*                                                             *
...
function writeAttachment(cmtObj, prodline, company, employee, seqNbr, action, frameStr)
{
	...
	attachObj.debug = false;
	actionGlobal = action;
	// may not need to refresh attachment on a write
...
function updateDetailedDesc(prodline, company, employee, seqNbr, funcIncoming, userType)
{
	...
	if (Number(seqNbr) == 0)
		seqNbr = self.lawheader.currentSeqNbr;
	if (funcIncoming)
		func1 = funcIncoming;
	var goalform = right.document.forms["goalform"];
...
function refreshFirstAttachment()
{
	...
	{
		var lookingForP = "P";
		for (var t=0; t<attachFrame.CmtRec.length; t++) 
		{
