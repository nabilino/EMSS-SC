/* $Header: /cvs/cvs_archive/LawsonPlatform/ui/portal/pages/graph/graph.js,v 1.2.4.1.18.1.2.2 2012/08/08 12:37:32 jomeli Exp $ */
/* $NoKeywords: $ */
		...
		for(var j = 0; j < g.rows[0].length; j++)
		{
			for(var i = 0; i < g.rows.length; i++)
			{
				if(parseInt(g.vscale*g.rows[i][j]) < 0)
				{
					...
					str += "<IMG SRC=\"clear.gif\" ALIGN=TOP BORDER=0 WIDTH="+parseInt(g.hscale)+" HEIGHT=5>";
				str += "</TD>\n<TD VALIGN=TOP";
				if(g.bgColor) str += " BGCOLOR=\""+g.bgColor+"\"";
				str += ">";
			}
