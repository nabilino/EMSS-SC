...
Slrmc.prototype.isPlanVisibleForGroup=function(company,plangroup,plancode,visibleStructure) {
  ...
  //GDD  01/19/15  If FSA1 and MED1 elected than make not visible
  if (plancode == parent.gFlexMedPlan && parent.gHasMedHighPlan) return 0;	 	
...
Slrmc.prototype.isPlanVisible=function(company,plangroup,planname,electedPlans) {
  ...
  //GDD  01/19/15  If Medical High Ded elected do not show Flex Spend Medical
  if (planname == gFlexMedPlan && gHasMedHighPlan) return 0;
  //GDD  01/19/15  If Medical High Ded Self only elected do not show HSA Family 
  if (planname == gHsaFamilyPlan && gHasMedHighSingle) return 0;
  //GDD  01/19/15  If Medical High Ded Family elected do not show HSA Self 
  if (planname == gHsaSinglePlan && gHasMedHighFamily) return 0;	
  //GDD  01/19/15  only show FSA Waive if Medical High Ded not elected
  if ((planname == gHsaSinglePlan || planname == gHsaFamilyPlan) && !gHasMedHighPlan) return 0;
  //GDD  01/19/15  If Medical High Ded elected only show FSA Limited 
  if (planname == gFlexLtdPlan && !gHasMedHighPlan) return 0; 
  //GDD  01/19/15  If Medical High Ded not elected  show FSA Med 
  if (planname == gFlexMedPlan && gHasMedHighPlan) return 0; 					 
...
Slrmc.prototype.isDualDiscountPlan2=function(company,plancode) {

  return (plancode == "MED1");
...
//			}
				...
				wording += '<p align="left">Your medical FSA account has a per pay period maximum of $98.08 up to an annual maximum of $2550.</p>';
      ...
      if(ruletype=="N") {
        ...
        wording += '<P align="left">You have 90 days from your date of hire to make your election for FSA.  If you choose an amount or decline coverage and want to make a change to your election within your first 90 days, please contact Benefit Services at 381-6180.</P>';
			...
			wording += '<P align="left">Please note:  If you choose to elect Air St. Luke\'s, your membership will be good through March 31, 2016.  At that time, you will need to reelect for the next plan year.</P>';
			...
			wording += '<LI>Benefits for St. Luke\'s employee family members take effect three days following the plan effective date and continue until March 31, 2016.</LI>';
