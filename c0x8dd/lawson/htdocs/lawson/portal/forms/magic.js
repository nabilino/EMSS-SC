/* $Header: /cvs/cvs_archive/LawsonPlatform/ui/portal/forms/magic.js,v 1.123.2.25.4.54.6.16.2.38.2.2 2013/04/03 03:58:59 jquito Exp $ */
/* $NoKeywords: $ */
/* LaVersion=8-)@(#)@9.0.1.11.442 2013-04-05 04:00:00 */
//-----------------------------------------------------------------------------
//		***************************************************************
//		*                                                             *                          
//		*                           NOTICE                            *
...
Magic.prototype.setTabData=function(tabPane, rowNo)
{
	...
	for(var i=0; i < len; i++)
	{
		...
		{
			this.setDetailAreaData(elmNodes[i].nbr)
		}
		else if (elmNodes[i].getAttribute("tp") == "tabregion")
		{
			var par = this.formWnd.document.getElementById(elmNodes[i].nbr)
			var len2 = par.childNodes[0].childNodes.length;
			for (var j = 0; j < len2; j++)
			{
				if ( par.childNodes[0].childNodes[j].className == "activeTab" )
...
Magic.prototype.transact=function(FC, bHasData)
{
	...
	var out = this.portalWnd.httpRequest(this.formWnd.strAGSPath,oPkgDS.document,"","",false);
	
	if (typeof(out) == "undefined") { //session timed-out and loginscreen has been cancelled
		return false;
	}
	...
	for (var i=0; i < len; i++)
	{
		try {
			...
			if (fldId)
			{
				...
				if (!(htmElements[i].getAttribute("nm") == "" && htmElements[i].getAttribute("tp") == "text" && strValue == "")) //ignore fields with no names and possibly no field in magic's datastorage
...
Magic.prototype.createTransactionPackage=function()
{
	...
	for (var i=0; i < loop; i++)
	{
		...
		var oCriteriaRE = new RegExp(/_PDL|_TKN|_CACHE|_OUT|_TRANSID|_DATEFMT|_STEPNBR/g);
...
Magic.prototype.triggerWorkflow=function(FC)
{
	...
	var wfResult = this.portalWnd.httpRequest("/bpm/xml/trigger.do","method=start&xmlDoc="+escape(wfApi),"text/plain","text/xml",false);
...
Magic.prototype.checkSecuredfields=function(fldId)
{
	...
	if(oFormNode)
	{
			...
			if (sec && htmElm && !htmElm.getAttribute("disabled"))
			{	
				htmElm.setAttribute("disabled",true);
			}
			else if (!sec && htmElm && htmElm.getAttribute("disabled"))
			{
				htmElm.setAttribute("disabled", false);
			}
	}
}
