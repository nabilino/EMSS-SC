/* $Header: /cvs/cvs_archive/LawsonPlatform/ui/portal/forms/lawform.js,v 1.154.2.70.4.92.6.52.2.48 2012/08/08 12:37:25 jomeli Exp $ */
/* $NoKeywords: $ */
...
//var batchFldNames= new Array("JOB-NAME","USER-NAME","PRODUCT-LINE"); //annafb - removed PRODUCT-LINE, its purpose is to disable Data Area/ID
//field that is not needed because it is only a text for display and it affects the textbox PRODUCT-LINE
var batchFldNames= new Array("JOB-NAME","USER-NAME");
var batchRoleNames= new Array("","","allow_jobschedule","allow_printfiles");
var LAWFORMJS="forms/lawform.js"		// filename constant for error handler
...
function lawformOnLoad(evt)
{
	...
	&& frmElement.getAttribute("allowTransfer") != "1" )
	{
	...
	// disable the context menu in Netscape
	...
	// field advance only available in IE
	if (portalWnd.oBrowser.isIE)
	{
...
function lawformInitBatchToken()
{
	...
	if (strHKValue && strHKValue.substr(0,9)=="_JOBPARAM")
	{
		...
		for (var i = 0; i < len; i++)
		{
			...
			{
				mElement.disabled=true;
				mElement=window.document.getElementById("SELBUTTON"+fname);
...
function lawformInitTabs(trId)
{
	for (tr in aTabRgns)
	{
		...
		for (var i = 0; i < nbrTabs; i++)
		{
			...
			// is tab disabled?
			if (portalWnd.oBrowser.isIE 
					? tabBtn.firstChild.disabled 
					: tabBtn.childNodes[1].disabled)
				continue;

			if (!tabBtn.className || tabBtn.className != "activeTab")
				continue;
			var tabPane=window.document.getElementById(tabStore.name+"PANE")
			if (!tabPane) break;
...
function lawformDoShowDropTab(id)
{
	...
	for (var i = 0; i < nbrTabs; i++)
	{
		...
		var title=(portalWnd.oBrowser.isIE
			? tabBtn.firstChild.getAttribute("title")
			: tabBtn.childNodes[1].getAttribute("title"));
		var text=(portalWnd.oBrowser.isIE
			? tabBtn.firstChild.innerHTML
			: tabBtn.childNodes[1].innerHTML);
...
function lawformDropTabSel(value)
{
	...
	mElement.focus()
	if (value!=null)
		portalWnd.frmSwitchTab(window,document.getElementById(value))
}
...
function formUnload(bLeavePageTab)
{
	...
	if ( ! window.bFormUnloading && ! window.bPushedForm )
	{
		...
		{
			lastControl.focus()
			if (lawForm.getMessage())
				lawForm.setMessage(lawForm.getMessage());
		}
		else
			lawForm.positionInFirstField()
...
function lawformIncTabPage(mElement,inc)
{
	try {
		...
		while ( htmPane )
		{
			...
			if (!htmRgn) return;
			portalWnd.frmSwitchTab(window,document.getElementById(curPane))
			htmPane=null
		}
	} catch (e) {
...
{
	//just like in lawformTextFocus, make sure the form is ready before validating to avoid hanging LSF-4491
	//annafb - deleted this code portalWnd.frmPositionInToolbar(); to avoid cursor focus in Back button LSF-5526
	if(!formState.formReady)
	{
		return;
	}
...
function lawformCallRules(elemId, rowNbr, bForceCall, bSynch)
{
	...
	|| bForceCall)
	{
		...
		try
		{
			...
			{
				var xmlDoc=portalWnd.httpRequest(RulesCall, null, null, null, false);
				if (xmlDoc.status)
				{
...
function lawformCalendarSel(date)
{
	var mElement=lastControl
	if (date)
	{
...
function lawformGetNextElement(evt)
{
	...
	if (!ed || ed == "upper" || (ed == "numeric" && !evtElem.getAttribute("decsz")))
	{
		...
		else if (evtElem.value.length == parseInt(sz, 10) && parseInt(sz,10) == 1) //create exception for sz=1 fields
			fldNbr = lawformGetNextFieldNbr(evtElem.getAttribute("id"));
...
function lawformGetNextFieldSibling(sibNode)
{
	...
	while (retNode != null && !bDone)
	{
		...
		{
			retFldNbr = lawformGetNextDtlFieldNbr(retNode.firstChild);
			if (retFldNbr) return retFldNbr;
		}
...
function lawformGetNextDtlFieldNbr(fldNode)
{
	...
	while (retNode != null && !bDone)
	{
		if (retNode.nodeName == "push" && document.getElementById(retNode.getAttribute("nbr").replace("r0", "r" + curRow)) && document.getElementById(retNode.getAttribute("nbr").replace("r0", "r" + curRow)).value != "") 	//also check if btn is existing.
		{
		...
		while (retNode && retNode.nodeName != "fld")
			retNode = retNode.nextSibling;
		var tp;
		if (retNode && retNode.nodeName != "push")
		{
