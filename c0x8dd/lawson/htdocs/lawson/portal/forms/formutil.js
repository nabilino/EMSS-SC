/* $Header: /cvs/cvs_archive/LawsonPlatform/ui/portal/forms/formutil.js,v 1.98.2.25.4.63.6.25.2.21 2012/08/08 12:37:26 jomeli Exp $ */
/* $NoKeywords: $ */
...
function frmBuildXpressCall(TKN, PDL, mode, hkey, id, custom, output, host)
{
	...
	var allowTransfer = (hkey.indexOf("&_ALLOWTRANSFER=1") != -1) ? "1" : "0";
	hkey = (allowTransfer == "1") ? hkey.substring(0, hkey.indexOf("&_ALLOWTRANSFER=1")) : hkey;
...
LawFormObj.prototype.setElementValue=function(elemID,newValue,rowNbr)
{
	...
	try {
		...
		if (!this.magic.initialized || !mElement || mElement.getAttribute("nm") == "") //add exception to unbounded fields
...
LawFormObj.prototype.changeTabName=function(tabNbr,name)
{
	...
	this.UIDocument.getElementById(tabNbr).firstChild.innerHTML=name;
}
...
{
	this.portalWnd=portalWnd;
	var oXML = portalWnd.httpRequest(this.portalWnd.lawsonPortal.formsDir + "/actionframework.xsl");
...
ActionFrameworkService.prototype.loadForm=function(actionFrameworkCall)
{
	...
	*/
	if (!myXML)
	{
		...
		myXML=this.portalWnd.httpRequest(api,null,"","",false);
...
{
	this.portalWnd=portalWnd;
	var oXML = portalWnd.httpRequest(this.portalWnd.lawsonPortal.formsDir + "/lawform.xsl");
	this.oXSL=portalWnd.objFactory.createInstance("DOM");
	this.oXSL.async=false;
	this.oXSL.loadXML(oXML.xml);
	if (this.oXSL.parseError.errorCode!=0)
...
IEFormService.prototype.loadForm=function(xpressCall)
{
	...
	this.uselists=this.portalWnd.getXpressParm("uselists",parms);
	this.iosVersion=this.portalWnd.getXpressParm("iosVersion",parms);;
	this.allowTransfer=this.portalWnd.getXpressParm("allowTransfer",parms);
	this.status=null;
	...
	try {
		...
		frmNode.setAttribute("uselists",this.uselists);
		frmNode.setAttribute("iosVersion",this.iosVersion);
		frmNode.setAttribute("allowTransfer", this.allowTransfer);
...
LawFieldScroll.prototype.issueIdaCall=function(idaCall)
{
	...
	var xmlDoc=this.portalWnd.httpRequest(idaAPI,null,"","",false);

	var bDisp = (idaAPI.indexOf("&_FF=") != -1) ? false : true;
	var msg=this.portalWnd.lawsonPortal.getPhrase("ERROR_LOAD_XML")+"\n";
	this.portalWnd.oError.setMessage(msg);
	return (this.portalWnd.oError.isErrorResponse(xmlDoc,true,bDisp,false,msg,this.formWnd)
		? null 
