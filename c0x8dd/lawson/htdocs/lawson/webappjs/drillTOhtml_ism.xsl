 ...
 *                                                             *
 ************************************************************-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xt="http://www.jclark.com/xt" xmlns:fn="http://www.w3.org/2005/xpath-functions" version="2.0" extension-element-prefixes="xt">
				...
				<tbody>
					...
					<xsl:when test="not(normalize-space($drillCalls)='')">	
						<xsl:for-each select="IDARETURN/LINES/LINE">
							...
							<xsl:choose>
								<xsl:when test="not(normalize-space(//IDARETURN/@title)='') and ((normalize-space(IDACALL)='')or(IDACALL/@type='CMT')or(IDACALL/@type='URL'))">
								...
								<xsl:otherwise>
									<xsl:for-each select="COLS/COL">
											<xsl:choose>
											<xsl:when test="not(normalize-space(node())='') and
											(
												(normalize-space(node())='Assignment') or
										...
										<td style="text-align:center">
												<a styler="hyperlink" style="font-size:inherit !important;">
													<xsl:attribute name="href">
														<xsl:value-of select="substring-before($drillURL,'_ism')"/>
														<xsl:value-of select="substring-after($drillURL,'_ism')"/>
													</xsl:attribute>
											...
											<xsl:when test="not(normalize-space(node())='') and not(normalize-space(//IDARETURN/@title)='')">
