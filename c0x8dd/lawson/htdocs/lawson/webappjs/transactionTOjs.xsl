<?xml version="1.0"?>
<!-- $Header: /cvs/cvs_archive/applications/webtier/webappjs/transactionTOjs.xsl,v 1.9.2.3.2.4 2014/02/18 16:42:31 brentd Exp $ -->
 ...
 ************************************************************-->
 <xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xt="http://www.jclark.com/xt" version="1.0" extension-element-prefixes="xt">
	...
	<xsl:variable name="xslName">&amp;amp;_XSL1=/lawson/webappjs/transactionTOjs.xsl</xsl:variable>
		...
		<html lang="en">
			<head>
				...
				<script>
					...
					<xsl:text>var field = new Array();</xsl:text>
					
					<xsl:for-each select="//*/*/*">
						<xsl:variable name="fieldNm">
							<xsl:call-template name="incrementRowNbr">
								<xsl:with-param name="stringVal" select="name()"/>
							</xsl:call-template>
						</xsl:variable>	
						<xsl:text>field["</xsl:text>
						<xsl:value-of select="$fieldNm"/>
						<xsl:text>"] = new V("</xsl:text>
						<xsl:value-of select="$fieldNm"/>	
						<xsl:text>","</xsl:text>
						<xsl:choose>
							<xsl:when test="$fieldNm='FldNbr'">
								<xsl:call-template name="incrementRowNbr">
					...
					var NbrRecs = field.length;
					var RtnMsgField = '<xsl:value-of select="$fldNbr"/>';
					var RtnMsgErrNbr = '<xsl:value-of select="//MsgNbr"/>';
					var RtnMsg = '<xsl:value-of select="//Message"/>';
					var CrtRequest = '';
					var CrtScreen = '';
					var CrtPassXlt = '';
					var CrtDspXlt = '';
